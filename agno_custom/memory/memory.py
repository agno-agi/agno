...
from agno_custom.run.response import RunResponse
from agno_custom.run.team import TeamRunResponse
...
class TeamMemberInteraction:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "TeamMemberInteraction":
        ...
        # NOTE: bugfix - added missing support to create TeamMemberInteractions from Teams (TeamRunResponse):
        if "team_id" in member_response:
            return cls(
                member_name=data["member_name"], task=data["task"], response=TeamRunResponse.from_dict(member_response)
            )
        return cls(member_name=data["member_name"], task=data["task"], response=RunResponse.from_dict(member_response))
...
class Memory:
    ...
    # NOTE: New Feature
    # This method is used to get the most recent runs' messages that fit within a token budget.
    def get_max_messages_from_history(
        ...
        usage per run (after applying the same filtering rules as
        `get_messages_from_last_n_runs`) and accumulates runs until adding the
        next run would exceed `max_tokens`. Only a single system message is
        included overall.
        ...
        Args:
            ...
            skip_history_messages: Skip messages tagged as history in previous runs.
        ...
        """
        if not self.runs or max_tokens <= 0:
            return []
        ...
        # Iterate from newest to oldest, selecting runs that fit within the budget
        total_tokens = 0
        system_added = False
        ...
        for run_response in reversed(session_runs):
            ...
            for message in run_response.messages:
                ...
                if message.role == "system":
                    # Consider system message only if not already added globally
                    if not system_added and system_in_this_run is None:
                        system_in_this_run = message
                    continue  # don't add system here yet; we'll add it once per run if applicable
                else:
                    filtered_messages.append(message)

            # If a system message is applicable and not already added globally, include it at the start
            ...
            run_tokens = count_tokens(candidate_messages, model_encoding=model_encoding)
            ...
            if total_tokens + run_tokens <= max_tokens:
                ...
                if system_in_this_run and not system_added:
                    system_added = True
            # If this run doesn't fit, skip it and continue to older runs

        # Flatten selected runs in chronological order
    ...
    # NOTE: Custom feature: added ability to specify how many of the most recent interactions to share via <max_interactions_to_share> to manage context size
    def get_team_member_interactions_str(self, session_id: str, max_interactions_to_share: int = None) -> str:
        if not self.team_context:
            return ""
        ...
        if session_team_context and session_team_context.member_interactions:
            ...
            # Only add last <max_interactions_to_share> interactions starting from the most recent interaction
            for interaction in reversed(session_team_context.member_interactions):
                ...
                interaction_str = (
                    f"Member: {interaction.member_name}\nTask: {interaction.task}\nResponse: {response_content}\n\n"
                )
                # Add the current interaction at the start of the list to maintain the order
            ...
            log_debug(
                f"Added {interactions_added} previous member interactions out of {len(session_team_context.member_interactions)} total"
            )
