...
def _log_messages(messages: List[Message]) -> None:
    ...
    for m in messages:
        ...
        # m.log(metrics=False)
        # NOTE: New Feature - truncate system messages for readability
        log_message(m, system_message_truncate_length=SETTINGS.AGNO_SYSTEM_MESSAGE_LOG_TRUNCATE_LENGTH, metrics=False)
    ...
    def run_function_call(
        ...
        if isinstance(function_call.result, (GeneratorType, collections.abc.Iterator)):
            for item in function_call.result:
                ...
                elif isinstance(item, BaseBanavoStreamEvent):
                    yield item
