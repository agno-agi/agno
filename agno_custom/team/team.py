...
class Team:
    ...
    def _handle_model_response_chunk(
        ...
        if isinstance(model_response_event, BaseBanavoStreamEvent):
            # UI events (UserFacingMessageEvent etc.) always propagate regardless of stream_member_events flag
            yield model_response_event  # type: ignore
        elif isinstance(model_response_event, tuple(get_args(RunResponseEvent))) or isinstance(
    ...
    def get_system_message(
        ...
        system_message_content += "\n<how_to_respond>\n"
        # NOTE: New Feature - allow task transfer instructions to be overriden
        if self.custom_transfer_system_prompt is not None:
            system_message_content += self.custom_transfer_system_prompt
        elif self.mode == "coordinate":
            system_message_content += (
