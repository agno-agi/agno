...
class DocumentationGap(BaseModel):
    ...
    priority: str = Field(description="Priority: high, medium, low")
    location: str = Field(
        description="File and location where documentation is needed"
    )
    description: str = Field(description="What documentation is missing or needs improvement")
...
class CodeReviewResult(BaseModel):
    ...
    risk_level: str = Field(
        description="Overall risk level: high, medium, low"
    )
    estimated_review_time: Optional[str] = Field(
        default=None, description="Estimated time for a human to review (e.g., '15 minutes')"
    )
