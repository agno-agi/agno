...
Note: The Meeting Tasks Agent requires live Linear API access for full testing.
This evaluate script provides basic validation of agent configuration.

For full testing:
- Run the examples with valid LINEAR_API_KEY
- Verify task extraction and issue creation manually
...
def test_agent_import(verbose: bool = True) -> dict[str, Any]:
    ...
    if verbose:
        ...
        print("  Description: Verify meeting_tasks_agent imports correctly")

    try:
        from agent import meeting_tasks_agent  # noqa: F401

        if verbose:
            print("  [PASS] meeting_tasks_agent imported successfully")
...
def test_tools_available(verbose: bool = True) -> dict[str, Any]:
    ...
    try:
        ...
        tools = meeting_tasks_agent.tools
...
def test_helper_import(verbose: bool = True) -> dict[str, Any]:
    ...
    if verbose:
        ...
        print("  Description: Verify process_meeting_file function imports")

    try:
        from agent import process_meeting_file  # noqa: F401

        if verbose:
            print("  [PASS] process_meeting_file function imported")
...
def run_evaluation(verbose: bool = True) -> dict[str, Any]:
    ...
    if verbose:
        print("=" * 60)
        print("Meeting Tasks Agent - Evaluation")
        print("=" * 60)
...
if __name__ == "__main__":
    print("\n" + "=" * 60)
    print("Meeting Tasks Agent - Evaluation Suite")
    print("=" * 60)
    print(
        """
This script tests the meeting tasks agent's configuration.
