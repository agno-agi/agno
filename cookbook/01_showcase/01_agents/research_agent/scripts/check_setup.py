...
2. API keys (OPENAI_API_KEY, PARALLEL_API_KEY)
...
def check_api_keys() -> bool:
    ...
    # Parallel is required for web search
    parallel_key = os.environ.get("PARALLEL_API_KEY")
    if parallel_key:
        print(f"   [OK] PARALLEL_API_KEY is set ({parallel_key[:8]}...)")
    else:
        print("   [FAIL] PARALLEL_API_KEY not set (required for web search)")
        print("   -> Run: export PARALLEL_API_KEY=your-key")
        print("   -> Get a key at: https://www.parallel.ai/")
        all_set = False
...
def check_import() -> bool:
    ...
    try:
        ...
        from agent import research_agent  # noqa: F401

        print("   [OK] research_agent imported successfully")
        return True
...
def main() -> int:
    ...
    print("=" * 60)
    print("Research Agent Tutorial - Setup Check")
    print("=" * 60)
    ...
    if all_passed:
        ...
        print()
        print("Try:")
        print("  python examples/01_quick_research.py")
        return 0
    else:
