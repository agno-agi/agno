...
Note: The Startup Analyst requires live ScrapeGraph API access for full testing.
This evaluate script provides basic validation of agent configuration.

For full testing:
- Run the examples with valid SCRAPEGRAPH_API_KEY
- Verify startup analysis quality manually

Prerequisites:
    1. SCRAPEGRAPH_API_KEY set
...
def test_agent_import(verbose: bool = True) -> dict[str, Any]:
    ...
    if verbose:
        ...
        print("  Description: Verify startup_analyst imports correctly")

    try:
        from agent import startup_analyst  # noqa: F401

        if verbose:
            print("  [PASS] startup_analyst imported successfully")
...
def test_tools_available(verbose: bool = True) -> dict[str, Any]:
    """Test that ScrapeGraph tools are available."""
    if verbose:
        print("\n  Testing: tools_available")
        print("  Description: Verify ScrapeGraphTools are configured")

    try:
        from agent import startup_analyst

        # Check that tools are configured
        tools = startup_analyst.tools
        tool_names = [t.__class__.__name__ for t in tools if hasattr(t, "__class__")]

        has_scrape = any("ScrapeGraph" in name for name in tool_names)
        has_reasoning = any("Reasoning" in name for name in tool_names)

        if has_scrape and has_reasoning:
            if verbose:
...
def test_schema_import(verbose: bool = True) -> dict[str, Any]:
    ...
    if verbose:
        ...
        print("  Description: Verify StartupReport schema imports")

    try:
        from schemas import StartupReport  # noqa: F401

        if verbose:
            print("  [PASS] StartupReport schema imported")
...
def run_evaluation(verbose: bool = True) -> dict[str, Any]:
    ...
    if verbose:
        print("=" * 60)
        print("Startup Analyst - Evaluation")
        print("=" * 60)
    ...
    if verbose:
        ...
        print(
            """
Note: Full evaluation requires live ScrapeGraph API access.
...
if __name__ == "__main__":
    print("\n" + "=" * 60)
    print("Startup Analyst - Evaluation Suite")
    print("=" * 60)
    print(
        """
This script tests the startup analyst's configuration.

Note: Full functional testing requires:
  - Valid SCRAPEGRAPH_API_KEY
