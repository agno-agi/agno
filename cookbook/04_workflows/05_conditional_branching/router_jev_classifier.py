...
judgments in roughly a tenth of a second. That makes it a cheap first step - classify the
...
class Triage(BaseModel):
    kind: Literal["bug_report", "feature_request", "question"] = Field(
    ...
    is_urgent: bool = Field(
        description="Does the message say that work is blocked or that time is short?"
    )
...
def route_by_triage(step_input: StepInput) -> List[Step]:
    ...
    print(f"Jev classified the message as {triage.kind} (urgent: {triage.is_urgent})")
...
if __name__ == "__main__":
    for message in (
        ...
        print(str(response.content)[:500])
