...
def resolve_executor_pause(run_output):
    ...
    for req in run_output.step_requirements or []:
        if req.requires_executor_input:
            ...
            answer = Prompt.ask("  Approve publish?", choices=["y", "n"], default="y").strip().lower()
            for executor_req in req.executor_requirements or []:
...
if __name__ == "__main__":
    ...
    pause_count = 0
    for event in workflow.run("Write and publish a short blog post about AI safety", stream=True):
