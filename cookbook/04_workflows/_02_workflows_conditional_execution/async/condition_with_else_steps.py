"""
Condition with else_steps (async) — Customer Support Routing

Demonstrates the `else_steps` parameter in `Condition`. When the evaluator
returns True the `steps` (if-branch) run; when it returns False the
`else_steps` (else-branch) run instead.

Scenario:
  1. A triage evaluator checks whether the customer query is technical.
  2. Technical queries -> diagnostic agent then engineering agent (if-branch).
  3. Non-technical queries -> general support agent (else-branch).
  4. Both branches feed into a final follow-up step.
"""
...
async def main():
    ...
    # Non-technical query -> else branch
    print()
    print("=" * 60)
    ...
    await workflow_2.aprint_response(
