...
filesystem_agent_2 = Agent(
    id="filesystem-agent-2",
    name="File System Agent 2",
    model=OpenAIResponses(id="gpt-5.6-luna"),
    db=db,
    filesystem=fs,
    instructions="Keep durable working notes in your filesystem 2.",
    markdown=True,
)
filesystem_agent_3 = Agent(
    id="filesystem-agent-3",
    name="File System Agent 3",
    model=OpenAIResponses(id="gpt-5.6-luna"),
    db=db,
    filesystem=fs,
    instructions="Keep durable working notes in your filesystem 3.",
    markdown=True,
)
custom_filesystem_agent = Agent(
...
if __name__ == "__main__":
    agent_os.serve(app="multiple_filesystem:app", reload=True)
