"""
Discord Gateway Channel Summarizer
==================================

An agent that reads recent Discord channel history and produces a structured
summary, triggered by a simple mention: ``@YourBot summarize this channel``.
Replies land directly in the channel (``reply_in_thread=False``); follow-up
questions work via session history.

Key concepts:
  - ``DiscordTools`` with ``enable_get_channel_messages`` lets the agent
    pull channel history as a tool call; the live tool-call status shows
    ``Running tool: get_channel_messages...``.
  ...
  - ``add_history_to_context`` + a ``db`` enables follow-up questions.
...
**View Channels** so it can fetch channel messages. No public URL or tunnel
needed.
"""
...
agent_db = SqliteDb(
    session_table="discord_summarizer_sessions", db_file="tmp/discord_gw_summarizer.db"
)
...
agent_os = AgentOS(
    agents=[summarizer],
    interfaces=[DiscordGateway(agent=summarizer, reply_in_thread=False)],
)
...
if __name__ == "__main__":
    ...
    """
    agent_os.serve(app="channel_summarizer:app", reload=False)
