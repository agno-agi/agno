"""
Discord Gateway Research Assistant
==================================

An agent that combines Discord channel/server introspection with web search,
reachable through fluid chat: mention it with a research question, then keep
refining inside the thread it opens. The agent can list channels in the
server, fetch recent channel history, and search the web for external context.
...
settings, and install discord.py. Grant the bot the **Read Message History**
and **View Channels** permissions. No public URL or tunnel needed.
"""
...
agent_db = SqliteDb(
    session_table="discord_research_sessions", db_file="tmp/discord_gw_research.db"
)
...
agent_os = AgentOS(
    ...
    interfaces=[DiscordGateway(agent=research_assistant)],
)
...
if __name__ == "__main__":
    ...
    """
    agent_os.serve(app="research_assistant:app", reload=False)
