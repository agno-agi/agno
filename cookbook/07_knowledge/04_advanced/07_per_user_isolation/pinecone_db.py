...
Pinecone stores the owner in each vector's metadata; shared chunks omit the
field and scoped reads filter on caller OR field-not-present.
...
# Start clean. Provisioning a serverless index takes minutes, so the index is
# reused and only its vectors are cleared.
vector_db.create()
vector_db.delete()
