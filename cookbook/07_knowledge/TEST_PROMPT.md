# Prompt to test: `cookbook/07_knowledge`

Use this prompt with an AI coding agent to test and validate the knowledge cookbook.
...
1. Spawn a parallel agent for each top-level subdirectory under `cookbook/07_knowledge/`. Each agent handles one subdirectory independently, including any nested subdirectories within it.
2. Each agent must:
   ...
   e. Update `cookbook/07_knowledge/<SUBDIR>/TEST_LOG.md` with fresh PASS/FAIL entries per file.
