...
This cookbook demonstrates using the S3Config.list_files() method
directly without running an AgentOS server. Useful for scripts
and testing.
...
def list_all_files():
    ...
    if result.next_cursor:
        print(f"\nMore files available (cursor: {result.next_cursor[:20]}...)")
...
def paginate_all():
    ...
    while True:
        result = s3_config.list_files(limit=10, cursor=cursor)
        total += len(result.files)
        print(f"Page {page}: {len(result.files)} files")
