...
1. Test root-level files and each subdirectory. Spawn a parallel agent for `memory_manager/` and `optimize_memories/` if desired.
2. Each agent must:
   ...
   e. Update `cookbook/11_memory/TEST_LOG.md` (root), `cookbook/11_memory/memory_manager/TEST_LOG.md`, and `cookbook/11_memory/optimize_memories/TEST_LOG.md` with fresh PASS/FAIL entries per file.
