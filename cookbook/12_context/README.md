...
All read+write providers (`WikiContextProvider`, `DatabaseContextProvider`, `SlackContextProvider`, `GitHubContextProvider`) accept `read=True, write=True` flags. Set `write=False` for a read-only surface (e.g. a code-managed voice wiki, an analytics-only DB), or `read=False` for a write-only sink. Both `False` raises.
...
| `18_github.py` | GitHub repo: read public repo (always) + optional edit-via-PR via `GITHUB_WRITE_REPO` |
| `19_github_changelog.py` | Generate a changelog from recent GitHub commits |
| `20_github_multi_repo.py` | Query multiple GitHub repos with a single agent |
...
OPENAI_API_KEY=... .venvs/demo/bin/python cookbook/12_context/18_github.py
```
