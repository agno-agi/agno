"""
AgentCash MCP Tools - Give Your Agent Money
=============================
...
Configuration:
- Wallet location: ~/.agentcash/wallet.json (auto-generated)
- Environment variable: AGENTCASH_PRIVATE_KEY (optional, use existing wallet)
"""
...
async def onboarding_example() -> None:
    ...
    On first run:
    - Wallet auto-generated at ~/.agentcash/wallet.json (unique per user)
    - Balance will be $0
    ...
    async with MCPTools("npx -y agentcash@latest") as agentcash:
        ...
        await agent.aprint_response(
            "I just installed agentcash. Show me my wallet info and how to get started.",
            stream=True,
        )
    ...
    async def spending_hook(
        ...
        if function_name == "fetch":
            ...
            if total_spent["amount"] > 4.50:
                logger.warning(f"Budget alert: ${total_spent['amount']:.2f} spent")
...
# Example 6: Production-ready with cost controls
    ...
    async def production_hook(
        ...
        if function_name == "fetch":
            ...
            try:
                ...
                logger.info(f"OK {url}: ${actual_cost:.4f}")
                return result

            except Exception as e:
                logger.error(f"FAIL {url}: {str(e)}")
