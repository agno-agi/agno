...
cursor = conn.execute(
    "SELECT * FROM agno_auth_tokens WHERE provider='google' AND user_id='test-user-1' AND service='gmail'"
)
row = cursor.fetchone()
conn.close()
...
conn = sqlite3.connect(str(db_path))
conn.row_factory = sqlite3.Row
cursor = conn.execute("SELECT provider, user_id, service FROM agno_auth_tokens")
...
conn = sqlite3.connect(str(db_path))
cursor = conn.execute(
    "SELECT COUNT(*) FROM agno_auth_tokens WHERE user_id='test-user-2'"
)
...
print(
    f"  sqlite3 {db_path} 'SELECT provider, user_id, service, created_at, updated_at FROM agno_auth_tokens;'"
)
