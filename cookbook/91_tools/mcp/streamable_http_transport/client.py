"""
Show how to connect to MCP servers that use either SSE or Streamable HTTP transport using our MCPTools class.
...
# Passing one MCPTools instance per server, we can connect to multiple MCP servers at once,
# even if they use different transports. In this example we connect to both our example server
# (Streamable HTTP transport), and a different server (stdio transport).
