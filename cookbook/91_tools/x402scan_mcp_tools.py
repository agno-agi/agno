...
# Example 8: Learning Machine - agent learns cost optimization
async def learning_example() -> None:
    ...
    Requires PostgreSQL for persistent storage.
    """
    ...
    db = PostgresDb(
        ...
        table_name="x402_learning_sessions"
    )
    ...
    async with MCPTools("npx -y @x402scan/mcp@latest") as x402:
        agent = Agent(
            ...
            Store learnings with save_user_memory tool.""",
            markdown=True,
        )
        ...
        print("="*60)
        memories = agent.learning_machine.user_memory_store.read(user_id="user@company.com")
        for mem in memories:
            print(f"  • {mem.content}")
        print("="*60 + "\n")
