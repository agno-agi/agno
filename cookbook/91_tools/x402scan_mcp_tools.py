    ...
    async def spending_hook(
        ...
        if function_name == "fetch" and isinstance(result, dict):
            ...
            if cost:
                ...
                logger.info(
                    f"Transaction cost: ${cost:.4f} (Total: ${total_spent['amount']:.2f})"
                )
    ...
    async with MCPTools("npx -y @x402scan/mcp@latest") as x402:
        ...
        if response and response.content:
            ...
            for source in report.sources_used:
                print(
                    f"  • {source.provider}: ${source.cost:.4f} ({source.data_quality} quality)"
                )
...
async def skills_example() -> None:
    ...
    if not skills_dir.exists():
        print(f"Skills not found at {skills_dir}")
        print(
            "Install: git clone https://github.com/merit-systems/x402scan-skills ~/x402scan-skills"
        )
        return
    ...
    async def production_hook(
        ...
        if function_name == "fetch":
            ...
            try:
                ...
                actual_cost = (
                    result.get("cost", estimated_cost)
