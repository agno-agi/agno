...
1. Spawn a parallel agent for each subdirectory under `cookbook/92_integrations/` (`a2a/`, `discord/`, `memory/`, `observability/`, `rag/`, `surrealdb/`). Each agent handles one subdirectory independently, including any nested subdirectories.
2. Each agent must:
