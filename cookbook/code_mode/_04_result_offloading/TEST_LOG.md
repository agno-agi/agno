...
**Description:** An agent with `offload=True` over SqliteDb whose `fetch_catalog` tool returns a 4000-row, 146,300-character table. The agent was asked which warehouse SKU-00042 is in and how many rows the catalog has, and told to use `search_result` rather than re-fetching.
