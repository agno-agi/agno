...
**Result:** Summary line: "wrote 6 rows to .../math_problems.jsonl, kept 6, dropped 0 of 6 generated (every kept answer re-derived in code)". Ran twice today (second run after a cosmetic ruff reformat); both runs kept 6/6 - the model followed the exactly-two-numbers shape every time, e.g. 6 lb/cow x 74 cows = 444 (stated 444) and 15 carts x 6 kits = 90 (stated 90). The drop path did not fire in either run; it exists for the occasional extra-digit or wrong-product return. Problems are recognizably persona-grounded (grain rations, suture kits, slider portions, editing hours, fuel gallons, shelving).
