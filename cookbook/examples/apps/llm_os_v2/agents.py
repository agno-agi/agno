...
def get_sql_agent(
    ...
    """
    # Add log at the beginning of the function
    logger.info(f"Entering get_sql_agent with model_id={model_id}, session_id={session_id}, user_id={user_id}")
    ...
    # Add log before creating the Agent object
    logger.info(f"Creating Agent instance for SQL Agent with {model=}")
    agent = Agent(
        name="SQL Agent",
        model=model,
        user_id=user_id,
        ...
        tools=[
            # Add log before initializing SQLTools
            logger.info("Initializing SQLTools") or SQLTools(db_url=db_url, list_tables=False),
            # Add log before initializing FileTools
            logger.info("Initializing FileTools") or FileTools(base_dir=output_dir),
            # Add log before initializing ReasoningTools
            logger.info("Initializing ReasoningTools") or ReasoningTools(add_instructions=True),
        ],
    ...
    # Add log before returning the agent object
    logger.info(f"Returning agent object: {agent.name}")
    return agent
