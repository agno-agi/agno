...
class ProcurementAnalysisRequest(BaseModel):
    companies: List[str] = Field(..., min_length=1, max_length=5, description="List of 1-5 companies to analyze")
    category_name: str = Field(..., min_length=1, description="Category name for analysis")
    analyses_requested: List[str] = Field(..., min_length=1, description="List of analysis types to perform")
    model_id: str = Field(default="gpt-4o", description="AI model to use")
    buyer_org_url: Optional[str] = Field(default=None, description="Buyer organization URL for context")
    annual_spend: Optional[float] = Field(default=None, description="Annual spend amount for context")
    region: Optional[str] = Field(default=None, description="Regional context")
    incumbent_suppliers: List[str] = Field(default_factory=list, description="Current/incumbent suppliers")
...
class AnalysisConfig(BaseModel):
    ...
    model_id: str = Field(default="gpt-4o", description="AI model to use for this analysis")
    max_companies: int = Field(default=5, description="Maximum number of companies to analyze")
    include_market_data: bool = Field(default=True, description="Whether to include market data in analysis")
    include_financial_data: bool = Field(default=True, description="Whether to include financial data in analysis")
...
class SupplierProfile(BaseModel):
    ...
    technical_capabilities: List[str] = Field(default_factory=list, description="Technical capabilities")
    certifications: List[str] = Field(default_factory=list, description="Quality certifications")
    annual_revenue: Optional[float] = Field(default=None, description="Annual revenue in USD")
    employee_count: Optional[int] = Field(default=None, description="Number of employees")
    key_differentiators: List[str] = Field(default_factory=list, description="Key competitive advantages")
    financial_stability_score: Optional[int] = Field(default=None, ge=1, le=10, description="Financial stability score (1-10)")
    suitability_score: Optional[int] = Field(default=None, ge=1, le=10, description="Suitability score for requirements (1-10)")
...
class AnalysisResult(BaseModel):
    ...
    score: Optional[int] = Field(default=None, ge=1, le=9, description="Overall score (1-9 scale)")
    summary: Optional[str] = Field(default=None, description="Analysis summary")
    detailed_findings: Optional[str] = Field(default=None, description="Detailed analysis findings")
