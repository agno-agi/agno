# Discord Interactions Cookbook

Examples for the `DiscordInteractions` interface: slash commands over
Discord's HTTP Interactions API. Discord sends a signed HTTPS POST to your
AgentOS endpoint for every command invocation; the interface verifies the
Ed25519 signature, acks within 3 seconds, and streams the answer back through
the interaction webhook.

## What makes Interactions special

### Installed to users, triggered everywhere

Discord apps can be installed in two ways, and these commands register for
both (`user_install=True` is the default):

- **Server install** — the classic bot-in-a-server setup.
- **User install** — the app is installed to a *user's account*. The commands
  then travel with that user: they can run `/ask` in **any server, DM, or
  group DM**, even where the bot was never invited. Replies ride the
  interaction webhook, so no bot membership or channel permissions are needed.

In channels where the app lacks access (e.g. a server that never installed
the bot), Discord may force the reply to be ephemeral — which pairs naturally
with the next feature. Pass `user_install=False` to register classic
server-install-only commands.

### Ephemeral replies

Interaction responses support an ephemeral mode (flag `64`): the reply is
visible **only to the person who invoked the command**, shows a "Only you can
see this" notice, is dismissible, and never creates a thread or posts anything
to the channel. Other members see nothing at all.

Two ways to use it:

- Per invocation: `/ask question:... ephemeral:True`
- As the default for every `/ask`: `DiscordInteractions(agent=..., ephemeral=True)`
  (a per-invocation `ephemeral:False` still overrides back to public)

This is exclusive to the Interactions transport — gateway messages are
regular Discord messages and cannot be ephemeral.

### Best for one-shot interactions

Slash commands are structured (named options, attachments, autocomplete-ready)
and every invocation is a clean, self-contained request — ideal for quick
lookups, utilities, and "ask once, get an answer" flows. For long free-form
conversations, the [gateway adapter](../gateway_adapter/) is the better fit.
...
4. Under **Installation**, enable **Guild Install** and **User Install**, add
   the `bot` and `applications.commands` scopes, and invite the bot to a
   server. Grant at minimum **Send Messages**, **Create Public Threads**, and
   **Send Messages in Threads** (the bot replies in threads by default). If
   you disable threads via `reply_in_thread=False`, only **Send Messages** is
   needed.
...
In any server where the bot is installed (or anywhere, if you installed the
app to your account):

```
/ask question: what's the weather on mars?
/ask question: describe this image file: <drop an image>
/ask question: draft a reply to this awkward email ephemeral: True
/new                                    # start a fresh conversation in the current channel
```

## Slash Commands

- `/ask question:<text> [file:<attachment>] [ephemeral:<bool>]` — ask the
  agent. With `reply_in_thread=True` (the default) a new thread is opened for
  the answer; inside an existing thread, the reply stays in that thread. With
  `ephemeral:True`, the reply is private to you and no thread is created.
...
you want it to introspect. The interface automatically passes
...
| `DISCORD_BOT_TOKEN` | Yes (default) | Bot token. Used to register slash commands at startup. Only optional if you pass `auto_register_command=False` to the `DiscordInteractions(...)` constructor. |
| `OPENAI_API_KEY` | Depends | Required for OpenAI-based examples. |

## Features

- `/ask` with a required `question`, optional `file` attachment, and optional
  `ephemeral` flag
- `/new` to rotate the current user's session in the current channel
- User-installable commands (`user_install=True` by default): usable in any
  server, DM, or group DM
- Ephemeral mode per invocation or as the interface default (`ephemeral=True`)
...
- Long responses are chunked at paragraph/line/word boundaries into
  Discord's 2000-character messages (ephemeral overflow rides as ephemeral
  followups).
...
| "This application did not respond in time" | Tunnel down, URL rotated, or the running app doesn't serve `/discord/interactions` | Verify ngrok is up, the portal URL is current, and you're running an interactions example (not a gateway-only app) |
...
| `/ask` missing the ephemeral option | Stale registration | Restart the app (commands bulk-overwrite on startup); Discord clients may cache for a few minutes |
| Can't use commands in other servers/DMs | App not user-installed, or `user_install=False` | Enable User Install in the portal's Installation page and install the app to your account |
