...
captures durable details (name, preferences, projects) from every ``/ask``,
keyed by the asker's Discord user id - so memories follow the user across
channels, threads, and even servers.

Try: ``/ask question:My name is Ray and I prefer short answers`` then, in a
different channel, ``/ask question:What do you know about me?``
...
agent_db = SqliteDb(session_table="discord_sessions", db_file="tmp/discord_memory.db")
...
if __name__ == "__main__":
    ...
    """
    agent_os.serve(app="agent_with_memory:app", reload=True)
