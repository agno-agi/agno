...
research_team = Team(
    ...
    tools=[DuckDuckGoTools(verify_ssl=False)],  # Team uses DuckDuckGo for research
    ...
    """).strip(),
    db=SqliteDb(db_file="tmp/dbs/full_compression_openai.db"),
    show_members_responses=True,
    compression_manager=compression_manager,
    add_history_to_context=True,
    num_history_runs=5,
    session_id="full_compression_team_demo_openai",
    markdown=True,
...
if __name__ == "__main__":
    asyncio.run(
        research_team.aprint_response(
            "What are the latest developments in AI agents? Which companies dominate the market? Find the latest news and reports on the companies. Based on your research, which competitor has the best pricing for small teams? Search for the latest news and reports on the companies.",
            stream=True,
        )
    )
