...
def get_read_result_function(agent: Agent, run_context: RunContext, async_mode: bool = False) -> Function:
    """Factory for the read_result tool (result offloading)."""

    def _resolve(result_id: str, row: Optional[Dict[str, Any]]) -> Optional[str]:
        """The error string when the id is unknown or belongs to another session."""
        if row is None:
            return f"Error: unknown result id {result_id}"
        # Access rule: a stored result is served only to its own session.
    ...
    def read_result(result_id: str, start_line: int = 1, end_line: Optional[int] = None) -> str:
        ...
        the reply names the next start_line when there is more.
        ...
        if store is None:
            return "Error: result offloading is not enabled for this agent"
        try:
            error = _resolve(result_id, store.get_row(result_id))
            if error is not None:
                return error
            return _render(result_id, store.read(result_id, start_line, end_line))
        except Exception as e:
            return f"Error reading result: {e}"

    async def aread_result(result_id: str, start_line: int = 1, end_line: Optional[int] = None) -> str:
        """Read a stored tool result. Lines are 1-indexed and inclusive.
        ...
        try:
            ...
            if error is not None:
                return error
            return _render(result_id, await store.aread(result_id, start_line, end_line))
        except Exception as e:
            return f"Error reading result: {e}"
...
def get_search_result_function(agent: Agent, run_context: RunContext, async_mode: bool = False) -> Function:
