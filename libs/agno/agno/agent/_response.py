        ...
        # If the output schema is a Pydantic model and parse_response is True, parse it into a BaseModel
            ...
            if isinstance(run_response.content, str) and agent.parse_response:
                try:
                    ...
                    log_warning("Failed to convert response to output model", exc_info=True)
            else:
...
def _parse_followups_response(model_response: ModelResponse) -> Optional[List[str]]:
    ...
    if followups_obj is None and model_response.content:
        try:
            ...
            log_warning("Failed to parse followups from model response", exc_info=True)
...
def generate_followups(
    ...
    try:
        ...
        log_warning("Error generating followups", exc_info=True)
