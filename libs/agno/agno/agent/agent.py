...
class Agent:
    ...
    # Introduction message for the agent
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        # Skip input_schema and output_schema as they are Pydantic model types
        ...
        if self.culture_manager is not None:
            # TODO: implement culture manager serialization
            # config["culture_manager"] = self.culture_manager.to_dict()
            pass
        if self.enable_agentic_culture:
            config["enable_agentic_culture"] = self.enable_agentic_culture
        if self.update_cultural_knowledge:
            config["update_cultural_knowledge"] = self.update_cultural_knowledge
        if self.add_culture_to_context is not None:
            config["add_culture_to_context"] = self.add_culture_to_context
        ...
        if self.reload_on_run:
            config["reload_on_run"] = self.reload_on_run
    ...
    def _load_from_config(self):
        ...
        if "team_id" in config_dict:
            self.team_id = config_dict["team_id"]
        if "workflow_id" in config_dict:
            self.workflow_id = config_dict["workflow_id"]

        # --- Context compression settings ---
        if "compress_tool_results" in config_dict:
            self.compress_tool_results = config_dict["compress_tool_results"]
        if "compression_manager" in config_dict and config_dict["compression_manager"]:
            # TODO: implement compression manager reconstruction
            pass
    ...
    def save(self, version: Optional[str] = None) -> "Agent":
        """
        Save the agent as config to the database.
        """
        if not self.db:
    ...
    async def asave(self, version: Optional[str] = None) -> "Agent":
        """
        Save the agent as config to the database.
        """
        if not self.db:
        ...
        if self._has_sync_db():
            return await self.db.upsert_agent(self, version=version)
        else:
            return self.db.upsert_agent(self, version=version)
    ...
    #     """
    #     Upsert the agent's config in the database.
    #     """
    #     if not self.db:
    #         raise ValueError("Database is not set. This is required to update the agent's config.")
    #     return self.db.upsert_config(config=config)
