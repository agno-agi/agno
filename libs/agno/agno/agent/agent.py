...
class Agent:
        ...
        **kwargs: Any,
        ...
        if background:
            ...
            if stream and websocket:
                ...
                raise ValueError("Background non-streaming execution is not yet supported for agents")
    ...
    async def _arun_background_stream(
        ...
        **kwargs: Any,
        ...
        """Execute agent in background with streaming and WebSocket broadcasting.

        This method runs the agent in a background task while streaming events
        ...
        Returns:
            RunOutput with pending status (updated by background execution)
        """
        import asyncio
        ...
        async def execute_agent_background_stream():
            ...
            try:
                ...
                async for event in self._arun_stream(
                    ...
                    **kwargs,
                    ...
                    # This ensures clients who reconnect after agent started still receive events
                    try:
                ...
                log_error(f"Background streaming agent execution failed: {e}")
                ...
                if websocket_handler:
                    error_event = create_run_error_event(run_response, error=str(e))
                    await websocket_handler.handle_event(error_event, run_id=run_id)

        # Start background execution
        loop = asyncio.get_running_loop()
        loop.create_task(execute_agent_background_stream())
