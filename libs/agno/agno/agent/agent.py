...
class Agent:
    ...
    def _resolve_model_strings(self) -> None:
        """Convert model strings to Model instances"""
        ...
        if self.parser_model is not None and isinstance(self.parser_model, str):
            self.parser_model = get_model_from_string(self.parser_model)

        if self.output_model is not None and isinstance(self.output_model, str):
            self.output_model = get_model_from_string(self.output_model)
    ...
    def _get_agent_data(self) -> Dict[str, Any]:
        ...
        if self.model is not None:
            agent_data["model"] = self.model.to_dict()  # type: ignore
        return agent_data
    ...
    def get_system_message(
        ...
        if (
            ...
            and not (
                (self.model.supports_native_structured_outputs or self.model.supports_json_schema_outputs)  # type: ignore
    ...
    def _parse_response_with_parser_model(self, model_response: ModelResponse, run_messages: RunMessages) -> None:
        ...
        if self.output_schema is not None:
            parser_response_format = self._get_response_format(self.parser_model)  # type: ignore
            messages_for_parser_model = self._get_messages_for_parser_model(model_response, parser_response_format)
            parser_model_response: ModelResponse = self.parser_model.response(  # type: ignore
    ...
    async def _aparse_response_with_parser_model(
        ...
        if self.output_schema is not None:
            parser_response_format = self._get_response_format(self.parser_model)  # type: ignore
            messages_for_parser_model = self._get_messages_for_parser_model(model_response, parser_response_format)
            parser_model_response: ModelResponse = await self.parser_model.aresponse(  # type: ignore
                messages=messages_for_parser_model,
                response_format=parser_response_format,
            )
            self._process_parser_response(
    ...
    async def _aparse_response_with_parser_model_stream(
        ...
        if self.parser_model is not None:
            if self.output_schema is not None:
                ...
                model_response_stream = self.parser_model.aresponse_stream(  # type: ignore
                    messages=messages_for_parser_model,
                    response_format=parser_response_format,
                    stream_model_response=False,
                )
    ...
    def _generate_response_with_output_model(self, model_response: ModelResponse, run_messages: RunMessages) -> None:
        ...
        output_model_response: ModelResponse = self.output_model.response(messages=messages_for_output_model)  # type: ignore
    ...
    def _generate_response_with_output_model_stream(
        ...
        for model_response_event in self.output_model.response_stream(messages=messages_for_output_model):  # type: ignore
            yield from self._handle_model_response_chunk(
                session=session,
                run_response=run_response,
                model_response=model_response,
    ...
    async def _agenerate_response_with_output_model(self, model_response: ModelResponse, run_messages: RunMessages):
        ...
        messages_for_output_model = self._get_messages_for_output_model(run_messages.messages)
        output_model_response: ModelResponse = await self.output_model.aresponse(messages=messages_for_output_model)  # type: ignore
        model_response.content = output_model_response.content
    ...
    async def _agenerate_response_with_output_model_stream(
        ...
        model_response_stream = self.output_model.aresponse_stream(messages=messages_for_output_model)  # type: ignore
    ...
    def _get_telemetry_data(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "model_name": self.model.name if self.model else None,  # type: ignore
            "model_id": self.model.id if self.model else None,  # type: ignore
            "parser_model": self.parser_model.to_dict() if self.parser_model else None,  # type: ignore
            "output_model": self.output_model.to_dict() if self.output_model else None,  # type: ignore
