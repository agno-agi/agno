"""Message building functions for the Agent class.

Functions in this module construct system messages, user messages, and run message
sequences. Each takes an Agent instance as its first parameter.
"""
...
async def aget_system_message(
    ...
    # 3.3.17 Add the session state to the system message
    if add_session_state_to_context and session_state is not None:
        system_message_content += _get_formatted_session_state_for_system_message(session_state)
...
def _get_formatted_session_state_for_system_message(session_state: Dict[str, Any]) -> str:
    return f"\n<session_state>\n{session_state}\n</session_state>\n\n"
    ...
    # 2. Build the user message for the Agent
        ...
        # If message is provided as a BaseModel, convert it to a Message
            ...
            if (
                ...
                user_msg_content_str += _convert_documents_to_string(agent, references.references) + "\n"
                user_msg_content_str += "</references>"
            # 4.2 Add context to user message
            if add_dependencies_to_context and dependencies is not None:
                user_msg_content_str += "\n\n<additional context>\n"
                user_msg_content_str += _convert_dependencies_to_string(dependencies) + "\n"
...
def _get_messages_for_output_model(agent: Agent, messages: List[Message]) -> List[Message]:
...
def _convert_documents_to_string(agent: Agent, docs: List[Union[Dict[str, Any], str]]) -> str:
...
def _convert_dependencies_to_string(context: Dict[str, Any]) -> str:
