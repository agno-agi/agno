...
class AgentRunTrait(AgentTraitBase):
    ...
    def _extract_replay_overrides(self, kwargs: Dict[str, Any]) -> Dict[str, Any]:
        return {
            "replay_mode": kwargs.pop("replay_mode", None),
            "replay_sample_rate": kwargs.pop("replay_sample_rate", None),
            "replay_max_payload_bytes": kwargs.pop("replay_max_payload_bytes", None),
            "replay_max_message_chars": kwargs.pop("replay_max_message_chars", None),
            "replay_compress_payload": kwargs.pop("replay_compress_payload", None),
        }
    ...
    def _resolve_run_options(
        ...
        *,
        ...
        replay_mode: Optional[Union[str, Any]] = None,
        replay_sample_rate: Optional[float] = None,
        replay_max_payload_bytes: Optional[int] = None,
        replay_max_message_chars: Optional[int] = None,
        replay_compress_payload: Optional[bool] = None,
    ) -> ResolvedRunOptions:
        ...
        from agno.agent.trait.replay import normalise_sample_rate, parse_replay_mode

        replay_mode_default = parse_replay_mode(self.replay_mode)
        resolved_replay_mode = parse_replay_mode(replay_mode) if replay_mode is not None else replay_mode_default
        resolved_sample_rate = normalise_sample_rate(
            replay_sample_rate if replay_sample_rate is not None else self.replay_sample_rate
        )
        resolved_max_payload = (
            replay_max_payload_bytes
            if replay_max_payload_bytes is not None and replay_max_payload_bytes > 0
            else self.replay_max_payload_bytes
        )
        resolved_max_message_chars = (
            replay_max_message_chars
            if replay_max_message_chars is not None and replay_max_message_chars > 0
            else self.replay_max_message_chars
        )
        if resolved_max_payload <= 0:
            resolved_max_payload = 262144
        if resolved_max_message_chars <= 0:
            resolved_max_message_chars = 4000
        resolved_compress = (
            bool(replay_compress_payload) if replay_compress_payload is not None else bool(self.replay_compress_payload)
        )
        ...
        return ResolvedRunOptions(
            ...
            replay_sample_rate=resolved_sample_rate,
            replay_max_payload_bytes=resolved_max_payload,
            replay_max_message_chars=resolved_max_message_chars,
            replay_compress_payload=resolved_compress,
        )
    ...
    def run(
        ...
        **kwargs: Any,
        ...
        replay_overrides = self._extract_replay_overrides(kwargs)
        ...
        resolved_options = self._resolve_run_options(
            ...
            replay_mode=replay_overrides["replay_mode"],
            replay_sample_rate=replay_overrides["replay_sample_rate"],
            replay_max_payload_bytes=replay_overrides["replay_max_payload_bytes"],
            replay_max_message_chars=replay_overrides["replay_max_message_chars"],
            replay_compress_payload=replay_overrides["replay_compress_payload"],
        )
