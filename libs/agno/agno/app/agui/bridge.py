...
class FrontendToolExecution(Exception):
    """Exception raised when a tool requires frontend execution"""
    
    def __init__(self, tool_name: str, tool_args: Dict[str, Any], tool_call_id: str):
        self.tool_name = tool_name
        self.tool_args = tool_args
        self.tool_call_id = tool_call_id
        super().__init__(f"Frontend tool execution required: {tool_name}")


class FrontendToolRequired(Exception):
    """Exception raised when frontend tools are needed but not provided"""
    pass
...
class AGUIBridge:
    ...
    async def _run_agent_stream(
        ...
        # Handle None user_message
        if not user_message:
            self.logger.warning("User message is None or empty, using default message")
            user_message = ""
