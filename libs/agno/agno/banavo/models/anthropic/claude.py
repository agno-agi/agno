...
class Claude(Model):
    ...
    cache_last_message: Optional[bool] = False
    ...
    def _prepare_request_kwargs(
        ...
        _log.info(
            f"[claude._prepare_request_kwargs] tools passed in: {len(tools) if tools else 0} — names: {[t.get('function', {}).get('name') for t in (tools or [])]}"
        )
        if tools:
            formatted = self._format_tools_for_model(tools)
            _log.info(
                f"[claude._prepare_request_kwargs] formatted tools for model: {len(formatted) if formatted else 0} — names: {[t.get('name') for t in (formatted or [])]}"
            )
            request_kwargs["tools"] = formatted
        return request_kwargs

    @staticmethod
    def _clear_cache_control_tags(chat_messages: List[Dict[str, Any]], request_kwargs: Dict[str, Any]) -> None:
        """
        Remove all cache_control tags from outgoing request payload.
        """
        system_blocks = request_kwargs.get("system") or []
    ...
    @staticmethod
    def _apply_cache_control_to_last_message(chat_messages: List[Dict[str, Any]]) -> bool:
        """
        Apply cache_control to the last message's last content block.
        Returns True if a tag was applied.
        """
    ...
    def _enforce_cache_control_policy(
        self, chat_messages: List[Dict[str, Any]], request_kwargs: Dict[str, Any]
    ) -> None:
        """
        Apply fixed cache policy:
        1) system prompt block (if enabled)
        2) last message block (if enabled)
        """
        self._clear_cache_control_tags(chat_messages=chat_messages, request_kwargs=request_kwargs)

        # Re-apply preferred tags in deterministic order.
        if self.cache_system_prompt:
            system_blocks = request_kwargs.get("system") or []
            if isinstance(system_blocks, list) and len(system_blocks) > 0 and isinstance(system_blocks[0], dict):
                cache_control = (
                    {"type": "ephemeral", "ttl": "1h"}
                    if self.extended_cache_time is not None and self.extended_cache_time is True
                    else {"type": "ephemeral"}
        ...
        if self.cache_last_message:
            self._apply_cache_control_to_last_message(chat_messages=chat_messages)

    def _format_tools_for_model(self, tools: Optional[List[Dict[str, Any]]] = None) -> Optional[List[Dict[str, Any]]]:
        """
        Transforms function definitions into a format accepted by the Anthropic API.

        The `parameters` dict coming from agno (whether from MCP tools or regular
        Python functions) is already valid JSON Schema.  Pass it through directly as
        `input_schema` instead of manually reconstructing it property-by-property,
        which previously dropped `items`, `default`, `enum`, and produced invalid
        `"type": ""` for anyOf / optional fields, causing Bedrock 400 errors.
        """
        if not tools:
            return None
    ...
    def invoke(
        ...
        try:
            ...
            self._enforce_cache_control_policy(chat_messages=chat_messages, request_kwargs=request_kwargs)
    ...
    def parse_provider_response_delta(
        ...
        if hasattr(response, "message") and hasattr(response.message, "usage") and response.message.usage is not None:
            usage_dict = {
                "input_tokens": response.message.usage.input_tokens or 0,
                "output_tokens": response.message.usage.output_tokens or 0,
            }

            if (
                hasattr(response.message.usage, "cache_creation_input_tokens")
                and response.message.usage.cache_creation_input_tokens
            ):
                usage_dict["cache_write_tokens"] = response.message.usage.cache_creation_input_tokens

            if (
                hasattr(response.message.usage, "cache_read_input_tokens")
                and response.message.usage.cache_read_input_tokens
            ):
                usage_dict["cached_tokens"] = response.message.usage.cache_read_input_tokens
