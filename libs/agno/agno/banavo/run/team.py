...
from agno.banavo.run.response import RunResponse, RunResponseEvent, RunResponseExtraData, run_response_event_from_dict
...
class ToolCallStartedEvent(BaseTeamRunResponseEvent):
    ...
    parent_step_id: Optional[str] = None  # Explicit parent for parallel tools emitted via consume_and_tag
    parallel_batch_id: Optional[str] = None  # Shared UUID for all tools in one asyncio.gather batch
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "TeamRunResponse":
        ...
        for event in events or []:
            if "agent_id" in event:
                ...
                from agno.banavo.run.response import run_response_event_from_dict
