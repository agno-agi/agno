...
class A2AClient:
    ...
    async def _get_sdk_client(self) -> Client:
        ...
        transport_protocol = TransportProtocol.jsonrpc if self.protocol == "json-rpc" else TransportProtocol.http_json
    ...
    def _convert_agent_card(self, sdk_card: SDKAgentCard) -> AgentCard:
        ...
        return AgentCard(
            ...
            capabilities=[cap for cap in dir(sdk_card.capabilities) if not cap.startswith("_")]
        ...
        if images:
            for img in images:
                if hasattr(img, "url") and img.url:
                    parts.append(Part(root=FilePart(file=FileWithUri(uri=img.url, mime_type="image/*"))))

        if audio:
            for aud in audio:
                if hasattr(aud, "url") and aud.url:
                    parts.append(Part(root=FilePart(file=FileWithUri(uri=aud.url, mime_type="audio/*"))))

        if videos:
            for vid in videos:
                if hasattr(vid, "url") and vid.url:
                    parts.append(Part(root=FilePart(file=FileWithUri(uri=vid.url, mime_type="video/*"))))

        if files:
            for f in files:
                if hasattr(f, "url") and f.url:
                    mime = getattr(f, "mime_type", "application/octet-stream")
                    parts.append(Part(root=FilePart(file=FileWithUri(uri=f.url, mime_type=mime))))
    ...
    async def send_message(
        ...
        *,
        ...
        try:
            ...
            raise RemoteServerUnavailableError(
                message="No task result received from A2A server", base_url=self.base_url
            )
