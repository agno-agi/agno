...
- Tool descriptions vary per server and change when the server updates.
  The sub-agent's instructions are built from ``list_tools()`` at
  connect time, so the calling agent never sees stale hand-written
  tool docs.
- A crashed server degrades gracefully: ``status()`` flips to
  ``ok=False`` without taking down the caller.

Connection is lazy — the first ``aquery()`` / ``astatus()`` call pays
the ``initialize()`` cost. ``aclose()`` releases the session on
shutdown (awaited from an app lifespan or test teardown).
"""
...
class MCPContextProvider(ContextProvider):
    ...
    def _all_tools(self) -> list:
        ...
        if tools is None:
            ...
            # already wired aclose() into the lifespan.
