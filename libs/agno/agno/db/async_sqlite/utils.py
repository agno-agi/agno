...
async def is_table_available(session: AsyncSession, table_name: str, db_schema: Optional[str] = None) -> bool:
    """
    ...
    try:
        ...
        result = await session.execute(exists_query, {"table": table_name})
...
async def is_valid_table(
    ...
    try:
        ...
        # Note: inspection requires sync connection, so we use run_sync
        def get_columns(connection):
            inspector = inspect(connection)
            return inspector.get_columns(table_name)

        async with db_engine.connect() as conn:
            existing_columns_info = await conn.run_sync(get_columns)
...
async def bulk_upsert_metrics(session: AsyncSession, table: Table, metrics_records: list[dict]) -> list[dict]:
    ...
    Args:
        session (AsyncSession): The async session to use.
...
def fetch_all_sessions_data(
    ...
    Args:
        sessions (List[Dict[str, Any]]): All sessions to process.
