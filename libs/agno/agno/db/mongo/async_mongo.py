...
class AsyncMongoDb(AsyncBaseDb):
    ...
    async def get_user_memory_v2(
        ...
        """
        try:
            collection = await self._get_collection(table_type="user_memory", create_collection_if_not_found=True)
            if collection is None:
                return None
    ...
    async def delete_user_memory_v2(self, user_id: str) -> None:
        ...
        """
        try:
            collection = await self._get_collection(table_type="user_memory")
            if collection is None:
                return
