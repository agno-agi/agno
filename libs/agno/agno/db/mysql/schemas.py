...
SCHEDULE_TABLE_SCHEMA = {
    ...
    "method": {"type": lambda: String(16), "nullable": False},
    "endpoint": {"type": lambda: String(512), "nullable": False},
    "payload": {"type": JSON, "nullable": True},
    "cron_expr": {"type": lambda: String(64), "nullable": False},
    ...
    # legacy rows). The poller's ``claim_due_schedule`` deliberately ignores
    # this column so it can fire schedules across all users.
    "user_id": {"type": lambda: String(128), "nullable": True, "index": True},
    "created_at": {"type": BigInteger, "nullable": False, "index": True},
    "updated_at": {"type": BigInteger, "nullable": True},
    "__composite_indexes__": [
        # Poller queries on (enabled, next_run_at).
        {"name": "enabled_next_run_at", "columns": ["enabled", "next_run_at"]},
        # User-facing list endpoint covering index for "my active schedules".
...
def _get_schedule_runs_table_schema(schedules_table_name: str = "agno_schedules") -> dict[str, Any]:
    """Schedule runs table schema with FK to the schedules table."""
    return {
        "id": {"type": lambda: String(128), "primary_key": True, "nullable": False},
        "schedule_id": {
        ...
        "status": {"type": lambda: String(32), "nullable": False, "index": True},
        ...
        # Denormalised from agno_schedules.user_id so the runs router can
        # scope by owner without a JOIN to schedules.
        "user_id": {"type": lambda: String(128), "nullable": True, "index": True},
        "created_at": {"type": BigInteger, "nullable": False, "index": True},
    }
...
def get_table_schema_definition(
    ...
    Args:
        ...
        schedules_table_name (str): The name of the schedules table (used for schedule_runs FK).
