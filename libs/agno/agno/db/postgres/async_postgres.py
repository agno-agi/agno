...
class AsyncPostgresDb(AsyncBaseDb):
    ...
    async def delete_run(self, run_id: str) -> bool:
        ...
        try:
            ...
            if deleted:
                await self._cascade_tool_results(run_ids=[run_id])
            return deleted
        ...
        try:
            ...
            async with self.async_session_factory() as sess, sess.begin():
                ...
                deleted = await sess.execute(
                    table.delete().where(table.c.run_id.in_(run_ids)).returning(table.c.run_id)
                )
                deleted_ids = [row[0] for row in deleted.fetchall()]

            await self._ascrub_run_ids_from_legacy_blob(list(run_ids))
            await self._cascade_tool_results(run_ids=deleted_ids)
    ...
    async def _cascade_tool_results(
        ...
        try:
            ...
            if session_ids is not None:
                ...
                return
            async with self.async_session_factory() as sess:
                result = await sess.execute(select(table.c.result_id, table.c.namespace, table.c.path).where(scope))
