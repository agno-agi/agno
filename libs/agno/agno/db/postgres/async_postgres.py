...
class AsyncPostgresDb(AsyncBaseDb):
    ...
    async def _create_table(self, table_name: str, table_type: str) -> Table:
        ...
        try:
            ...
            for col_name, col_config in table_schema.items():
                ...
                if col_config.get("identity", False):
                    # GENERATED BY DEFAULT: the database assigns the value
                    # when the insert omits the column
                    column_args.append(Identity(always=False))
    ...
    async def ensure_jobs_table(self) -> None:
        ...
        Provisioning also upgrades a jobs table created before the enqueue
        sequence column existed, and only then: schema validation would
        otherwise refuse the table as a mismatch. Without provisioning (the
        worker's auto_provision=False, an externally managed database) a
        pre-sequence table surfaces that mismatch on first use, naming the
        missing column, which is the operator's cue to add it.
        """
        try:
            table = await self._get_table(table_type="jobs", create_table_if_not_found=True)
        except SchemaMismatchError:
            # A jobs table from before the enqueue-sequence column. Only that
            # one gap is repaired here; any other mismatch propagates as is.
            if not await self._add_jobs_sequence_column_if_missing():
                raise
            table = await self._get_table(table_type="jobs", create_table_if_not_found=True)
    ...
    async def _add_jobs_sequence_column_if_missing(self) -> bool:
        """Add the enqueue-sequence column to an existing jobs table that
        lacks it; True when it was added. Reached only after schema
        validation refused the table, so the steady state costs nothing: an
        unconditional ADD COLUMN IF NOT EXISTS is not free even as a no-op,
        since Postgres takes the ACCESS EXCLUSIVE lock and checks ALTER
        privilege before evaluating the clause. Identity fills the existing
        rows, so pre-upgrade tickets are ordered too. Safe under concurrent
        replicas: IF NOT EXISTS makes the second ALTER a no-op once it
        acquires the lock."""
        async with self.db_engine.connect() as conn:
            columns = await conn.run_sync(_get_table_columns, self.job_table_name, self.db_schema)
        if "seq" in columns:
            return False
        async with self.async_session_factory() as sess, sess.begin():
            await sess.execute(
                text(
                    f'ALTER TABLE {self.db_schema}."{self.job_table_name}" '
                    "ADD COLUMN IF NOT EXISTS seq BIGINT GENERATED BY DEFAULT AS IDENTITY"
                )
            )
        self._invalidate_table_cache(self.job_table_name)
        log_info(f"Job queue: added the enqueue-sequence column to {self.db_schema}.{self.job_table_name}")
        return True
        ...
        try:
            async with self.async_session_factory() as sess:
                async with sess.begin():
                    ...
                    # Session lock, held to commit: a gated claim on this
                    # session waits for this row to become visible before
                    # choosing its head
                    await sess.execute(text(JOBS_SESSION_LOCK_SQL), {"sid": job["session_id"]})
                    # seq is database-assigned: an explicit NULL would defeat
                    # the identity default and violate NOT NULL
                    values = {k: v for k, v in job.items() if k != "seq" or v is not None}
                    row = (await sess.execute(table.insert().values(**values).returning(*table.c))).fetchone()
            ...
            async with self.async_session_factory() as sess:
                async with sess.begin():
                    ...
                    if candidate is None:
                        return None
                    if queue_per_session:
                        # Under the session lock every in-flight enqueue or
                        # claim of this session has committed; re-validate
                        # the head against that state before taking it (READ
                        # COMMITTED gives this statement a fresh snapshot)
                        await sess.execute(text(JOBS_SESSION_LOCK_SQL), {"sid": candidate.session_id})
                        still_head = (
                            await sess.execute(
