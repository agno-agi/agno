...
class PostgresDb(BaseDb):
    ...
    def _resolve_table(
        ...
        if table_type == "jobs":
            self._ensure_jobs_sequence_column(table_name)
    ...
    def _ensure_jobs_sequence_column(self, table_name: str) -> None:
        """Add the enqueue-sequence column to a jobs table created before it
        existed. Runs BEFORE schema validation, which would otherwise refuse
        the table as a mismatch. Identity fills existing rows on ADD COLUMN,
        so pre-upgrade tickets are ordered too. Idempotent (IF NOT EXISTS),
        and safe under concurrent replicas: the second ALTER waits on the
        first's lock and then finds the column present."""
        with self.Session() as sess, sess.begin():
            sess.execute(
                text(
                    f'ALTER TABLE {self.db_schema}."{table_name}" '
