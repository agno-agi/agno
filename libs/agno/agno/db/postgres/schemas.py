...
JOBS_TABLE_SCHEMA = {
    ...
    "updated_at": {"type": BigInteger, "nullable": True},
    "completed_at": {"type": BigInteger, "nullable": True},
    # Database-assigned enqueue sequence (GENERATED BY DEFAULT AS IDENTITY):
    # the submission order the per-session FIFO gate sorts by when
    # created_at ties (one-second resolution). Existing tables gain the
    # column on first use; the adapters add it before schema validation.
    "seq": {"type": BigInteger, "nullable": False, "identity": True},
    "__composite_indexes__": [
