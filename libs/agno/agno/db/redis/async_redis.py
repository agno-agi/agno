...
from agno.db.redis.utils import (
    acreate_index_entries,
    aget_all_keys_for_table,
    apply_filters,
    apply_pagination,
    apply_sorting,
    aremove_index_entries,
...
class AsyncRedisDb(AsyncBaseDb):
    def __init__(
        self,
        ...
        db_url only supports single-node Redis connections. For Redis Cluster support,
        provide a pre-configured `redis.asyncio.RedisCluster` instance via `redis_client`.

        Args:
            id (Optional[str]): The ID of the database.
            redis_client (Optional[AsyncRedis | AsyncRedisCluster]): Async Redis client instance.
                If not provided a new client will be created from db_url.
            db_url (Optional[str]): Redis connection URL (e.g., "redis://localhost:6379/0").
        ...
        if redis_client is not None:
            self.redis_client: Union[AsyncRedis, AsyncRedisCluster] = redis_client
        elif db_url is not None:
            self.redis_client = AsyncRedis.from_url(db_url, decode_responses=True)
        else:
            raise ValueError("One of redis_client or db_url must be provided")

    async def close(self) -> None:
        """Close the underlying Redis connection. Safe to call multiple times."""
    ...
    async def _store_record(
        ...
        """Generic async method to store a record in Redis, considering optional indexing.
        ...
        try:
            ...
            await self.redis_client.set(key, serialized_data, ex=self.expire)

            if index_fields:
                await acreate_index_entries(
    ...
    async def _get_record(self, table_type: str, record_id: str) -> Optional[Dict[str, Any]]:
        """Generic async method to get a record from Redis.
    ...
    async def _delete_record(self, table_type: str, record_id: str, index_fields: Optional[List[str]] = None) -> bool:
        """Generic async method to delete a record from Redis.
        ...
        try:
            ...
            if index_fields:
                record_data = await self._get_record(table_type, record_id)
                if record_data:
                    await aremove_index_entries(
                        redis_client=self.redis_client,
                        prefix=self.db_prefix,
                        table_type=table_type,
                        record_id=record_id,
    ...
    async def _get_all_records(self, table_type: str) -> List[Dict[str, Any]]:
        """Generic async method to get all records for a table type.

        Args:
            table_type (str): The type of table to get the records from.

        Returns:
            List[Dict[str, Any]]: The records data if found, an empty list otherwise.

        Raises:
            Exception: If any error occurs while getting the records.
        """
        try:
            keys = await aget_all_keys_for_table(
                redis_client=self.redis_client, prefix=self.db_prefix, table_type=table_type
            )
    ...
    async def get_latest_schema_version(self, table_name: str):
    ...
    async def get_session(
        ...
        Args:
            ...
            deserialize (Optional[bool]): Whether to deserialize to a Session object. Defaults to True.
    ...
    async def get_sessions(
        ...
        Args:
            ...
            start_timestamp (Optional[int]): Only return sessions created at or after this timestamp.
            end_timestamp (Optional[int]): Only return sessions created at or before this timestamp.
        ...
        Returns:
            Union[List[Session], Tuple[List[Dict[str, Any]], int]]:
                ...
                - When deserialize=False: Tuple of (raw session dicts, total filtered count).
        """
        try:
            all_sessions = await self._get_all_records("sessions")
    ...
    async def rename_session(
        ...
        try:
            ...
            success = await self._store_record("sessions", session_id, session)
            if not success:
                return None
    ...
    async def upsert_session(
        ...
        try:
            ...
            existing = await self._get_record(table_type="sessions", record_id=session.session_id)
            if (
                existing
            ...
            if isinstance(session, AgentSession):
                ...
                success = await self._store_record(
                    table_type="sessions",
    ...
    async def upsert_sessions(
        ...
        """Bulk upsert multiple sessions for parity with other DBs. Redis has no efficient bulk API,
        so this falls back to individual upserts.

        Args:
            sessions (List[Session]): List of sessions to upsert.
            deserialize (Optional[bool]): Whether to deserialize the sessions. Defaults to True.
            preserve_updated_at (bool): Ignored for Redis; kept for interface parity.
        ...
        try:
            log_info(
                f"AsyncRedisDb doesn't support efficient bulk operations, falling back to individual upserts for {len(sessions)} sessions"
            )
            ...
            for session in sessions:
                if session is not None:
                    result = await self.upsert_session(session, deserialize=deserialize)
    ...
    async def delete_user_memory(self, memory_id: str, user_id: Optional[str] = None) -> None:
        ...
        Args:
            ...
            user_id (Optional[str]): The ID of the user. If provided, verifies the memory
                belongs to this user before deleting.
        ...
        try:
            ...
            if user_id is not None:
                memory = await self._get_record("memories", memory_id)
                if memory is None:
    ...
    async def delete_user_memories(self, memory_ids: List[str], user_id: Optional[str] = None) -> None:
        ...
        Args:
            ...
            user_id (Optional[str]): The ID of the user. If provided, only deletes memories
                belonging to this user.

        Raises:
            Exception: If any error occurs while deleting the memories.
        """
        try:
    ...
    async def get_all_memory_topics(self, user_id: Optional[str] = None) -> List[str]:
        ...
        Args:
            user_id (Optional[str]): If provided, only collect topics from this user's memories.

        Returns:
            List[str]: The list of memory topics.
        """
        try:
            all_memories = await self._get_all_records("memories")
    ...
    async def get_user_memory(
        ...
        Args:
            ...
            user_id (Optional[str]): The ID of the user. If provided, only returns the memory
                if it belongs to this user.

        Returns:
            Optional[UserMemory]: The memory if found, None otherwise.
        """
        try:
            memory_raw = await self._get_record("memories", memory_id)
    ...
    async def upsert_memories(
        ...
        """Bulk upsert multiple user memories. Redis has no efficient bulk API,
        so this falls back to individual upserts.
        ...
        try:
            log_info(
                f"AsyncRedisDb doesn't support efficient bulk operations, falling back to individual upserts for {len(memories)} memories"
            )
            ...
            for memory in memories:
                if memory is not None:
                    result = await self.upsert_user_memory(memory, deserialize=deserialize)
                    if result is not None:
                        results.append(result)
            return results
    ...
    async def clear_memories(self) -> None:
        ...
        try:
            ...
            keys = await aget_all_keys_for_table(
                redis_client=self.redis_client, prefix=self.db_prefix, table_type="memories"
            )
    ...
    async def _get_metrics_calculation_starting_date(self) -> Optional[date]:
        ...
        """
        try:
            all_metrics = await self._get_all_records("metrics")
            ...
            sessions_raw, _ = await self.get_sessions(  # type: ignore[misc]
    ...
    async def calculate_metrics(self) -> Optional[list]:
        """Calculate metrics for all dates without complete metrics.

        Returns:
            Optional[list[dict]]: The list of metrics records, or None if there was nothing to calculate.

        Raises:
            Exception: If any error occurs while calculating the metrics.
        """
        try:
            starting_date = await self._get_metrics_calculation_starting_date()
            ...
            for date_to_process in dates_to_process:
                ...
                existing_record = await self._get_record("metrics", metrics_record["id"])
                if existing_record:
                ...
                success = await self._store_record("metrics", metrics_record["id"], metrics_record)
                if success:
    ...
    async def get_metrics(
        ...
        Returns:
            Tuple[List[dict], Optional[int]]: A tuple containing the list of metrics and the
                latest updated_at timestamp across them.
    ...
    async def delete_knowledge_content(self, id: str):
        ...
        """
        try:
            await self._delete_record("knowledge", id)
    ...
    async def get_knowledge_content(self, id: str) -> Optional[KnowledgeRow]:
        ...
        """
        try:
            document_raw = await self._get_record("knowledge", id)
    ...
    async def get_knowledge_contents(
        ...
        """
        try:
            all_documents = await self._get_all_records("knowledge")
    ...
    async def upsert_knowledge_content(self, knowledge_row: KnowledgeRow):
        ...
        try:
            ...
            success = await self._store_record("knowledge", knowledge_row.id, data)  # type: ignore
    ...
    async def get_eval_run(
        ...
        """
        try:
            eval_run_raw = await self._get_record("evals", eval_run_id)
    ...
    async def get_eval_runs(
        ...
        Args:
            ...
            filter_type (Optional[EvalFilterType]): Restrict results to runs that have the matching component set.
            eval_type (Optional[List[EvalType]]): Filter by one or more eval types.
            deserialize (Optional[bool]): Whether to deserialize the result. Defaults to True.

        Returns:
            Union[List[EvalRunRecord], Tuple[List[Dict[str, Any]], int]]:
                - When deserialize=True: List of EvalRunRecord objects.
                - When deserialize=False: Tuple of (raw dicts, total filtered count).

        Raises:
            Exception: If any error occurs while getting the eval runs.
        """
        try:
            all_eval_runs = await self._get_all_records("evals")
    ...
    async def rename_eval_run(
        ...
        """
        try:
            eval_run_data = await self._get_record("evals", eval_run_id)
            ...
            success = await self._store_record("evals", eval_run_id, eval_run_data)
            if not success:
                return None
    ...
    async def clear_cultural_knowledge(self) -> None:
        ...
        try:
            keys = await aget_all_keys_for_table(
                redis_client=self.redis_client, prefix=self.db_prefix, table_type="culture"
            )
    ...
    async def delete_cultural_knowledge(self, id: str) -> None:
        ...
        """
        try:
            if await self._delete_record("culture", id, index_fields=["name", "agent_id", "team_id"]):
    ...
    async def get_cultural_knowledge(
        ...
        """
        try:
            cultural_knowledge = await self._get_record("culture", id)
    ...
    async def get_all_cultural_knowledge(
        ...
        """
        try:
            all_cultural_knowledge = await self._get_all_records("culture")
    ...
    async def upsert_cultural_knowledge(
        ...
        try:
            ...
            success = await self._store_record("culture", item_id, data, index_fields=["name", "agent_id", "team_id"])
    ...
    async def upsert_trace(self, trace: "Trace") -> None:
        ...
        try:
            ...
            existing = await self._get_record("traces", trace.trace_id)
    ...
    async def get_trace(
        ...
        Args:
            ...
            run_id: Filter by run ID (returns most recent match).
            session_id: Filter by session ID (returns most recent match).
            user_id: Filter by user ID (returns most recent match).
            agent_id: Filter by agent ID (returns most recent match).
        ...
        try:
            ...
            if trace_id:
                result = await self._get_record("traces", trace_id)
                if result:
                    # Calculate total_spans and error_count
                    all_spans = await self._get_all_records("spans")
            ...
            all_traces = await self._get_all_records("traces")
            matching = []
            for t in all_traces:
            ...
            if not matching:
                if not any([run_id, session_id, user_id, agent_id]):
    ...
    async def get_traces(
        ...
        Args:
            ...
            filter_expr: Advanced filter expression dict. Not currently honored by AsyncRedisDb.
    ...
    async def get_trace_stats(
        ...
        Returns:
            tuple[List[Dict], int]: Tuple of (list of session stats dicts, total count).
                Each dict contains: session_id, user_id, agent_id, team_id, workflow_id,
                total_traces, first_trace_at (datetime), last_trace_at (datetime).
        """
        try:
            log_debug(
    ...
    async def get_span(self, span_id: str):
        ...
        try:
            ...
            result = await self._get_record("spans", span_id)
            if result:
    ...
    async def get_learning(
        ...
        raise NotImplementedError("Learning methods not yet implemented for AsyncRedisDb")
