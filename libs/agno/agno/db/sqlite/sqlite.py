    ...
    def _cascade_tool_results(self, session_ids: List[str]) -> None:
        ...
        try:
            ...
            with self.Session() as sess, sess.begin():
                ...
                if not namespaces:
                    return
                fs_exists = sess.execute(
                    text("SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'agno_fs'")
                ).fetchone()
                if fs_exists is not None:
                    for namespace in namespaces:
                        sess.execute(text("DELETE FROM agno_fs WHERE namespace = :ns"), {"ns": namespace})
