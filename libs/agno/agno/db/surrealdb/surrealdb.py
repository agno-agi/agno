...
class SurrealDb(BaseDb):
    ...
    # / ``release_schedule``) deliberately has no user_id — the poller must
    # fire schedules across all users.
    #
    # The claim uses SurrealQL's ``UPDATE ... WHERE ...`` to compare-and-set
    # on the lock fields atomically. SurrealDB serialises writes per record,
    # so the WHERE predicate guards against concurrent claims.
    @staticmethod
    def _strip_record_id(row: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        """SurrealDB returns ``id`` as a RecordID; flatten it to the raw key
        so the dict matches the SQL-backed shape callers expect."""
        if row is None:
            return None
        row = dict(row)
        rid = row.get("id")
    ...
    def get_schedule(self, schedule_id: str, user_id: Optional[str] = None) -> Optional[Dict[str, Any]]:
        try:
            table = self._get_table("schedules")
            record_id = RecordID(table, schedule_id)
            if user_id is None:
                raw = self._query_one("SELECT * FROM ONLY $record_id", {"record_id": record_id}, dict)
            else:
                raw = self._query_one(
                    "SELECT * FROM ONLY $record_id WHERE user_id = $user_id",
                    {"record_id": record_id, "user_id": user_id},
                    dict,
                )
            return self._strip_record_id(raw)
        except Exception as e:
    ...
    def get_schedule_by_name(self, name: str, user_id: Optional[str] = None) -> Optional[Dict[str, Any]]:
        try:
            ...
            if user_id is None:
                raw = self._query_one(
                    f"SELECT * FROM {table} WHERE name = $name LIMIT 1", {"name": name}, dict
                )
            else:
                raw = self._query_one(
                    f"SELECT * FROM {table} WHERE name = $name AND user_id = $user_id LIMIT 1",
    ...
    def get_schedules(
        ...
        try:
            ...
            if enabled is not None:
                ...
                vars_["enabled"] = enabled
            if user_id is not None:
                where_parts.append("user_id = $user_id")
                vars_["user_id"] = user_id
            where_clause = "WHERE " + " AND ".join(where_parts) if where_parts else ""
            total_count = self._count(table, where_clause, vars_)
            offset = (page - 1) * limit
            query = dedent(f"""
                SELECT * FROM {table}
                {where_clause}
                ORDER BY created_at DESC
                LIMIT {int(limit)} START {int(offset)}
            """)
            rows = self._query(query, vars_, dict)
            return [self._strip_record_id(dict(r)) for r in rows], total_count  # type: ignore[misc]
        except Exception as e:
    ...
    def create_schedule(self, schedule_data: Dict[str, Any]) -> Dict[str, Any]:
        try:
            table = self._get_table("schedules", create_table_if_not_found=True)
            data = dict(schedule_data)
            schedule_id = data.pop("id")
            self.client.upsert(RecordID(table, schedule_id), data)
    ...
    def update_schedule(
        ...
        try:
            ...
            # Read-then-write so we can enforce user_id scope. SurrealDB lacks
            # a single-statement scoped MERGE, so we check ownership first.
            if user_id is not None:
                current = self.get_schedule(schedule_id, user_id=user_id)
                if current is None:
                    return None
            self.client.merge(RecordID(table, schedule_id), kwargs)
    ...
    def delete_schedule(self, schedule_id: str, user_id: Optional[str] = None) -> bool:
        try:
            ...
            if user_id is not None:
                ...
                if current is None:
                    return False
            # Cascade-delete schedule_runs first.
            runs_table = self._get_table("schedule_runs")
            run_where = "schedule_id = $schedule_id"
            run_vars: Dict[str, Any] = {"schedule_id": schedule_id}
            if user_id is not None:
                run_where += " AND user_id = $user_id"
                run_vars["user_id"] = user_id
            self._query(f"DELETE FROM {runs_table} WHERE {run_where}", run_vars, dict)
            self.client.delete(RecordID(table, schedule_id))
            return True
        except Exception as e:
    ...
    def claim_due_schedule(self, worker_id: str, lock_grace_seconds: int = 300) -> Optional[Dict[str, Any]]:
        try:
            ...
            # SurrealQL: atomic UPDATE with WHERE predicate plus LIMIT 1.
            # SurrealDB serialises writes per record, so the WHERE predicate
            # guards against double-claim by another worker.
            query = dedent(f"""
                UPDATE {table}
                SET locked_by = $worker, locked_at = $now
                WHERE enabled = true
                  AND next_run_at <= $now
                  AND (locked_by IS NONE OR locked_at <= $stale)
            ...
            """)
            rows = self._query(
                query, {"worker": worker_id, "now": now, "stale": stale}, dict
            )
            if not rows:
                return None
            return self._strip_record_id(dict(rows[0]))  # type: ignore[arg-type]
        except Exception as e:
    ...
    def release_schedule(self, schedule_id: str, next_run_at: Optional[int] = None) -> bool:
        try:
            ...
            self.client.merge(RecordID(table, schedule_id), updates)
            return True
        except Exception as e:
    ...
    def create_schedule_run(self, run_data: Dict[str, Any]) -> Dict[str, Any]:
        try:
            table = self._get_table("schedule_runs", create_table_if_not_found=True)
            data = dict(run_data)
            run_id = data.pop("id")
            self.client.upsert(RecordID(table, run_id), data)
            return run_data
        except Exception as e:
    ...
    def update_schedule_run(self, schedule_run_id: str, **kwargs: Any) -> Optional[Dict[str, Any]]:
        try:
            table = self._get_table("schedule_runs")
            self.client.merge(RecordID(table, schedule_run_id), kwargs)
    ...
    def get_schedule_run(self, run_id: str, user_id: Optional[str] = None) -> Optional[Dict[str, Any]]:
        try:
            table = self._get_table("schedule_runs")
            record_id = RecordID(table, run_id)
            if user_id is None:
                raw = self._query_one("SELECT * FROM ONLY $record_id", {"record_id": record_id}, dict)
            else:
                raw = self._query_one(
    ...
    def get_schedule_runs(
        ...
        try:
            table = self._get_table("schedule_runs")
            where_parts = ["schedule_id = $schedule_id"]
            vars_: Dict[str, Any] = {"schedule_id": schedule_id}
            if user_id is not None:
                where_parts.append("user_id = $user_id")
                vars_["user_id"] = user_id
