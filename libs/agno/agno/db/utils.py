...
def deserialize_session_json_fields(session: dict) -> dict:
    ...
    if session.get("agent_data") is not None and isinstance(session["agent_data"], str):
        try:
            ...
            log_warning("Warning: Could not parse agent_data as JSON, keeping as string", exc_info=True)

    if session.get("team_data") is not None and isinstance(session["team_data"], str):
        try:
            session["team_data"] = json.loads(session["team_data"])
        except (json.JSONDecodeError, TypeError):
            log_warning("Warning: Could not parse team_data as JSON, keeping as string", exc_info=True)
    ...
    if session.get("workflow_data") is not None and isinstance(session["workflow_data"], str):
        try:
            ...
            log_warning("Warning: Could not parse workflow_data as JSON, keeping as string", exc_info=True)

    if session.get("metadata") is not None and isinstance(session["metadata"], str):
        try:
            session["metadata"] = json.loads(session["metadata"])
        except (json.JSONDecodeError, TypeError):
            log_warning("Warning: Could not parse metadata as JSON, keeping as string", exc_info=True)
    ...
    if session.get("chat_history") is not None and isinstance(session["chat_history"], str):
        try:
            ...
            log_warning("Warning: Could not parse chat_history as JSON, keeping as string", exc_info=True)

    if session.get("summary") is not None and isinstance(session["summary"], str):
        try:
            session["summary"] = json.loads(session["summary"])
        except (json.JSONDecodeError, TypeError):
            log_warning("Warning: Could not parse summary as JSON, keeping as string", exc_info=True)
    ...
    if session.get("session_data") is not None and isinstance(session["session_data"], str):
        try:
            ...
            log_warning("Warning: Could not parse session_data as JSON, keeping as string", exc_info=True)
    ...
    if session.get("runs") is not None and isinstance(session["runs"], str):
        try:
            ...
            log_warning("Warning: Could not parse runs as JSON, keeping as string", exc_info=True)
...
def db_from_dict(db_data: Dict[str, Any]) -> Optional[Union["BaseDb"]]:
    ...
    if db_type == "postgres":
        try:
            ...
            log_error("Error reconstructing PostgresDb from dictionary", exc_info=True)
            return None
    elif db_type == "sqlite":
        try:
            ...
            log_error("Error reconstructing SqliteDb from dictionary", exc_info=True)
            return None
    else:
