...
try:
    ...
    raise ImportError(
        "`aiofiles` not installed. Please install it with `pip install aiofiles`"
    )
...
class CSVReader(Reader):
    ...
    def read(
        self, file: Union[Path, IO[Any]], delimiter: str = ",", quotechar: str = '"'
    ) -> List[Document]:
        try:
            ...
            logger.error(
                f"Error reading: {file.name if isinstance(file, IO) else file}: {e}"
            )
            return []
        ...
        try:
            if isinstance(file, Path):
                ...
                async with aiofiles.open(
                    file, mode="r", encoding="utf-8", newline=""
                ) as file_content:
            ...
            csv_reader = csv.reader(
                file_content_io, delimiter=delimiter, quotechar=quotechar
            )
            ...
            if total_rows <= 10:
                ...
                async def _process_page(
                    page_number: int, page_rows: List[List[str]]
                ) -> Document:
                ...
                documents = await asyncio.gather(
                    *[
                        _process_page(page_number, page)
            ...
            logger.error(
                f"Error reading async: {file.name if isinstance(file, IO) else file}: {e}"
            )
            return []
