...
class OpenAIModerationGuardrail(BaseGuardrail):
    ...
    def __init__(
        ...
        # Cache clients to reuse HTTP connections
        self._sync_client: Any = None
        self._async_client: Any = None

    def _get_sync_client(self) -> Any:
        """Get or create cached sync OpenAI client."""
        if self._sync_client is None:
            try:
                from openai import OpenAI as OpenAIClient
            except ImportError:
                raise ImportError("`openai` not installed. Please install using `pip install openai`")
            self._sync_client = OpenAIClient(api_key=self.api_key)
    ...
    def _get_async_client(self) -> Any:
        ...
        if self._async_client is None:
            try:
                ...
                raise ImportError("`openai` not installed. Please install using `pip install openai`")
            self._async_client = AsyncOpenAIClient(api_key=self.api_key)
