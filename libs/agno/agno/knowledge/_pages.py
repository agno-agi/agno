...
class PageCoordinator:
    ...
    def _predicate(self):
        return and_(
            ...
            self.catalog.c.metadata["_agno"]["page"]["namespace"].astext == self.namespace,
        )
    ...
    def _rows(
        ...
        *,
        ...
        path = c.c.metadata["_agno"]["page"]["path"].astext
        stmt = (
            select(
                c.c.metadata,
            ...
            .where(self._predicate(), path > after, func.starts_with(path, prefix))
            .order_by(path)
            .limit(limit)
        )
        if ignore_case:
    ...
    def _one(self, conn: Any, path: str) -> tuple[Page, str]:
        rows = self._rows(conn, prefix=path, limit=1)
    ...
    def list(
        ...
        with self._snapshot(budget) as conn:
            ...
            for row in rows[:limit]:
                pages.append(self._checked(row)[0])
                if encoded_size(result(True)) > MAX_JSON_BYTES:
                    pages.pop()
                    break
            return result(len(rows) > len(pages))
