        ...
        if self.overlap > 0:
            ...
            for i in range(len(chunks)):
                if i > 0:
                    ...
                    if prev_text:
                        overlap_chunk_number = i + 1
                        meta_data = chunk_meta_data.copy()
                        meta_data["chunk"] = overlap_chunk_number
                        chunk_content = prev_text + chunks[i].content
                        chunk_id = self._generate_chunk_id(document, overlap_chunk_number, chunk_content)
                        meta_data["chunk_size"] = len(chunk_content)
