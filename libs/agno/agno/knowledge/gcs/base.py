...
class GCSKnowledgeBase(AgentKnowledge):
    ...
    def gcs_blobs(self) -> List[storage.Blob]:
        ...
        if self.blob_name is not None:
            blobs_to_read.append(self.bucket.blob(self.blob_name))
        elif self.prefix is not None:
            blobs_to_read.extend(self.bucket.list_blobs(prefix=self.prefix))
        else:
            blobs_to_read.extend(self.bucket.list_blobs())
