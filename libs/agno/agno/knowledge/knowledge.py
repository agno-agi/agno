...
class Knowledge(RemoteKnowledge):
    ...
    def _prepare_documents_for_insert(
        ...
        for document in documents:
            ...
            # Add linked_to to metadata for vector search filtering (only if knowledge has a name)
            if self.name:
                document.meta_data["linked_to"] = self.name
        return documents
    ...
    def _build_knowledge_row(self, content: Content) -> KnowledgeRow:
        ...
        return KnowledgeRow(
            ...
            linked_to=self.name if self.name else None,
            access_count=0,
