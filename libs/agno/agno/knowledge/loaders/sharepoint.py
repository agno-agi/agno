...
class SharePointLoader(BaseLoader):
    ...
    def _get_sharepoint_site_id(self, hostname: str, site_path: Optional[str], access_token: str) -> Optional[str]:
        ...
        try:
            ...
            log_error(f"Failed to get SharePoint site ID: {e.response.status_code} - {e.response.text}: {e}")
            return None
    ...
    def _list_sharepoint_folder_items(self, site_id: str, folder_path: str, access_token: str) -> List[dict]:
        ...
        try:
            ...
            log_error(f"Failed to list SharePoint folder: {e.response.status_code} - {e.response.text}: {e}")
    ...
    async def _aload_from_sharepoint(
        ...
        if path_to_process:
            try:
                ...
                log_error(f"Error checking SharePoint path {path_to_process}")
                return
