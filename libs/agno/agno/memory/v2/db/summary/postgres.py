...
class PostgresSummaryDb(SummaryDb):
    def __init__(
        ...
        """
        This class provides a summary store backed by a postgres table.
        ...
        Args:
            table_name (str): The name of the table to store summary rows.
    ...
    def get_table(self) -> Table:
        return Table(
            ...
            Column("summary", postgresql.JSONB, server_default=text("'{}'::jsonb")),
    ...
    def summary_exists(self, summary: SummaryRow) -> bool:
        ...
        with self.Session() as sess, sess.begin():
            stmt = select(*columns).where(self.table.c.id == summary.id)
    ...
    def read_summaries(
        ...
        try:
            with self.Session() as sess, sess.begin():
                ...
                for row in rows:
                    if row is not None:
                        summaries.append(SummaryRow.model_validate(row))
        except Exception as e:
    ...
    def upsert_summary(self, summary: SummaryRow, create_and_retry: bool = True) -> None:
        """Create a new summary if it does not exist, otherwise update the existing summary"""
        ...
        try:
            with self.Session() as sess, sess.begin():
                ...
                # Define the upsert if the summary already exists
    ...
    def __deepcopy__(self, memo):
        """
        Create a deep copy of the PgSummaryDb instance, handling unpickleable attributes.
