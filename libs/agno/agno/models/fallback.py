...
def call_model_with_fallback(
    ...
    **kwargs: Any,
    ...
    try:
        ...
        if not fallbacks:
            raise
        log_warning(f"Primary model '{model.id}' failed. Trying fallback models...: {primary_error}", exc_info=True)
...
def _try_fallback_models(
    ...
    for i, fallback in enumerate(fallback_models):
        try:
            ...
            log_warning(f"Fallback model '{fallback.id}' also failed: {e}", exc_info=True)
            continue
    raise primary_error
