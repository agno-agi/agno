...
class Message(BaseModel):
    ...
    private_tool_calls: Optional[List[Dict[str, Any]]] = Field(default=None, alias='tool_calls')
    ...
    def tool_calls(self) -> Optional[List[Dict[str, Any]]]:
        return self.private_tool_calls

    @tool_calls.setter
    def tool_calls(self, value: Optional[List[Dict[str, Any]]]) -> None:
        if value:
            seen_ids = set()
            unique_calls = []
            for call in value:
            ...
            self.private_tool_calls = unique_calls
        else:
            self.private_tool_calls = value
