...
def _create_events_from_chunk(
    ...
    elif isinstance(chunk, (AgentFollowupsCompletedEvent, TeamFollowupsCompletedEvent)):
        if chunk.followups:
            followup_event = CustomEvent(
                name="FollowupSuggestionsEvent",
                value={"followups": chunk.followups},
            )
            events_to_emit.append(followup_event)
