...
def attach_routes(
    ...
    bot_state = BotState(session_config=session_config, entity_id=entity_id)

    # Resolved in discord.py __init__; passed through as-is
    ...
    @router.get(
        "/status",
        operation_id=f"discord_status_{op_suffix}",
        name="discord_status",
    ...
    @router.post(
        "/interactions",
        operation_id=f"discord_interactions_{op_suffix}",
    ...
    async def discord_interactions(request: Request, background_tasks: BackgroundTasks):
        ...
        if interaction_type == INTERACTION_APPLICATION_COMMAND:
            ...
            # Discord may retry if our ACK arrives late
            if bot_state.is_duplicate_interaction(interaction_id):
    ...
    async def _process_interaction(data: dict) -> None:
        ...
        resolved_app_id = data.get("application_id") or app_id
        interaction_token = data.get("token", "")

        try:
            message_text = extract_interaction_options(data)
            if not message_text:
                await patch_webhook_message(
                    session, resolved_app_id, interaction_token, content="Please provide a message."
                )
                return
            ...
            if streaming:
                await _stream_response(session, resolved_app_id, interaction_token, message_text, run_kwargs)
            else:
                await _sync_response(session, resolved_app_id, interaction_token, message_text, run_kwargs)

        except Exception as e:
            log_error(f"Error processing Discord interaction: {e}")
            try:
                await patch_webhook_message(session, resolved_app_id, interaction_token, content=error_message)
            except Exception:
                pass
    ...
    async def _stream_response(
        ...
        if not is_workflow and state.final_run_output:
            if state.final_run_output.status == "ERROR":
                return
            await send_response_media(session, resolved_app_id, interaction_token, state.final_run_output)

        # Workflows don't yield RunOutput — media is collected via streaming events
        if is_workflow and (state.images or state.videos or state.audio or state.files):
            await send_response_media(session, resolved_app_id, interaction_token, state)
    ...
    async def _sync_response(
        ...
        if not response:
            await patch_webhook_message(session, resolved_app_id, interaction_token, content="No response generated.")
            return
        ...
        # Reasoning first, then content — same order as streaming
        ...
        embed = build_status_embed(
            title="Complete",
            description=full_description[:DC_MAX_EMBED_DESC],
            fields=[],
            color=EMBED_COLOR_COMPLETE,
        )
        await patch_webhook_message(session, resolved_app_id, interaction_token, embeds=[embed])

        if len(full_description) > DC_MAX_EMBED_DESC:
            overflow = full_description[DC_MAX_EMBED_DESC:]
            for part in chunk_text(overflow, DC_CHUNK_SIZE):
                await post_followup_message(session, resolved_app_id, interaction_token, content=part)

        await send_response_media(session, resolved_app_id, interaction_token, response)
