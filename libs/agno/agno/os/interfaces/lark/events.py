"""Stream event dispatcher for the Lark interface.

Maps agno ``RunEvent`` / ``WorkflowRunEvent`` stream events to state mutations
on :class:`StreamState` — accumulating content, updating status lines (tool
calls, reasoning, workflow steps), and triggering card PATCHs.

This is adapted from the Telegram interface's ``events.py``. The handler logic
(status lines + content accumulation) is platform-agnostic; only the display
rendering differs — Lark uses :meth:`StreamState.build_display_card` (a card
JSON string) where Telegram used ``build_display_html``.
"""
...
from agno.os.interfaces.lark.state import LARK_STREAM_EDIT_INTERVAL, StreamState
...
async def _on_tool_call_started(chunk: "BaseRunOutputEvent", state: StreamState) -> bool:
    ...
    if not tool_name:
        # Lark has no "typing" indicator API comparable to Telegram's
        # send_chat_action; just wait for the next content chunk.
        return False
...
async def _on_run_content(chunk: "BaseRunOutputEvent", state: StreamState) -> bool:
    ...
    if content is not None:
        ...
        now = time.monotonic()
        if now - state.last_edit_time >= LARK_STREAM_EDIT_INTERVAL:
            try:
                await state.send_or_edit(state.build_display_card())
            except Exception as e:
                log_warning(f"Lark stream edit failed (will retry on next chunk): {str(e)}")
    return False
...
async def _on_run_error(chunk: "BaseRunOutputEvent", state: StreamState) -> bool:
    ...
    log_error(f"Run error during Lark stream: {error_content}")
...
async def dispatch_stream_event(ev_raw: str, chunk: "BaseRunOutputEvent", state: StreamState) -> bool:
    """Dispatch a stream event to its handler.

    Returns ``True`` if the stream should be aborted (e.g. on error),
    ``False`` to continue.
    """
