...
def get_base_router(
    ...
    # -- Dynamic Runtime Management Routes ---
    
    @router.post(
        "/runtime/agents",
        tags=["Runtime Management"],
        operation_id="add_agent_runtime",
        summary="Add Agent at Runtime",
        description=(
            "Dynamically add a new agent to the AgentOS instance without restarting.\n\n"
            "**Features:**\n"
            "- Runtime agent addition with full initialization\n"
            "- Automatic route registration and database discovery\n"
            "- Thread-safe operations\n"
            "- Validation to prevent duplicate agent IDs\n\n"
            "**Note:** Agent configuration should be provided as JSON in the request body."
        ),
        responses={
            200: {"description": "Agent added successfully"},
            400: {"description": "Agent with same ID already exists or invalid configuration", "model": BadRequestResponse},
            500: {"description": "Failed to add agent", "model": InternalServerErrorResponse},
        },
    )
    async def add_agent_runtime(
        request: Request,
        agent_config: dict = Form(...),  # JSON configuration for the agent
    ):
        """Add a new agent to the AgentOS at runtime"""
        try:
            ...
            # Create agent from config (simplified - extend this based on your needs)
            agent = Agent(
                id=agent_config["id"],
                name=agent_config.get("name", agent_config["id"]),
                description=agent_config.get("description"),
                # Add other agent configuration parameters as needed
            )
            
            # Add agent to OS
            if os.add_agent(agent):
                return JSONResponse(content={"message": f"Agent {agent.id} added successfully"}, status_code=200)
            else:
    ...
    @router.delete(
        ...
        tags=["Runtime Management"],
        operation_id="remove_agent_runtime",
        summary="Remove Agent at Runtime",
        description=(
            "Dynamically remove an existing agent from the AgentOS instance.\n\n"
            "**Features:**\n"
            "- Runtime agent removal with cleanup\n"
            "- Automatic route refresh\n"
            "- Thread-safe operations\n\n"
            "**Warning:** This will stop any running sessions for the removed agent."
        ),
        responses={
            200: {"description": "Agent removed successfully"},
            404: {"description": "Agent not found", "model": NotFoundResponse},
        },
    )
    async def remove_agent_runtime(agent_id: str):
        """Remove an agent from the AgentOS at runtime"""
        if os.remove_agent(agent_id):
            return JSONResponse(content={"message": f"Agent {agent_id} removed successfully"}, status_code=200)
        else:
    ...
    @router.post(
        "/runtime/teams",
        tags=["Runtime Management"],
        operation_id="add_team_runtime",
        summary="Add Team at Runtime",
        description=(
            "Dynamically add a new team to the AgentOS instance without restarting.\n\n"
            "**Features:**\n"
            "- Runtime team addition with full initialization\n"
            "- Automatic route registration and database discovery\n"
            "- Thread-safe operations\n"
            "- Validation to prevent duplicate team IDs"
        ),
        responses={
            200: {"description": "Team added successfully"},
            400: {"description": "Team with same ID already exists or invalid configuration", "model": BadRequestResponse},
            500: {"description": "Failed to add team", "model": InternalServerErrorResponse},
        },
    )
    async def add_team_runtime(
        request: Request,
        team_config: dict = Form(...),  # JSON configuration for the team
    ):
        """Add a new team to the AgentOS at runtime"""
        try:
            from agno.team.team import Team
            
            # Basic validation
            if not team_config.get("id"):
                raise HTTPException(status_code=400, detail="Team ID is required")
            
            # Create team from config (simplified - extend this based on your needs)
            team = Team(
                id=team_config["id"],
                name=team_config.get("name", team_config["id"]),
                description=team_config.get("description"),
                # Add other team configuration parameters as needed
            )
            
            # Add team to OS
            if os.add_team(team):
                return JSONResponse(content={"message": f"Team {team.id} added successfully"}, status_code=200)
            else:
    ...
    @router.delete(
        "/runtime/teams/{team_id}",
        tags=["Runtime Management"],
        operation_id="remove_team_runtime",
        summary="Remove Team at Runtime",
        description=(
            "Dynamically remove an existing team from the AgentOS instance.\n\n"
            "**Features:**\n"
            "- Runtime team removal with cleanup\n"
            "- Automatic route refresh\n"
            "- Thread-safe operations\n\n"
            "**Warning:** This will stop any running sessions for the removed team."
        ),
        responses={
            200: {"description": "Team removed successfully"},
            404: {"description": "Team not found", "model": NotFoundResponse},
        },
    )
    async def remove_team_runtime(team_id: str):
        """Remove a team from the AgentOS at runtime"""
        if os.remove_team(team_id):
            return JSONResponse(content={"message": f"Team {team_id} removed successfully"}, status_code=200)
        else:
    ...
    @router.post(
        "/runtime/workflows",
        tags=["Runtime Management"],
        operation_id="add_workflow_runtime",
        summary="Add Workflow at Runtime",
        description=(
            "Dynamically add a new workflow to the AgentOS instance without restarting.\n\n"
            "**Features:**\n"
            "- Runtime workflow addition with full initialization\n"
            "- Automatic route registration\n"
            "- Thread-safe operations\n"
            "- Validation to prevent duplicate workflow IDs"
        ),
        responses={
            200: {"description": "Workflow added successfully"},
            400: {"description": "Workflow with same ID already exists or invalid configuration", "model": BadRequestResponse},
            500: {"description": "Failed to add workflow", "model": InternalServerErrorResponse},
        },
    )
    async def add_workflow_runtime(
        request: Request,
        workflow_config: dict = Form(...),  # JSON configuration for the workflow
    ):
        """Add a new workflow to the AgentOS at runtime"""
        try:
            from agno.workflow.workflow import Workflow
            ...
            # Create workflow from config (simplified - extend this based on your needs)
            workflow = Workflow(
                id=workflow_config["id"],
                name=workflow_config.get("name", workflow_config["id"]),
                description=workflow_config.get("description"),
                # Add other workflow configuration parameters as needed
            )
            
            # Add workflow to OS
            if os.add_workflow(workflow):
                return JSONResponse(content={"message": f"Workflow {workflow.id} added successfully"}, status_code=200)
            else:
                raise HTTPException(status_code=400, detail=f"Workflow with ID {workflow.id} already exists")
                
        except Exception as e:
            raise HTTPException(status_code=500, detail=f"Failed to add workflow: {str(e)}")
    
    @router.delete(
        "/runtime/workflows/{workflow_id}",
        tags=["Runtime Management"],
        operation_id="remove_workflow_runtime",
        summary="Remove Workflow at Runtime",
        description=(
            "Dynamically remove an existing workflow from the AgentOS instance.\n\n"
            "**Features:**\n"
            "- Runtime workflow removal with cleanup\n"
            "- Automatic route refresh\n"
            "- Thread-safe operations\n\n"
            "**Warning:** This will stop any running executions for the removed workflow."
        ),
        responses={
            200: {"description": "Workflow removed successfully"},
            404: {"description": "Workflow not found", "model": NotFoundResponse},
        },
    )
    async def remove_workflow_runtime(workflow_id: str):
        """Remove a workflow from the AgentOS at runtime"""
        if os.remove_workflow(workflow_id):
            return JSONResponse(content={"message": f"Workflow {workflow_id} removed successfully"}, status_code=200)
        else:
    ...
    @router.post(
        "/runtime/refresh",
        tags=["Runtime Management"],
        operation_id="refresh_routes",
        summary="Refresh Routes",
        description=(
            "Manually refresh all routes to ensure newly added agents, teams, and workflows are accessible.\n\n"
            "**Note:** This is automatically called when adding/removing components, "
            "but can be manually triggered if needed."
        ),
        responses={
            200: {"description": "Routes refreshed successfully"},
            500: {"description": "Failed to refresh routes", "model": InternalServerErrorResponse},
        },
    )
    async def refresh_routes_endpoint():
        """Refresh all routes to include newly added components"""
        if os.refresh_routes():
            return JSONResponse(content={"message": "Routes refreshed successfully"}, status_code=200)
        else:
            raise HTTPException(status_code=500, detail="Failed to refresh routes - FastAPI app not initialized")
    
    @router.get(
        "/runtime/stats",
        tags=["Runtime Management"],
        operation_id="get_runtime_stats",
        summary="Get Runtime Statistics",
        description=(
            "Get current runtime statistics about agents, teams, workflows, and databases.\n\n"
            "**Returns information about:**\n"
            "- Number and IDs of active agents\n"
            "- Number and IDs of active teams\n"
            "- Number and IDs of active workflows\n"
            "- Database connection information"
        ),
        responses={
            200: {
                "description": "Runtime statistics retrieved successfully",
                "content": {
                    "application/json": {
                        "example": {
                            "agents": {"count": 2, "ids": ["agent-1", "agent-2"]},
                            "teams": {"count": 1, "ids": ["team-1"]},
                            "workflows": {"count": 3, "ids": ["workflow-1", "workflow-2", "workflow-3"]},
                            "databases": {"count": 1, "ids": ["main-db"]}
                        }
                    }
                }
            }
    ...
    async def get_runtime_stats():
        """Get runtime statistics about current components"""
        return os.get_runtime_stats()
