...
def attach_routes(router: APIRouter, knowledge_instances: List[Union[Knowledge, RemoteKnowledge]]) -> APIRouter:
    ...
    async def search_knowledge(
        http_request: Request, request: VectorSearchRequest
    ...
    async def get_config(
        ...
        for reader_info in readers_info:
            reader_schemas[reader_info["id"]] = ReaderResponse(
                id=reader_info["id"],
        ...
        if readers_dict:
            for reader_id, reader in readers_dict.items():
                ...
                if reader_id not in reader_schemas:
                    reader_schemas[reader_id] = ReaderResponse(
                        id=reader_id,
        ...
        for chunker_info in chunkers_list:
            ...
            if chunker_key:
                chunkers_dict[chunker_key] = ChunkerResponse(
                    key=chunker_key,
