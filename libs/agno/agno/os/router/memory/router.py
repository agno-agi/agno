...
def attach_routes(router: APIRouter, dbs: dict[str, list[Union[BaseDb, AsyncBaseDb, RemoteDb]]]) -> APIRouter:
    ...
    @router.get(
        "/memories",
        response_model=PaginatedResponse[UserMemoryResponse],
    ...
    async def get_memories(
        ...
        memories = [UserMemoryResponse.from_dict(user_memory) for user_memory in user_memories]  # type: ignore
    ...
    async def get_memory_config(
        ...
        if isinstance(db, RemoteDb):
            auth_token = get_auth_token_from_request(request)
            headers = {"Authorization": f"Bearer {auth_token}"} if auth_token else None
            return await db.get_memory_config(
                db_id=db_id,
                table=table,
                headers=headers,
            )
        ...
        return MemoryConfigResponse(topics=topics)

    @router.get(
        "/user_memory_stats",
        response_model=PaginatedResponse[UserStatsResponse],
        response_model_exclude_none=True,
        status_code=200,
    ...
    async def get_user_memory_stats(
        ...
        try:
            ...
            return PaginatedResponse(
                data=[UserStatsResponse.from_dict(stats) for stats in user_stats],
                meta=PaginationInfo(
                    page=page,
                    limit=limit,
