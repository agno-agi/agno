...
class TeamResponse(BaseModel):
    ...
    pre_hooks: Optional[List[HookResponse]] = Field(None, description="The pre-hooks of the team")
    post_hooks: Optional[List[HookResponse]] = Field(None, description="The post-hooks of the team")
    ...
    async def from_team(cls, team: Team) -> "TeamResponse":
        ...
        if team.pre_hooks:
            pre_hooks_response = [HookResponse.from_hook(hook) for hook in team.pre_hooks]

        post_hooks_response: Optional[List[HookResponse]] = None
        if team.post_hooks:
            post_hooks_response = [HookResponse.from_hook(hook) for hook in team.post_hooks]
