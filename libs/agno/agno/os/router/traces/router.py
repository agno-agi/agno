...
def attach_routes(router: APIRouter, dbs: dict[str, list[Union[BaseDb, AsyncBaseDb, RemoteDb]]]) -> APIRouter:
    @router.get(
        "/traces",
        response_model=PaginatedResponse[TraceSummaryResponse],
    ...
    async def get_traces(
        ...
        try:
            ...
            # Build response
            trace_summaries = [
                TraceSummaryResponse.from_trace(trace, input=trace_inputs.get(trace.trace_id)) for trace in traces
            ]
    ...
    @router.get(
        "/traces/{trace_id}",
        response_model=Union[TraceDetailResponse, TraceNodeResponse],
        response_model_exclude_unset=True,
        tags=["Traces"],
    ...
    async def get_trace(
        ...
        try:
            ...
            if span_id:
                ...
                return TraceNodeResponse.from_span(span, spans=None)
            ...
            return TraceDetailResponse.from_trace_and_spans(trace, spans)
    ...
    @router.get(
        "/trace_session_stats",
        response_model=PaginatedResponse[TraceSessionStatsResponse],
        response_model_exclude_unset=True,
        tags=["Traces"],
