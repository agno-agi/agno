...
class TraceNodeResponse(BaseModel):
    ...
    spans: Optional[List["TraceNodeResponse"]] = Field(None, description="Child spans in the trace hierarchy")
    ...
    @classmethod
    def from_span(cls, span: Any, spans: Optional[List["TraceNodeResponse"]] = None) -> "TraceNodeResponse":
...
class TraceSummaryResponse(BaseModel):
    ...
    @classmethod
    def from_trace(cls, trace: Trace, input: Optional[str] = None) -> "TraceSummaryResponse":
...
class TraceSessionStatsResponse(BaseModel):
...
class TraceDetailResponse(BaseModel):
    ...
    workflow_id: Optional[str] = Field(None, description="Associated workflow ID")
    created_at: datetime = Field(..., description="Time when trace was created (Pydantic auto-serializes to ISO 8601)")
    tree: List[TraceNodeResponse] = Field(..., description="Hierarchical tree of spans (root nodes)")

    @classmethod
    def from_trace_and_spans(cls, trace: Trace, spans: List[Span]) -> "TraceDetailResponse":
    ...
    def _build_span_tree(
        ...
        def build_node(span: Any, is_root: bool = False, step_index: Optional[int] = None) -> TraceNodeResponse:
            ...
            children_nodes: Optional[List[TraceNodeResponse]] = None
            ...
            if is_root:
                ...
                node = TraceNodeResponse.from_span(span, spans=children_nodes)
