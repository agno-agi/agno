...
async def handle_workflow_via_websocket(websocket: WebSocket, message: dict, os: "AgentOS"):
    ...
    try:
        ...
        logger.error("Error executing workflow via WebSocket", exc_info=True)
        error_payload = {
            "event": "error",
            "error": str(e),
...
async def handle_workflow_subscription(websocket: WebSocket, message: dict, os: "AgentOS"):
    ...
    try:
        ...
        logger.error("Error handling workflow subscription", exc_info=True)
        await websocket.send_text(
            json.dumps(
                {
                    "event": "error",
        ...
        if os.db and isinstance(os.db, BaseDb):
            ...
            for db_workflow in get_workflows(db=os.db, registry=os.registry):
                try:
                    ...
                    logger.error(f"Error converting workflow {workflow_id} to response", exc_info=True)
                    continue
