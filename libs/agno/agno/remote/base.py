...
class BaseRemote:
    ...
    def __init__(
        ...
        Executes entities on a remote AgentOS instance through its RemoteAccess interface.
        The target AgentOS must mount the RemoteAccess interface (agno.os.interfaces.remote_access.RemoteAccess)
        and explicitly opt the entity in; entities not exposed through the interface are not
        remotely callable.

        Note: the db and knowledge proxies (RemoteDb, RemoteKnowledge) still use the main
        AgentOS API (e.g. /sessions, /memories, /knowledge) and require those routes to be
        reachable on the remote instance.

        Args:
            base_url: Base URL for remote instance (e.g., "http://localhost:7777")
            timeout: Request timeout in seconds (default: 60)
            config_ttl: Time-to-live for cached config in seconds (default: 300)
            api_prefix: Path prefix where the RemoteAccess interface is mounted on the target
                AgentOS (default: "/remote"). Only change this if the interface is mounted
                at a custom prefix.
        """
    ...
    def get_os_client(self) -> "AgentOSClient":
        """Get an AgentOSClient for the remote instance's main API.

        This client is only used by the db and knowledge proxies (RemoteDb,
        RemoteKnowledge); remote execution goes through the Remote interface
        endpoints instead.
    ...
    def _config(self) -> Optional["ConfigResponse"]:
        ...
        if self._cached_config is not None:
            cached_config, cached_at = self._cached_config
    ...
    async def refresh_os_config(self) -> "ConfigResponse":
        ...
        from agno.os.schema import ConfigResponse
        from agno.remote.http import aget_json

        config: ConfigResponse = ConfigResponse.model_validate(
            await aget_json(self.base_url, f"{self.api_prefix}/config", timeout=self.timeout)
        )
