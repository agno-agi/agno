...
class RunRequirement:
    ...
    def needs_user_input(self) -> bool:
        ...
        if self.user_input_schema:
            if all(field.value is not None for field in self.user_input_schema):
                return False
            return True
    ...
    def provide_user_input(self, values: Dict[str, Any]) -> None:
        """Provide user input values for user-input requirements."""
        if not self.needs_user_input:
            raise ValueError("This requirement does not require user input")
        if self.user_input_schema:
            for ui_field in self.user_input_schema:
                if ui_field.name in values:
                    ui_field.value = values[ui_field.name]
            if all(f.value is not None for f in self.user_input_schema):
                if self.tool_execution:
