...
from agno.utils.os import extract_input_media, format_team_tools, format_tools, get_run_input, get_session_name
...
class WorkflowResponse(BaseModel):
    ...
    @classmethod
    def get_workflow_input_schema_dict(cls, workflow: Workflow) -> Optional[Dict[str, Any]]:
        ...
        if workflow.steps and callable(workflow.steps):
            ...
            if custom_params and len(custom_params) > 1:  # More than just 'message'
                return cls._generate_schema_from_params(custom_params)
    ...
    async def from_workflow(cls, workflow: Workflow) -> "WorkflowResponse":
        ...
        return cls(
            ...
            description=workflow.description,
            steps=steps,
            input_schema=cls.get_workflow_input_schema_dict(workflow),
