...
from agno.storage.base import Session, Storage
from agno.utils.log import log_debug, log_info, log_warning, logger


def _banavo_memory(session: Session):
    metadata = getattr(session, "metadata", None) or {}
    return getattr(session, "memory", None) or metadata.get("banavo_memory")


def _banavo_extra_data(session: Session):
    metadata = getattr(session, "metadata", None) or {}
    return getattr(session, "extra_data", None) or metadata.get("banavo_extra_data")


def _banavo_team_session_id(session: Session):
    metadata = getattr(session, "metadata", None) or {}
    return getattr(session, "team_session_id", None) or metadata.get("banavo_team_session_id")


def _team_session_from_row(row) -> TeamSession:
    mapping = dict(row._mapping)
    metadata = {
        "banavo_memory": mapping.get("memory"),
        "banavo_extra_data": mapping.get("extra_data"),
        "banavo_team_session_id": mapping.get("team_session_id"),
    }
    return TeamSession(
        session_id=mapping["session_id"],
        team_id=mapping.get("team_id"),
        user_id=mapping.get("user_id"),
        team_data=mapping.get("team_data"),
        session_data=mapping.get("session_data"),
        metadata=metadata,
...
def _agent_session_from_row(row) -> AgentSession:
    mapping = dict(row._mapping)
    metadata = {
        "banavo_memory": mapping.get("memory"),
        "banavo_extra_data": mapping.get("extra_data"),
    ...
    return AgentSession(
        ...
        team_id=mapping.get("team_id"),
        user_id=mapping.get("user_id"),
        agent_data=mapping.get("agent_data"),
        session_data=mapping.get("session_data"),
        metadata=metadata,
        created_at=mapping.get("created_at"),
        updated_at=mapping.get("updated_at"),
...
def _workflow_session_from_row(row) -> WorkflowSession:
    mapping = dict(row._mapping)
    metadata = {
        "banavo_memory": mapping.get("memory"),
        "banavo_extra_data": mapping.get("extra_data"),
...
class PostgresStorage(Storage):
    ...
    def read(self, session_id: str, user_id: Optional[str] = None) -> Optional[Session]:
        ...
        try:
            with self.Session() as sess:
                ...
                if self.mode == "agent":
                    return _agent_session_from_row(result) if result is not None else None
                elif self.mode == "team":
                    return _team_session_from_row(result) if result is not None else None
                elif self.mode == "workflow":
                    return _workflow_session_from_row(result) if result is not None else None
        except Exception as e:
    ...
    def get_all_sessions(self, user_id: Optional[str] = None, entity_id: Optional[str] = None) -> List[Session]:
        ...
        try:
            with self.Session() as sess, sess.begin():
                ...
                if rows is not None:
                    if self.mode == "agent":
                        return [_agent_session_from_row(row) for row in rows]  # type: ignore
                    elif self.mode == "team":
                        return [_team_session_from_row(row) for row in rows]  # type: ignore
                    else:
                        return [_workflow_session_from_row(row) for row in rows]  # type: ignore
                else:
                    return []
        except Exception as e:
            log_debug(f"Exception reading from table: {e}")
            ...
            with self.Session() as sess, sess.begin():
                ...
                if rows is not None:
                    ...
                    for row in rows:
                        ...
                        if self.mode == "agent":
                            session = _agent_session_from_row(row)  # type: ignore
                        elif self.mode == "team":
                            session = _team_session_from_row(row)  # type: ignore
                        elif self.mode == "workflow":
                            session = _workflow_session_from_row(row)  # type: ignore
            ...
            with self.Session() as sess, sess.begin():
                ...
                if self.mode == "agent":
                    stmt = postgresql.insert(self.table).values(
                        ...
                        team_session_id=_banavo_team_session_id(session),
                        user_id=session.user_id,
                        memory=_banavo_memory(session),
                        agent_data=session.agent_data,  # type: ignore
                        session_data=session.session_data,
                        extra_data=_banavo_extra_data(session),
                    )
    ...
    def update(
        ...
        **kwargs,
        ...
        """Banavo-compatible alias for upsert; accepts the session object to persist."""
        session = session or kwargs.get("team_session") or kwargs.get("agent_session")
        if session is None:
            log_warning("PostgresStorage.update called without session; nothing persisted")
            return None
