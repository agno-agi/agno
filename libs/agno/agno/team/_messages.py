...
def _get_mode_instructions(team: "Team") -> str:
    ...
    if team.mode == TeamMode.tasks:
        ...
        content += (
            ...
            "select the best member(s), delegate with clear task descriptions, and by default "
            "synthesize their outputs into a unified response. When a member's output should be "
            "the final user-facing answer (for example a long-form writer), call "
            "`delegate_task_to_member` with `respond_directly=True` instead of synthesizing. "
            "For requests you can handle directly — simple questions, "
            ...
            "- Use `respond_directly=False` (default) for intermediate specialist work whose results "
            "you still need to review or combine.\n"
            "- Use `respond_directly=True` only for the final deliverable that should be returned to "
            "the user as-is. After that call, do not rewrite or summarize the member's output — "
            "the run stops and their response is the answer.\n\n"
            "After receiving member responses (when `respond_directly` was False):\n"
