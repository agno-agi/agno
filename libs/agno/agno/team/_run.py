    ...
    **kwargs: Any,
    ...
    async def _background_producer() -> None:
        ...
        try:
            ...
            try:
                fresh_session = await _aread_or_create_session(team, session_id=session_id, user_id=user_id)
                cancelled_run = run_response if run_response is not None else fresh_session.get_run(_run_id)
                if cancelled_run is not None:
                    cancelled_run.status = RunStatus.cancelled
                    fresh_session.upsert_run(run_response=cancelled_run)
                    await asave_session(team, session=fresh_session)
            except Exception:
                log_error(
            ...
            try:
                fresh_session = await _aread_or_create_session(team, session_id=session_id, user_id=user_id)
                errored_run = run_response if run_response is not None else fresh_session.get_run(_run_id)
                if errored_run is not None:
                    errored_run.status = RunStatus.error
                    fresh_session.upsert_run(run_response=errored_run)
                    await asave_session(team, session=fresh_session)
            except Exception:
                log_error(
