...
def save_session(team: "Team", session: TeamSession) -> None:
    ...
    if team.db is not None and team.parent_team_id is None and team.workflow_id is None:
        ...
        # Runs when media_storage is set and store_media is True; offload is idempotent
        # (already-offloaded media is skipped), so this is a no-op when _cleanup_and_store
        # already offloaded.
