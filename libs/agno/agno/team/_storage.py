...
def _hydrate_from_graph(
    ...
    *,
    ...
    team = cls.from_dict(
        config, db=db, registry=registry, skill_executor=skill_executor, links=member_links, strict=strict
    )
    ...
    for child in graph.get("children", []):
        ...
        if member_type == "agent":
            agent = Agent.from_dict(
                child_config, db=db, registry=registry, skill_executor=skill_executor, strict=strict
            )
            ...
            nested_team = _hydrate_from_graph(
                cls, child_graph, db=db, registry=registry, skill_executor=skill_executor, strict=strict
            )
...
def load(
    ...
    *,
    ...
    return _hydrate_from_graph(cls, graph, db=db, registry=registry, skill_executor=skill_executor, strict=strict)
