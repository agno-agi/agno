...
class Team:
    ...
    def __init__(
        ...
        if context_settings is not None:
            ...
            warn_unsupported_setting(context_settings, "user_message_role", "Team")
            warn_unsupported_setting(context_settings, "build_context", "Team")
            warn_unsupported_setting(context_settings, "build_user_context", "Team")
        ...
        if history_settings is not None:
            ...
            warn_unsupported_setting(history_settings, "read_tool_call_history", "Team")
        if output_settings is not None:
            ...
            warn_unsupported_setting(output_settings, "structured_outputs", "Team")
            warn_unsupported_setting(output_settings, "save_response_to_file", "Team")
        ...
        if event_settings is not None:
            ...
            warn_unsupported_setting(event_settings, "stream_executor_events", "Team")
        ...
        if cache_settings is not None:
            ...
            callable_members_cache_key = resolve_setting(
                cache_settings, "callable_members_cache_key", callable_members_cache_key
            )
        if delegation_settings is not None:
            mode = resolve_setting(delegation_settings, "mode", mode)
            respond_directly = resolve_setting(delegation_settings, "respond_directly", respond_directly)
            determine_input_for_members = resolve_setting(
                delegation_settings, "determine_input_for_members", determine_input_for_members
            )
            delegate_to_all_members = resolve_setting(
                delegation_settings, "delegate_to_all_members", delegate_to_all_members
            )
            max_iterations = resolve_setting(delegation_settings, "max_iterations", max_iterations)
            add_team_history_to_members = resolve_setting(
                delegation_settings, "add_team_history_to_members", add_team_history_to_members
            )
            num_team_history_runs = resolve_setting(delegation_settings, "num_team_history_runs", num_team_history_runs)
            share_member_interactions = resolve_setting(
                delegation_settings, "share_member_interactions", share_member_interactions
            )
            add_member_tools_to_context = resolve_setting(
                delegation_settings, "add_member_tools_to_context", add_member_tools_to_context
            )
            get_member_information_tool = resolve_setting(
                delegation_settings, "get_member_information_tool", get_member_information_tool
            )
            store_member_responses = resolve_setting(
                delegation_settings, "store_member_responses", store_member_responses
            )
            stream_member_events = resolve_setting(delegation_settings, "stream_member_events", stream_member_events)
            show_members_responses = resolve_setting(
                delegation_settings, "show_members_responses", show_members_responses
            )
