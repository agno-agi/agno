...
2. Destructive tools default False (recommend `requires_confirmation_tools` in the class docstring for the most dangerous ones)
...
### Design
21. Prefer no `all` param if ≤2 tools (toolkits that had it in 2.x keep it for backwards compatibility)
...
Shipped toolkits enforce safety through default-False flags: the user opts in
to destructive tools at construction time. Users can additionally pass
`requires_confirmation_tools=["delete"]` to any toolkit; mention this in the
class docstring for highly destructive tools (see `shell.py`).
