...
class CartesiaTools(Toolkit):
            ...
            if voice_objects:
                for voice in voice_objects:
                    try:
                        ...
                        log_error(f"AttributeError accessing voice data. Voice data: {voice}")
                        continue
                    except Exception:
                        log_error(f"Unexpected error processing voice. Voice data: {voice}")
                        continue

            return json.dumps(filtered_result, indent=4)
        except Exception as e:
            log_error(f"Error listing voices from Cartesia: {e}")
    ...
    def localize_voice(
        ...
        try:
            ...
            log_error(f"Error localizing voice with Cartesia: {e}")
    ...
    def text_to_speech(
        ...
        try:
            ...
            log_error(f"Error generating speech with Cartesia: {e}")
