...
class GmailTools(Toolkit):
    ...
    def get_message(self, message_id: str, download_attachments: bool = False) -> str:
        ...
        try:
            ...
            log_exception(f"Failed to get message {message_id}")
    ...
    def get_thread(self, thread_id: str) -> str:
        ...
        try:
            ...
            log_exception(f"Failed to get thread {thread_id}")
            return json.dumps({"error": f"Gmail API error: {e}"})
        except Exception as e:
            log_exception("Unexpected error")
            return json.dumps({"error": f"Unexpected error: {type(e).__name__}: {e}"})
    ...
    def search_threads(self, query: str, count: int = 10) -> str:
        ...
        try:
            ...
            log_exception("Thread search failed")
            return json.dumps({"error": f"Gmail API error: {e}"})
        except Exception as e:
            log_exception("Unexpected error")
            return json.dumps({"error": f"Unexpected error: {type(e).__name__}: {e}"})
    ...
    def modify_thread_labels(
        ...
        try:
            ...
            log_exception(f"Failed to modify labels on thread {thread_id}")
            return json.dumps({"error": f"Gmail API error: {e}"})
        except Exception as e:
            log_exception("Unexpected error")
            return json.dumps({"error": f"Unexpected error: {type(e).__name__}: {e}"})
    ...
    def trash_thread(self, thread_id: str) -> str:
        ...
        try:
            ...
            log_exception(f"Failed to trash thread {thread_id}")
            return json.dumps({"error": f"Gmail API error: {e}"})
        except Exception as e:
            log_exception("Unexpected error")
            return json.dumps({"error": f"Unexpected error: {type(e).__name__}: {e}"})
    ...
    def get_draft(self, draft_id: str) -> str:
        ...
        try:
            ...
            log_exception(f"Failed to get draft {draft_id}")
            return json.dumps({"error": f"Gmail API error: {e}"})
        except Exception as e:
            log_exception("Unexpected error")
            return json.dumps({"error": f"Unexpected error: {type(e).__name__}: {e}"})
    ...
    def list_drafts(self, count: int = 10) -> str:
        ...
        try:
            ...
            log_exception("Failed to list drafts")
            return json.dumps({"error": f"Gmail API error: {e}"})
        except Exception as e:
            log_exception("Unexpected error")
            return json.dumps({"error": f"Unexpected error: {type(e).__name__}: {e}"})
    ...
    def send_draft(self, draft_id: str) -> str:
        ...
        try:
            ...
            log_exception(f"Failed to send draft {draft_id}")
            return json.dumps({"error": f"Gmail API error: {e}"})
        except Exception as e:
            log_exception("Unexpected error")
            return json.dumps({"error": f"Unexpected error: {type(e).__name__}: {e}"})
    ...
    def update_draft(
        ...
        try:
            ...
            log_exception(f"Failed to update draft {draft_id}")
            return json.dumps({"error": f"Gmail API error: {e}"})
        except Exception as e:
            log_exception(f"Failed to update draft {draft_id}")
    ...
    def list_labels(self) -> str:
        ...
        try:
            ...
            log_exception("Failed to list labels")
            return json.dumps({"error": f"Gmail API error: {e}"})
        except Exception as e:
            log_exception("Unexpected error")
            return json.dumps({"error": f"Unexpected error: {type(e).__name__}: {e}"})
    ...
    def modify_message_labels(
        ...
        try:
            ...
            log_exception(f"Failed to modify labels on message {message_id}")
            return json.dumps({"error": f"Gmail API error: {e}"})
        except Exception as e:
            log_exception("Unexpected error")
            return json.dumps({"error": f"Unexpected error: {type(e).__name__}: {e}"})
    ...
    def trash_message(self, message_id: str, undo: bool = False) -> str:
        ...
        try:
            ...
            log_exception(f"Failed to {action_name} message {message_id}")
            return json.dumps({"error": f"Gmail API error: {e}"})
        except Exception as e:
            log_exception("Unexpected error")
            return json.dumps({"error": f"Unexpected error: {type(e).__name__}: {e}"})
    ...
    def download_attachment(self, message_id: str, attachment_id: str, filename: str) -> str:
        ...
        try:
            ...
            log_exception(f"Failed to download attachment from {message_id}")
            return json.dumps({"error": f"Gmail API error: {e}"})
        except Exception as e:
            log_exception("Unexpected error")
            return json.dumps({"error": f"Unexpected error: {type(e).__name__}: {e}"})
