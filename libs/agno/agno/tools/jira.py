...
class JiraTools(Toolkit):
    ...
    def get_issue(self, issue_key: str) -> str:
        ...
        try:
            ...
            logger.error(f"Error retrieving issue {issue_key}", exc_info=True)
    ...
    def create_issue(self, project_key: str, summary: str, description: str, issuetype: str = "Task") -> str:
        ...
        try:
            ...
            logger.error(f"Error creating issue in project {project_key}", exc_info=True)
            return json.dumps({"error": str(e)})
    ...
    def search_issues(self, jql_str: str, max_results: int = 50) -> str:
        ...
        try:
            ...
            logger.error(f"Error searching issues with JQL '{jql_str}'", exc_info=True)
            return json.dumps([{"error": str(e)}])
    ...
    def add_comment(self, issue_key: str, comment: str) -> str:
        ...
        try:
            ...
            logger.error(f"Error adding comment to issue {issue_key}", exc_info=True)
            return json.dumps({"error": str(e)})
    ...
    def add_worklog(self, issue_key: str, time_spent: str, comment: Optional[str] = None) -> str:
        ...
        try:
            ...
            logger.error(f"Error adding worklog to issue {issue_key}", exc_info=True)
            return json.dumps({"error": str(e)})
