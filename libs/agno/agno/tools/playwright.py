...
class PlaywrightTools(Toolkit):
    ...
    def _ensure_browser_ready(self) -> Page:
        """Ensures local browser is ready. Creates and initializes if needed.

        Returns:
            The active Page instance (guaranteed non-None)

        Raises:
            RuntimeError: If browser initialization fails
        """
        if self._session_initialized and self._page is not None:
            return self._page
    ...
    def navigate_to(self, url: str) -> str:
        ...
        page = self._ensure_browser_ready()
        page.goto(url, wait_until="networkidle", timeout=self.timeout)
        result = {"status": "complete", "title": page.title(), "url": url}
    ...
    def go_back(self) -> str:
        ...
        page = self._ensure_browser_ready()
        page.go_back(wait_until="networkidle", timeout=self.timeout)
        new_url = page.url
    ...
    def go_forward(self) -> str:
        ...
        page = self._ensure_browser_ready()
        page.go_forward(wait_until="networkidle", timeout=self.timeout)
        new_url = page.url
    ...
    def fill_input(self, selector: str, text: str) -> str:
        ...
        page = self._ensure_browser_ready()
        page.fill(selector, text, timeout=self.timeout)
