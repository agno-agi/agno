...
Semantics:
    ...
    * Persisted components rebuild from their stored config. Registry-backed
      ...
      it.
    * list_* report everything this runner can dispatch: stored components plus
      admitted code-defined components. Code-defined components arrive through
      the registry, which is passed so persisted components can rehydrate rather
      than to grant the runner the run of the application, so dispatching them
      is opt-in via include_all_components. An explicit agents_list/teams_list/
      workflows_list is itself the allowlist and always runs, even without a
      database. 'total' counts distinct runnable components, so a capped list is
      visible as capped and a stored row shadowed by a code-defined id is counted
      only once.
...
def _pinned_reference_version(
    ...
    Step.from_dict prefers the matching link key and otherwise accepts a child
    fallback only when every matching link pins the same version. Mirror that
    rule so the fidelity check judges the same config version that
    reconstruction used and never chooses one side of an ambiguous pin.
    """
    ...
    for link in links:
        ...
        candidate_key = link.get("link_key")
        if link_key is not None and candidate_key == link_key:
            return child_version
        candidates.append((candidate_key if isinstance(candidate_key, str) else None, child_version))
    if not candidates:
        return None
    ...
    raise ComponentNotDispatchableError(
        f"Reference '{ref_id}' matches no link for key '{link_key}', and the parent pins that child "
        "at multiple versions. Re-save the parent so every reference pin is keyed to its occurrence."
    )
