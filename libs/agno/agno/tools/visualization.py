        ...
        try:
            ...
            if is_timeseries:
                ...
                if all(isinstance(d, datetime) for d in datetime_values):
                    parsed_x = datetime_values

            # Create the chart
            plt.figure(figsize=(10, 6))
            color = colors[0] if colors else None
            plt.plot(parsed_x, y_values, marker="o", linewidth=2, markersize=6, color=color)
            plt.title(title, fontsize=14, fontweight="bold")
            plt.xlabel(x_label, fontsize=11)
            plt.ylabel(y_label, fontsize=11)
            
            # Auto-rotate if many x-values or datetime
            if len(x_values) > 8 or isinstance(parsed_x[0], datetime):
                plt.xticks(rotation=45, ha="right")
            
            # Show values at points
            if show_values:
                for i, (x, y) in enumerate(zip(parsed_x, y_values)):
                    plt.text(x, y, self._format_large_numbers(y), ha="center", va="bottom", fontsize=8)
        ...
        try:
            ...
            if is_timeseries:
                ...
                if all(isinstance(d, datetime) for d in parsed_dates):
                    display_categories = parsed_dates
            ...
            if chart_type == "bar":
                ...
                plt.xticks([x + bar_width * (len(series_names) - 1) / 2 for x in x_positions], display_categories)
                ...
                plt.xticks(x_positions, display_categories)
                ...
                for i, series_name in enumerate(series_names):
                    values = [normalized_series[series_name].get(cat, 0) for cat in categories]
                    color = colors[i] if colors and i < len(colors) else None
                    plt.plot(display_categories, values, marker="o", linewidth=2, markersize=6, label=series_name, color=color)
                    
                    if show_values:
                        for x, y in zip(display_categories, values):
                            plt.text(x, y, self._format_large_numbers(y), ha="center", va="bottom", fontsize=7)
                
                if len(categories) > 8 or isinstance(display_categories[0], datetime):
                    plt.xticks(rotation=45, ha="right")
            else:
