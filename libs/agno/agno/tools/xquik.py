...
class XquikTools(Toolkit):
    ...
    def search_posts(self, query: str, max_results: int = 20, cursor: Optional[str] = None) -> str:
        """
        ...
        Args:
            ...
            max_results (int): The maximum number of posts to retrieve (1-200).
            cursor (Optional[str]): Pagination cursor from a previous response.

        Returns:
            A JSON string with matching posts, engagement metrics, and pagination cursors.
        """
        try:
            if not query:
    ...
    def get_tweet(self, tweet_id: str) -> str:
        ...
        try:
            ...
            result = self._format_tweet({**tweet, "author": data.get("author")})
            return json.dumps(result, indent=2)
        except Exception as e:
