"""The Tinker dataset acceptance check. Private, and deliberately vendored twice.

This is the runtime gate in front of every paid `fit` call, not a test oracle. It
lives in two homes on purpose: `agno/environments/exporters/_validate.py` and
`agno/trainers/_validate.py`, byte-identical, because `agno.trainers` must never
import `agno.environments` (the dependency direction is one-way). A named test
asserts the two files are equal so they cannot drift.
