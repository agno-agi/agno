"""Example usage of CAMB AI toolkit with agno."""
...
AUDIO_SAMPLE = os.environ.get(
    "CAMB_AUDIO_SAMPLE",
    str(Path(__file__).resolve().parents[4] / "yt-dlp" / "voices" / "original" / "sabrina-original-clip.mp3"),
)
...
tools = CambTools(api_key=API_KEY)


def test_tts():
    """1. Text-to-Speech: convert text to audio."""
    path = tools.text_to_speech("Hello from CAMB AI and agno! This is a text to speech test.")
...
def test_translation():
    ...
    result = tools.translate("Hello, how are you?", source_language=1, target_language=2)
...
def test_transcription():
    ...
    result = tools.transcribe(language=1, audio_file_path=AUDIO_SAMPLE)
...
def test_translated_tts():
    ...
    path = tools.translated_tts(text="Hello, how are you?", source_language=1, target_language=2)
    print(f"  Audio saved to: {path}")
...
def test_text_to_sound():
    ...
    path = tools.text_to_sound("gentle rain on a rooftop", duration=5.0, audio_type="sound")
    print(f"  Audio saved to: {path}")
    assert path.endswith(".wav")
    play(path)
...
def test_voice_clone():
    ...
    result = tools.clone_voice(voice_name="test_clone_agno", audio_file_path=AUDIO_SAMPLE, gender=2)
    print(f"  Result: {result}")
    ...
    path = tools.text_to_speech(
        "Hello! This is the cloned voice from the audio sample, generated with CAMB AI and agno.",
        voice_id=voice_id,
    )
    play(path)
...
def test_audio_separation():
    ...
    result = tools.separate_audio(audio_file_path=AUDIO_SAMPLE)
    print(f"  Result: {result}")
...
def test_voice_from_description():
    ...
    result = tools.create_voice_from_description(
