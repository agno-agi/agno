...
def validation_pre_hook(run_input: RunInput) -> None:
    ...
    if hasattr(run_input, 'input_content') and isinstance(run_input.input_content, str) and "forbidden" in run_input.input_content.lower():
...
def tracking_post_hook(run_output: RunOutput, agent: Agent) -> None:
    ...
    hook_execution_tracker["post_hooks"].append(f"post_hook:{agent.name}:{len(run_output.content) if run_output.content else 0}")
...
async def async_tracking_post_hook(run_output: RunOutput, agent: Agent) -> None:
    ...
    hook_execution_tracker["post_hooks"].append(f"async_post_hook:{agent.name}:{len(run_output.content) if run_output.content else 0}")
...
def test_combined_input_output_validation():
    ...
    def input_validator(run_input: RunInput) -> None:
        if hasattr(run_input, 'input_content') and isinstance(run_input.input_content, str) and "hack" in run_input.input_content.lower():
...
def test_hook_error_handling_comprehensive():
    ...
    try:
        result = agent.run(input="Test error handling")
...
def test_hook_with_guardrail_exceptions():
    ...
    def strict_input_hook(run_input: RunInput) -> None:
        if hasattr(run_input, 'input_content') and isinstance(run_input.input_content, str) and len(run_input.input_content) > 50:
...
def test_hook_receives_correct_parameters():
    ...
    def param_capturing_pre_hook(run_input: RunInput, agent: Agent, session: AgentSession, user_id: Optional[str] = None, debug_mode: Optional[bool] = None) -> None:
    ...
    def param_capturing_post_hook(run_output: RunOutput, agent: Agent, session: AgentSession, user_id: Optional[str] = None, debug_mode: Optional[bool] = None) -> None:
...
def test_pre_hook_modifies_run_input():
    ...
    agent = create_test_agent(
        pre_hooks=input_modifying_pre_hook,
