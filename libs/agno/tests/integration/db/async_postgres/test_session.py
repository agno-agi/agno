...
async def test_upsert_session_updates_existing_session_when_user_id_changes(
    ...
    await async_postgres_db_real.upsert_session(session_factory("@alice:example.org", ["run_1"]))

    result = await async_postgres_db_real.upsert_session(session_factory("@bob:example.org", ["run_1", "run_2"]))
    ...
    stored = await async_postgres_db_real.get_session(result.session_id, session_type)
