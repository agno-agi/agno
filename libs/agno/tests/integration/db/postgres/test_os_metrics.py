...
def _stored_rows(db: PostgresDb) -> Dict[tuple, Dict]:
    ...
    with db.Session() as sess:
        return {
            ...
            for row in sess.execute(select(table)).fetchall()
        }
...
def test_calculate_os_metrics_skips_a_rebuild_already_running(
    ...
    assert len(_stored_rows(postgres_db_real)) == len(YESTERDAY_ROW_KEYS) + 1
...
def test_calculate_os_metrics_skips_messages_carried_over_from_history(postgres_db_real):
    ...
    row = _stored_rows(postgres_db_real)[(_utc_date(1), "alice", "agent-1", "", "")]
...
def test_get_os_metrics_by_date(postgres_db_real: PostgresDb, sample_sessions_for_os_metrics):
    ...
    metrics, latest_updated_at = postgres_db_real.get_os_metrics(starting_date=yesterday, ending_date=today)
    ...
    metrics, _ = postgres_db_real.get_os_metrics(starting_date=yesterday, ending_date=today, user_id="bob")
    ...
    metrics, _ = postgres_db_real.get_os_metrics(starting_date=yesterday, ending_date=today, user_id="")
    assert [m["date"] for m in metrics] == [yesterday]
...
def test_get_os_metrics_fields(postgres_db_real: PostgresDb, sample_sessions_for_os_metrics):
    ...
    with pytest.raises(ValueError):
        postgres_db_real.get_os_metrics(starting_date=yesterday, ending_date=yesterday, fields=["users_count"])
...
def test_get_os_metrics_no_rows(postgres_db_real: PostgresDb):
    ...
    metrics, latest_updated_at = postgres_db_real.get_os_metrics(starting_date=_utc_date(1), ending_date=_utc_date(0))
