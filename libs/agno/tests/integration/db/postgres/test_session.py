...
def _make_agent_session_for_user(user_id: str, run_ids: List[str]) -> AgentSession:
    now = int(time.time())
    return AgentSession(
        session_id="upsert_user_change_agent",
        agent_id="upsert_user_change_agent_id",
        user_id=user_id,
        runs=[
            RunOutput(
                run_id=run_id,
...
def _make_team_session_for_user(user_id: str, run_ids: List[str]) -> TeamSession:
    now = int(time.time())
    return TeamSession(
        session_id="upsert_user_change_team",
        team_id="upsert_user_change_team_id",
        user_id=user_id,
        runs=[
            TeamRunOutput(
                run_id=run_id,
...
def _make_workflow_session_for_user(user_id: str, run_ids: List[str]) -> WorkflowSession:
    ...
    return WorkflowSession(
        session_id="upsert_user_change_workflow",
        workflow_id="upsert_user_change_workflow_id",
        user_id=user_id,
        runs=[
            WorkflowRunOutput(
                run_id=run_id,
...
def test_upsert_session_updates_existing_session_when_user_id_changes(
    postgres_db_real: PostgresDb,
    session_type: SessionType,
    session_factory,
):
    """Upserting an existing session with a new user_id updates instead of silently skipping."""
    postgres_db_real.upsert_session(session_factory("@alice:example.org", ["run_1"]))

    result = postgres_db_real.upsert_session(session_factory("@bob:example.org", ["run_1", "run_2"]))
    ...
    stored = postgres_db_real.get_session(result.session_id, session_type)
...
def test_upsert_sessions_updates_existing_sessions_when_user_id_changes(postgres_db_real: PostgresDb):
    """Bulk upserting existing sessions with new user_ids updates instead of silently skipping."""
    original_sessions = [
        _make_agent_session_for_user("@alice:example.org", ["agent_run_1"]),
        _make_team_session_for_user("@alice:example.org", ["team_run_1"]),
        _make_workflow_session_for_user("@alice:example.org", ["workflow_run_1"]),
    ]
    postgres_db_real.upsert_sessions(original_sessions)

    updated_sessions = [
        _make_agent_session_for_user("@bob:example.org", ["agent_run_1", "agent_run_2"]),
        _make_team_session_for_user("@bob:example.org", ["team_run_1", "team_run_2"]),
        _make_workflow_session_for_user("@bob:example.org", ["workflow_run_1", "workflow_run_2"]),
    ]

    results = postgres_db_real.upsert_sessions(updated_sessions)

    assert len(results) == 3
    for result in results:
        assert result.user_id == "@bob:example.org"
        assert result.runs is not None
        assert [run.run_id for run in result.runs][-1].endswith("_run_2")
