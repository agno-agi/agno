...
def run_race_test(db: PostgresDb, num_tasks: int = 10):
    """Run a single race condition test using PostgresDb.create_trace()."""
    ...
    traces = [
        create_test_trace(trace_id, f"Agent.run-task-{i}", i)
    ...
    with ThreadPoolExecutor(max_workers=num_tasks) as executor:
        futures = [
            executor.submit(concurrent_create_trace, db, traces[i], i, barrier)
            for i in range(num_tasks)
        ]
