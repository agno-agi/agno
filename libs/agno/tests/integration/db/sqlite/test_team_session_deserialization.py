"""
Integration tests for SqliteDb Team session deserialization.
Tests the fix for issue #4894 for SqliteDb - Team sessions
with mixed RunOutput and TeamRunOutput types.
"""
...
def temp_sqlite_db():
    ...
    with tempfile.NamedTemporaryFile(suffix=".db", delete=False) as temp_file:
        db = SqliteDb(db_file=temp_file.name, session_table="test_sessions")
        yield db


def test_sqlitedb_deserialize_team_session_with_mixed_runs(temp_sqlite_db):
    """
    Test that SqliteDb can correctly deserialize Team sessions containing
    ...
    # Store the session
    temp_sqlite_db.upsert_session(team_session)

    # Read the session back - this is where the bug occurred
    # Before fix: Would raise TypeError when trying to deserialize mixed run types
    retrieved_session = temp_sqlite_db.get_session(
...
def test_sqlitedb_agent_only_runs_in_team_session(temp_sqlite_db):
...
def test_sqlitedb_team_only_runs_in_team_session(temp_sqlite_db):
