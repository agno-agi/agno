...
@pytest.fixture
def setup_vector_db_a():
    """Setup a temporary vector DB for testing."""
    table_name = f"text_test_{os.urandom(4).hex()}"
    vector_db = LanceDb(table_name=table_name, uri="tmp/lancedb", embedder=get_embedder())
    yield vector_db
    # Clean up after test
    vector_db.drop()
...
async def aprepare_knowledge_base_a(setup_vector_db):
...
async def aprepare_knowledge_base_b(setup_vector_db):
    """Prepare a knowledge base with filtered data asynchronously."""
    # Create knowledge base
    kb = TextKnowledgeBase(vector_db=setup_vector_db)
...
def test_text_knowledge_base_directory(setup_vector_db_a, setup_vector_db_b):
    """Test loading a directory of text files into the knowledge base."""
    text_dir = get_test_data_dir()

    kb_a = TextKnowledgeBase(
        path=text_dir,
        formats=[".txt"],
        vector_db=setup_vector_db_a,
    )

    kb_b = TextKnowledgeBase(
        path=text_dir,
        formats=[".txt"],
        vector_db=setup_vector_db_b,
    )
    ...
    assert setup_vector_db_a.exists()
    assert setup_vector_db_b.exists()
    # pg_essay.txt is split into 9 documents
    assert setup_vector_db_a.get_count() == 9
    assert setup_vector_db_b.get_count() == 9

    agent = Agent(knowledge=kb, model=get_model())
...
@pytest.mark.asyncio
async def test_text_knowledge_base_async_directory(setup_vector_db_a, setup_vector_db_b):
...
def test_text_knowledge_base_with_metadata_path(setup_vector_db_a, setup_vector_db_b):
...
def test_knowledge_base_with_metadata_path_invalid_filter(setup_vector_db_a, setup_vector_db_b):
    ...
    agent = Agent(knowledge=kb, knowledge_filters={"nonexistent_filter": "value"}, model=get_model())
...
def test_knowledge_base_with_valid_filter(setup_vector_db_a, setup_vector_db_b):
    ...
    agent = Agent(knowledge=kb, knowledge_filters={"user_id": "jordan_mitchell"}, model=get_model())
...
def test_knowledge_base_with_run_level_filter(setup_vector_db_a, setup_vector_db_b):
...
def test_knowledge_base_priority_sources(setup_vector_db_a, setup_vector_db_b):
    """Test filtering knowledge base with filters passed at run time."""
    kb_a = TextKnowledgeBase(
        path=[
            {
    ...
    # Run a query with filters provided at run time
    response = agent.run("Tell me information about software engineer?", markdown=True)
    ...
    assert "jordan mitchell" in response_content.lower() and "taylor_brooks" not in response_content.lower()
