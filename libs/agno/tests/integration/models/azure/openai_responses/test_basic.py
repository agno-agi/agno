...
def test_basic(azure_openai_responses_model):
    """Test basic functionality of the AzureOpenAIResponses model."""
...
async def test_async_basic(azure_openai_responses_model):
    """Test basic async functionality of the AzureOpenAIResponses model."""
    agent = Agent(model=azure_openai_responses_model, markdown=True, telemetry=False)
...
def test_exception_handling():
    ...
    agent = Agent(model=AzureOpenAIResponses(id="invalid-model-123"), markdown=True, telemetry=False)
...
def test_with_memory(azure_openai_responses_model):
    """Test that the model retains context from previous interactions."""
    agent = Agent(
        model=azure_openai_responses_model,
        db=SqliteDb(db_file="tmp/azure/openai_responses/test_with_memory.db"),
...
def test_structured_output_json_mode(azure_openai_responses_model):
    """Test structured output with Pydantic models using JSON mode."""
...
def test_structured_output(azure_openai_responses_model):
...
def test_history(azure_openai_responses_model):
    """Test conversation history in the agent."""
    agent = Agent(
        model=azure_openai_responses_model,
        db=SqliteDb(db_file="tmp/azure/openai_responses/test_history.db"),
        add_history_to_context=True,
        telemetry=False,
    )
...
def test_client_persistence(azure_openai_responses_model):
    """Test that the same OpenAI client instance is reused across multiple calls"""
    agent = Agent(model=azure_openai_responses_model, markdown=True, telemetry=False)

    # First call should create a new client
    agent.run("Hello")
    first_client = azure_openai_responses_model.client
    assert first_client is not None

    # Second call should reuse the same client
    agent.run("Hello again")
    second_client = azure_openai_responses_model.client
    assert second_client is not None
    assert first_client is second_client, "Client should be persisted and reused"


@pytest.mark.asyncio
async def test_async_client_persistence(azure_openai_responses_model):
    ...
    first_client = azure_openai_responses_model.async_client
    assert first_client is not None

    # Second call should reuse the same async client
    await agent.arun("Hello again")
    second_client = azure_openai_responses_model.async_client
    assert second_client is not None
