...
def test_basic():
    agent = Agent(model=DaoXE(id=DAOXE_MODEL_ID), markdown=True, telemetry=False)
...
def test_with_memory():
    agent = Agent(
        db=SqliteDb(db_file="tmp/test_with_memory_daoxe.db"),
...
def test_history():
    agent = Agent(
        model=DaoXE(id=DAOXE_MODEL_ID),
        db=SqliteDb(db_file="tmp/daoxe/test_basic.db"),
        add_history_to_context=True,
