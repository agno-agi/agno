...
def test_image_input():
    agent = Agent(
        model=Gemini(id="gemini-2.0-flash-exp"), tools=[DuckDuckGoTools()], markdown=True, telemetry=False, monitoring=False
    )
...
def test_audio_input_bytes():
    ...
    agent = Agent(model=Gemini(id="gemini-2.0-flash-exp"), markdown=True, telemetry=False, monitoring=False)
