...
from agno.run.team import TeamRunOutput, TeamRunInput
...
def tracking_post_hook(run_output: TeamRunOutput, team: Team) -> None:
    ...
    hook_execution_tracker["post_hooks"].append(f"post_hook:{team.name}:{len(run_output.content) if run_output.content else 0}")
...
async def async_tracking_post_hook(run_output: TeamRunOutput, team: Team) -> None:
    ...
    hook_execution_tracker["post_hooks"].append(f"async_post_hook:{team.name}:{len(run_output.content) if run_output.content else 0}")
...
def test_hooks_actually_execute_during_run():
    ...
    team = create_test_team(
        pre_hooks=tracking_pre_hook,
        post_hooks=tracking_post_hook
    )
...
def test_prompt_injection_detection():
    ...
    def prompt_injection_check(run_input: TeamRunInput) -> None:
        ...
        if any(pattern in run_input.input_content.lower() for pattern in injection_patterns):
            raise InputCheckError("Prompt injection detected", check_trigger=CheckTrigger.INJECTION_DETECTED)
    ...
    assert exc_info.value.check_trigger == CheckTrigger.INJECTION_DETECTED
...
async def test_async_hooks_with_arun():
    ...
    team = create_test_team(
        pre_hooks=async_tracking_pre_hook,
        post_hooks=async_tracking_post_hook
    )
...
def test_post_hook_modifies_output():
    ...
    team = create_test_team(
        post_hooks=output_modifying_post_hook,
...
def test_hook_receives_correct_parameters():
    ...
    def param_capturing_pre_hook(run_input: TeamRunInput, team: Team, session: TeamSession, user_id: Optional[str] = None, debug_mode: Optional[bool] = None) -> None:
    ...
    def param_capturing_post_hook(run_output: TeamRunOutput, team: Team, session: TeamSession, user_id: Optional[str] = None, debug_mode: Optional[bool] = None) -> None:
    ...
    team = create_test_team(
        pre_hooks=param_capturing_pre_hook,
        post_hooks=param_capturing_post_hook
    )
