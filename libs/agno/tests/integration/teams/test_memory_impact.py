...
def test_team_memory_impact_with_gc_monitoring(agent_storage, team_storage, memory):
    ...
    try:
        ...
        assert len(memory.runs[session_id]) == len(prompts), f"Expected {len(prompts)} runs, got {len(memory.runs[session_id])}"
        
        print(f"✅ Team memory impact test completed successfully")
...
def test_team_memory_cleanup_after_session_switch(agent_storage, team_storage, memory):
    ...
    try:
        ...
        print(f"✅ Team session switching memory test completed successfully")
...
def test_team_memory_with_multiple_members(agent_storage, team_storage, memory):
    """
    ...
    try:
        ...
        print(f"--- Running realistic multi-member team test ---")
        monitor.take_reading("before_realistic_prompts")
        
        for i, prompt in enumerate(realistic_prompts):
            print(f"Processing realistic prompt {i+1}/{len(realistic_prompts)}")
            monitor.take_reading(f"realistic_prompt_{i+1}")
        ...
        monitor.take_reading("after_realistic_prompts")
        monitor.force_gc()
        ...
        assert session_id in memory.runs
        assert len(memory.runs[session_id]) == len(realistic_prompts)
        
        # Verify user memories were created with meaningful content
        user_memories = memory.get_user_memories(user_id=user_id)
        assert len(user_memories) > 0, "No user memories were created"
        
        print(f"✅ Realistic multi-member team test completed successfully")
