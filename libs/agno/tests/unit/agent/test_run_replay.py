...
from agno.agent.trait.replay import prepare_replay_record, should_record_replay
...
def _sampled_options(sample_rate: float) -> ResolvedRunOptions:
    return ResolvedRunOptions(
        stream=False,
        stream_events=False,
        yield_run_output=False,
        ...
        metadata=None,
        output_schema=None,
        replay_mode=ReplayMode.SAMPLED,
        replay_sample_rate=sample_rate,
        replay_max_payload_bytes=262_144,
        replay_max_message_chars=4_000,
        replay_compress_payload=False,
    )


def test_sampled_replay_is_deterministic():
    """Same run_id always produces same include/exclude decision."""
    options = _sampled_options(sample_rate=0.5)
    results = [
        should_record_replay(options, status=RunStatus.completed, run_id="deterministic-run-42") for _ in range(100)
    ]
    assert len(set(results)) == 1, "should_record_replay must be deterministic for the same run_id"


def test_sampled_replay_respects_sample_rate_boundaries():
    """rate=0.0 always excludes; rate=1.0 always includes."""
    run_ids = [f"run-{i}" for i in range(50)]
    options_zero = _sampled_options(sample_rate=0.0)
    options_one = _sampled_options(sample_rate=1.0)

    for run_id in run_ids:
        assert not should_record_replay(options_zero, status=RunStatus.completed, run_id=run_id), (
            f"sample_rate=0.0 should never record, but recorded for {run_id}"
        )
        assert should_record_replay(options_one, status=RunStatus.completed, run_id=run_id), (
            f"sample_rate=1.0 should always record, but skipped for {run_id}"
        )


def test_sampled_replay_returns_false_for_none_run_id():
    """Sampled mode must return False when run_id is None."""
    options = _sampled_options(sample_rate=1.0)
    assert not should_record_replay(options, status=RunStatus.completed, run_id=None)
