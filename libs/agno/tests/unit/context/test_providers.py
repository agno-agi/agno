...
def test_slack_read_surfaces_are_split_by_mode():
    ...
    assert "slack_search_workspace" not in bot_tools.functions
    assert "slack_get_channel_history" in bot_tools.functions
    assert "slack_search_workspace" in assisted_tools.functions
    assert "slack_get_channel_history" in assisted_tools.functions
    assert "slack_get_thread" in assisted_tools.functions


def test_slack_bot_token_without_user_token_excludes_search_messages():
    """Without a user token, slack_search_messages should not be registered."""
    p = SlackContextProvider(token="xoxb-x")
    bot_tools = p._ensure_bot_read_tools()
    assisted_tools = p._ensure_assisted_read_tools()

    assert "slack_search_messages" not in bot_tools.functions
    assert "slack_search_messages" not in assisted_tools.functions


def test_slack_user_token_enables_search_messages():
    """With a user token, slack_search_messages should be registered."""
    p = SlackContextProvider(token="xoxb-x", user_token="xoxp-user")
    bot_tools = p._ensure_bot_read_tools()
    assisted_tools = p._ensure_assisted_read_tools()

    assert "slack_search_messages" in bot_tools.functions
    assert "slack_search_messages" in assisted_tools.functions
...
def test_slack_default_read_instructions_stay_tool_specific(monkeypatch):
    ...
    assert "slack_get_channel_history" in captured["slack-bot-read"]
    assert "slack_get_channel_history" in captured["slack-assisted-read"]
    assert "slack_search_workspace" in captured["slack-assisted-read"]
