...
def test_migration_rerun_keeps_a_run_updated_after_the_first_run():
    ...
    from agno.db.migrations.versions.v3_0_0 import _migrate_firestore

    db = _new_db()
    legacy = [_make_run(f"r{i}", "s7", f"stale-{i}").to_dict() for i in range(2)]
    _insert_legacy_session(db, "s7", legacy)

    assert _migrate_firestore(db, "sessions", "agno_sessions") is True
    ...
    _migrate_firestore(db, "sessions", "agno_sessions")
