...
def test_get_embedding(mock_litellm):
    ...
    mock_usage = Mock()
    mock_usage.model_dump.return_value = {"prompt_tokens": 3, "completion_tokens": 0, "total_tokens": 3}
...
async def test_async_batch_embeddings(mock_litellm):
    ...
    mock_usage = Mock()
    mock_usage.model_dump.return_value = {"prompt_tokens": 6, "completion_tokens": 0, "total_tokens": 6}
    mock_response.usage = mock_usage
...
async def test_async_batch_embeddings_short_response(mock_litellm):
    """Test that missing batch entries preserve input alignment."""
    mock_response = Mock()
    mock_response.data = [{"index": 2, "embedding": [0.1, 0.2, 0.3]}]
...
@patch("agno.knowledge.embedder.litellm.litellm")
@pytest.mark.asyncio
async def test_async_batch_embeddings_reorders_dict_entries_by_index(mock_litellm):
    """Test that LiteLLM dict entries are associated with their input indices."""
    mock_response = Mock()
    mock_response.data = [
    ...
    embedder = LiteLLMEmbedder(id="openai/text-embedding-3-small", batch_size=10)
    embeddings, usages = await embedder.async_get_embeddings_batch_and_usage(["Text 0", "Text 1", "Text 2"])

    assert embeddings == [[0.0], [], [2.0]]
    assert usages == [None, None, None]


@patch("agno.knowledge.embedder.litellm.litellm.get_model_info")
def test_dimensions_are_derived_from_litellm_model_info(mock_get_model_info):
    mock_get_model_info.return_value = {"output_vector_size": 3072}

    embedder = LiteLLMEmbedder(id="openai/text-embedding-3-large")

    assert embedder.dimensions == 3072
    mock_get_model_info.assert_called_once_with("openai/text-embedding-3-large")


@patch("agno.knowledge.embedder.litellm.litellm.get_model_info")
def test_dimensions_fall_back_when_model_info_has_no_vector_size(mock_get_model_info):
    mock_get_model_info.return_value = {"output_vector_size": None}

    embedder = LiteLLMEmbedder(id="cohere/embed-english-v3.0")
