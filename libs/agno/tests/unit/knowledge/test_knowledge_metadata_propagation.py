...
def test_prepare_documents_for_insert_with_metadata():
    ...
    # Verify metadata was merged (linked_to is only added when knowledge has a name)
...
def test_prepare_documents_for_insert_without_metadata():
    ...
    # Verify existing metadata is preserved (linked_to is only added when knowledge has a name)
...
def test_load_from_path_without_metadata(temp_text_file, mock_vector_db):
    ...
    # Verify documents were inserted with original metadata preserved (linked_to is only added when knowledge has a name)
    assert len(mock_vector_db.inserted_documents) == 1
    doc = mock_vector_db.inserted_documents[0]
