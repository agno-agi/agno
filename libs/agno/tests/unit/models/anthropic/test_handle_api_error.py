...
and an overload signalled inside a stream (which the SDK raises with the stream's HTTP status, 200)
must be reported as an overload, not as "status 200"."""
...
def _handle(exc: Exception) -> Exception:
    with pytest.raises((ModelProviderError, ModelRateLimitError)) as raised:
        Claude(id="claude-sonnet-4-6", api_key="x")._handle_api_error(exc)
    return raised.value
...
def test_rate_limit_stays_a_warning(logs):
    ...
    raised = _handle(RateLimitError("slow down", response=httpx.Response(429, request=REQUEST), body=None))
