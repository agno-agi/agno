...
def test_poll_once_pending_echoes_interval_one_post():
    ...
    assert len(calls) == 1  # exactly one POST - no hidden loop
...
async def test_apoll_once_pending_and_slow_down():
    ...
    assert len(calls) == 2  # one POST per call
...
async def test_apoll_once_terminal_and_unknown_outcomes():
    async with httpx.AsyncClient(
        transport=httpx.MockTransport(lambda r: httpx.Response(400, json={"error": "access_denied"}))
    ) as client:
    ...
    async with httpx.AsyncClient(
        transport=httpx.MockTransport(lambda r: httpx.Response(400, json={"error": "expired_token"}))
    ) as client:
    ...
    async with httpx.AsyncClient(
        transport=httpx.MockTransport(lambda r: httpx.Response(500, text="upstream exploded"))
    ) as client:
