...
async def test_run_entity_with_context_passes_dependencies_kwarg():
    ...
    Regression for #9519: pre-seeding run_context.dependencies bypassed the merge and
...
async def test_agui_call_pattern_merges_agent_dependencies_with_ui_context():
    """Regression #9519: AG-UI call shape must preserve Agent.dependencies when UI context is set.
