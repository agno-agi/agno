...
class TestAgentFactory:
    ...
    @pytest.mark.asyncio
    async def test_invoke_async_with_async_callable_object_factory(self):
        class Builder:
            async def __call__(self, ctx):
                return _make_mock_agent(f"callable-object-agent-{ctx.user_id}")

        factory = AgentFactory(db=_mock_db, id="f1", factory=Builder())
        assert factory.is_async()
        ctx = _make_ctx(user_id="user-321")
        result = await factory.invoke_async(ctx)
        assert result.id == "callable-object-agent-user-321"
