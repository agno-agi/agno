...
class TestSealedRegistry:
    """A sealed registry is the author's complete declaration: AgentOS never
    folds OS-registered components, their wiring, managers, or knowledge into
    it -- at construction or when the populate methods re-run (resync)."""

    def test_sealed_registry_is_never_folded_into(self):
        declared = Toolkit(name="declared-tools", tools=[])
        registry = Registry(name="curated", tools=[declared], sealed=True)

        mm = MemoryManager(id="mm-seal")
        agent = Agent(
            name="A1",
            id="a1",
            tools=[Toolkit(name="agent-wired", tools=[])],
            memory_manager=mm,
            telemetry=False,
        )
        team = Team(name="T1", id="t1", members=[agent], telemetry=False)

        AgentOS(agents=[agent], teams=[team], registry=registry, telemetry=False)

        assert registry.agents == []
        assert registry.teams == []
        assert registry.tools == [declared]
        assert registry.memory_managers == []
        assert registry.knowledge == []

    def test_sealed_registry_survives_populate_reruns(self):
        registry = Registry(sealed=True)
        agent = Agent(name="A1", id="a1", telemetry=False)
        os = AgentOS(agents=[agent], registry=registry, telemetry=False)

        # resync() re-runs the same populate chain; call it directly.
        os._populate_registry()
        os._populate_registry_managers()
        os._populate_registry_knowledge()
        os._populate_registry_components()
    ...
    def test_unsealed_registry_still_folds(self):
        registry = Registry()
        agent = Agent(name="A1", id="a1", telemetry=False)
        AgentOS(agents=[agent], registry=registry, telemetry=False)

        assert any(getattr(a, "id", None) == "a1" for a in registry.agents)
