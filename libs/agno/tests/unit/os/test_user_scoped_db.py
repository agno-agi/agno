"""Tests for user-scoped DB adapters and get_scoped_user_id."""
...
from agno.os.user_scoped_db import AsyncUserScopedDbAdapter, UserScopedDbAdapter, is_user_scoped_db
...
class TestUserScopedDbAdapter:
    """Test sync UserScopedDbAdapter."""

    def test_user_id_injected_into_get_sessions(self, mock_db):
        scoped = UserScopedDbAdapter(mock_db, user_id="user-123")
    ...
    def test_user_id_injected_into_get_trace(self, mock_db):
        # Return a lightweight object with the matching user_id so the adapter's
        # post-filter returns it. (On backends without user_id support the adapter
    ...
    def test_user_id_overrides_caller_provided_value(self, mock_db):
        """The scoped adapter should always use its own user_id, not the caller's."""
        scoped = UserScopedDbAdapter(mock_db, user_id="user-123")
    ...
    def test_marker_identifies_user_scoped_adapter(self, mock_db):
        scoped = UserScopedDbAdapter(mock_db, user_id="user-123")
        assert is_user_scoped_db(scoped)
        assert not is_user_scoped_db(mock_db)


class TestAsyncUserScopedDbAdapter:
    """Test async AsyncUserScopedDbAdapter."""

    @pytest.mark.asyncio
    async def test_user_id_injected_into_get_sessions(self, mock_async_db):
        scoped = AsyncUserScopedDbAdapter(mock_async_db, user_id="user-123")
    ...
    def test_marker_identifies_async_user_scoped_adapter(self, mock_async_db):
        scoped = AsyncUserScopedDbAdapter(mock_async_db, user_id="user-123")
        assert is_user_scoped_db(scoped)
        assert not is_user_scoped_db(mock_async_db)


class TestGetScopedUserId:
    """Test get_scoped_user_id admin bypass logic."""

    def _make_request(self, user_id=None, scopes=None, admin_scope=None, user_isolation_enabled=True):
        request = MagicMock()
        request.state = MagicMock()
        # The default for these tests is "isolation on" because they exercise
        # the legacy "is this user scoped?" branches. A dedicated test below
        # covers the off-by-default short-circuit.
    ...
    def test_off_by_default_short_circuits_to_none(self):
        """When user isolation is disabled, the helper must return None even
        if a non-admin JWT user_id is present. This is the opt-in gate that
        keeps the legacy "JWT/RBAC without DB scoping" workflow working."""
        request = self._make_request(
            user_id="user-123",
            scopes=["agents:read"],
    ...
    def test_missing_user_isolation_attr_short_circuits_to_none(self):
        """If the JWT middleware didn't set the flag at all (e.g. legacy
        deployments that pin an older agno SDK), the helper must default to
