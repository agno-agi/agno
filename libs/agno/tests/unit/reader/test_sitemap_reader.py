...
@pytest.fixture(
    params=[
        pytest.param(b"\x1f\x8b", id="truncated-header"),
        pytest.param(gzip.compress(b"<urlset/>", mtime=0)[:-1], id="truncated-trailer"),
        # A gzip header followed by a reserved DEFLATE block type.
        pytest.param(b"\x1f\x8b\x08\x00\x00\x00\x00\x00\x00\xff\x07", id="invalid-deflate"),
        pytest.param(b"\x1f\x8b" + b"\x00" * 8, id="unsupported-compression"),
    ]
)
def invalid_gzip(request):
    return request.param


@pytest.mark.parametrize("async_mode", [False, True])
def test_invalid_gzip_sitemap_tries_next_candidate(invalid_gzip, async_mode):
    routes = {
    ...
    with mock_site(routes) as requested:
        documents = (
            asyncio.run(reader.async_read("https://example.com/sitemap.xml.gz"))
            if async_mode
            else reader.read("https://example.com/sitemap.xml.gz")
        )
...
@pytest.mark.parametrize("async_mode", [False, True])
def test_invalid_gzip_index_child_preserves_healthy_pages(invalid_gzip, async_mode):
    routes = {
        "https://example.com/sitemap.xml": (
            sitemapindex_xml("https://example.com/broken.xml.gz", "https://example.com/healthy.xml"),
            "application/xml",
        ),
        "https://example.com/broken.xml.gz": (invalid_gzip, "application/gzip"),
        "https://example.com/healthy.xml": (urlset_xml("https://example.com/page-a"), "application/xml"),
        "https://example.com/page-a": (html_page("A", "Alpha content"), "text/html"),
    }
    reader = make_reader()
    with mock_site(routes):
