...
``Registry.add_tool`` records how a tool arrived and ``Registry.folded_tool_names``
...
class TestDeclarationWinsOverFold:
    ...
    def test_declaring_an_equivalent_toolkit_after_the_fold_unfolds_the_name(self):
        ...
        registry = Registry()
        registry.add_tool(search_kit(), source=ToolSource.FOLDED)
        assert registry.folded_tool_names == {"search_kit"}

        registry.add_tool(search_kit())

        assert registry.folded_tool_names == set()
    ...
    def test_declaring_the_folded_instance_itself_unfolds_the_name(self):
        ...
        registry = Registry()
        toolkit = search_kit()
        registry.add_tool(toolkit, source=ToolSource.FOLDED)
    ...
    def test_declaring_a_folded_callable_unfolds_the_name(self):
        ...
        registry = Registry()
        registry.add_tool(sample_search, source=ToolSource.FOLDED)
        assert registry.folded_tool_names == {"sample_search"}
    ...
    def test_declaring_a_folded_function_unfolds_the_name(self):
        ...
        registry.add_tool(function, source=ToolSource.FOLDED)
        assert registry.folded_tool_names == {"sample_search"}
    ...
    def test_declaring_a_same_named_toolkit_with_a_different_function_set(self):
        ...
        registry = Registry()
        registry.add_tool(Toolkit(name="search_kit", tools=[other_search]), source=ToolSource.FOLDED)
    ...
    def test_folding_a_same_named_toolkit_does_not_unlist_a_declaration(self):
        ...
        registry.add_tool(CalculatorTools(include_tools=["add", "subtract"]), source=ToolSource.FOLDED)
    ...
    def test_legacy_string_sources_decide_the_same_way_on_the_dedup_path(self):
        registry = Registry()
        registry.add_tool(search_kit(), source="folded")
        assert registry.folded_tool_names == {"search_kit"}
...
class TestFoldStaysFolded:
    ...
    def test_two_distinct_folded_toolkits_sharing_a_name_stay_folded(self):
        ...
        registry = Registry()
        registry.add_tool(Toolkit(name="search_kit", tools=[sample_search]), source=ToolSource.FOLDED)
        registry.add_tool(Toolkit(name="search_kit", tools=[other_search]), source=ToolSource.FOLDED)
    ...
    def test_three_distinct_folded_toolkits_sharing_a_name_stay_folded(self):
        ...
        registry.add_tool(Toolkit(name="search_kit", tools=[sample_search]), source=ToolSource.FOLDED)
        registry.add_tool(Toolkit(name="search_kit", tools=[other_search]), source=ToolSource.FOLDED)
        registry.add_tool(Toolkit(name="search_kit", tools=[sample_search, other_search]), source=ToolSource.FOLDED)

        assert registry.folded_tool_names == {"search_kit"}
        assert len(registry.tools) == 3

    def test_an_unrecognised_source_neither_folds_nor_unfolds(self):
    ...
    def test_a_folded_callable_sharing_a_folded_toolkits_name_stays_folded(self):
        registry = Registry()
        registry.add_tool(Toolkit(name="sample_search", tools=[other_search]), source=ToolSource.FOLDED)
        registry.add_tool(sample_search, source=ToolSource.FOLDED)
    ...
    def test_a_tool_without_a_usable_name_is_never_marked(self):
        ...
        registry = Registry()
        registry.add_tool(partial(sample_search, "q"), source=ToolSource.FOLDED)
