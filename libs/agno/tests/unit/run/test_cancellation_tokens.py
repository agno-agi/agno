...
def _redis_manager(enable_token_cleanup: bool = True):
    ...
    return RedisRunCancellationManager(
        ...
        enable_token_cleanup=enable_token_cleanup,
    )
...
class TestRedisTokens:
    ...
    @pytest.mark.asyncio
    async def test_token_cleanup_is_opt_in_and_defaults_off(self):
        """Round-6 P1: with any pre-token replica in the fleet, token-scoped
        cleanup is unsafe (see next test) - so it ships DISABLED. Token reads
        return None and callers skip cleanup entirely."""
        m = _redis_manager(enable_token_cleanup=False)
        await m.acancel_run("r1")
        assert await m.ais_cancelled("r1")
        assert await m.aget_cancellation_token("r1") is None, "disabled: no token, callers skip cleanup"
        assert await m.acleanup_run_if_token("r1", "anything") is False
        assert await m.ais_cancelled("r1")

    @pytest.mark.asyncio
    async def test_mixed_writer_sequence_cannot_erase_old_replica_cancel(self):
        """The reviewer's exact sequence: new replica cancels (token A);
        an OLD replica then issues a NEWER cancel, rewriting only the legacy
        intent key and leaving token A behind. A token-scoped cleanup holding
        A would match the stale sidecar and erase the old replica's newer
        cancel - which is exactly why cleanup ships disabled. With the
        default configuration the newer cancel survives."""
        m = _redis_manager(enable_token_cleanup=False)
        await m.acancel_run("r1")  # new-replica cancel: intent "1" + sidecar token A
        # Old replica's NEWER cancel: rewrites ONLY the legacy intent key
        m.redis_client.set("test:cancel:r1", "1")
        assert await m.aget_cancellation_token("r1") is None
        assert await m.acleanup_run_if_token("r1", "stale-token-A") is False
        assert await m.ais_cancelled("r1"), "the old replica's newer cancel must survive"

    @pytest.mark.asyncio
    async def test_sidecar_shares_the_cluster_slot_and_hides_from_scans(self):
        """Round-6 P1+P2: the sidecar is hash-tagged ("{intent-key}:token") so
        both keys share a Redis Cluster hash slot (multi-key ops and the
        WATCH transaction stay legal), and the brace prefix keeps sidecars
        out of the key_prefix* scan - get_active_runs must not report fake
        ":token" runs."""
        m = _redis_manager()
        assert m._get_token_key("test:cancel:r1") == "{test:cancel:r1}:token"
        await m.aregister_run("r1")
        await m.acancel_run("r2")
        active = m.get_active_runs()
        assert set(active.keys()) == {"r1", "r2"}, f"sidecars must not appear as runs: {active}"
