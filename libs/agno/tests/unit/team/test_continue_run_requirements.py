...
from agno.team._default_tools import _propagate_member_pause
...
# ---------------------------------------------------------------------------
# Issue fix: UUID vs name-based member ID in chained HITL propagation
# ---------------------------------------------------------------------------


def test_propagate_still_paused_uses_routable_id_not_uuid():
    """Verify chained HITL re-propagation uses the name-based routable ID from existing
    requirements rather than the UUID agent_id from the result, so that routing succeeds."""
    from agno.team.team import Team

    member_agent = Agent(name="Weather Agent")
    top_team = Team(name="TopTeam", members=[member_agent], telemetry=False)
    routable_id = get_member_id(member_agent)  # e.g. "weather-agent"

    # Simulate: first pause propagated a resolved requirement with the routable ID,
    # and the member pauses again with a NEW requirement (member_agent_id=None).
    resolved_req = _make_requirement("tc_resolved", confirmed=True)
    resolved_req.member_agent_id = routable_id
    resolved_req.member_agent_name = member_agent.name
    resolved_req.member_run_id = "agent_run_1"

    new_req = _make_requirement("tc_new_pause")
    # new_req has no member context (as happens for fresh pauses from the agent)

    paused_member = RunOutput(
        run_id="agent_run_1",
        agent_id="aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee",  # UUID — NOT routable
        agent_name="Weather Agent",
        status=RunStatus.paused,
        requirements=[resolved_req, new_req],
    )

    team_run = TeamRunOutput(run_id="team_run")
    _run._propagate_still_paused_member_requirements(team_run, {"Weather Agent": paused_member})

    # Only the unresolved requirement should be propagated
    assert len(team_run.requirements) == 1
    propagated = team_run.requirements[0]
    assert propagated.tool_execution.tool_call_id == "tc_new_pause"
    # Should use the routable name-based ID, NOT the UUID
    assert propagated.member_agent_id == routable_id
    assert propagated.member_agent_name == member_agent.name

    # Verify it can actually be routed
    found = top_team._find_member_by_id(propagated.member_agent_id)
    assert found is not None


def test_propagate_still_paused_skips_resolved_requirements():
    """Verify resolved requirements are not re-propagated in chained HITL flows."""
    resolved_req = _make_requirement("tc_done", confirmed=True)
    resolved_req.member_agent_id = "member-1"
    resolved_req.member_agent_name = "MemberOne"
    resolved_req.member_run_id = "run_1"

    unresolved_req = _make_requirement("tc_pending")
    # Leave unresolved — no member context

    paused_member = RunOutput(
        run_id="run_1",
        agent_id="member-1",
        agent_name="MemberOne",
        status=RunStatus.paused,
        requirements=[resolved_req, unresolved_req],
    )

    team_run = TeamRunOutput(run_id="team_run")
    _run._propagate_still_paused_member_requirements(team_run, {"MemberOne": paused_member})

    assert len(team_run.requirements) == 1
    assert team_run.requirements[0].tool_execution.tool_call_id == "tc_pending"


# ---------------------------------------------------------------------------
# Issue fix: needs_confirmation with tool_execution.confirmed=False
# ---------------------------------------------------------------------------


def test_needs_confirmation_false_when_tool_confirmed_is_false():
    """Verify needs_confirmation returns False when tool_execution.confirmed is explicitly False,
    even if RunRequirement.confirmation is None (direct manipulation scenario)."""
    te = ToolExecution(
        tool_call_id="tc_rejected_direct",
    ...
    # confirmation is None (not set via reject()), but tool_execution.confirmed is False
    assert req.confirmation is None
    assert req.tool_execution.confirmed is False
    # Should NOT still need confirmation
