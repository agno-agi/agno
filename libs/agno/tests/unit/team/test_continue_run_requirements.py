...
class TestPausedMemberRunPersistence:
    """Verify that when a member pauses during routing, its RunOutput is
    persisted to session.runs so subsequent continue_run calls can find it
    after session reload.

    This is the fix for GitHub issue #8925: on 2nd+ HITL round, the paused
    member run was not findable because it was never upserted to session.
    """
    ...
    def test_async_routing_persists_paused_member_run(self):
        """When member.acontinue_run returns a paused response, it should be upserted."""
        from agno.team._run import _aroute_requirements_to_members
        ...
        member = MagicMock()
        member.name = "Member 1"
        paused_member_response = MagicMock(is_paused=True, content=None, run_id="member-run-1")
        paused_member_response.requirements = [_make_requirement(requires_user_input=True)]
        member.acontinue_run = AsyncMock(return_value=paused_member_response)
        ...
        async def _exercise():
            with (
                ...
                patch("agno.team._tools._propagate_member_pause") as mock_propagate,
            ):
                await _aroute_requirements_to_members(
                    team, run_response=run_response, session=session, run_context=None
                )

                mock_propagate.assert_called_once()
                session.upsert_run.assert_called_once_with(paused_member_response)

        asyncio.run(_exercise())

    def test_async_routing_persists_completed_member_run(self):
        """When member completes (not paused), it should also be upserted."""
        from agno.team._run import _aroute_requirements_to_members
    ...
    def test_sync_routing_persists_paused_member_run(self):
        """Sync variant: paused member run should be upserted."""
        from agno.team._run import _route_requirements_to_members
    ...
    def test_async_streaming_routing_persists_paused_member_run(self):
        """Async streaming variant: paused member run should be upserted."""
        ...
        # Create a real RunOutput so isinstance() check passes in the streaming helper
        # Set status to paused so is_paused property returns True
    ...
    def test_sync_streaming_routing_persists_paused_member_run(self):
        """Sync streaming variant: paused member run should be upserted."""
        from agno.run.agent import RunOutput
        from agno.run.base import RunStatus
        from agno.team._run import _route_requirements_to_members_stream
