...
def test_dynamic_team_prompt_fields_sync():
    ...
    team = Team(
        ...
        name=lambda session_state: f"{session_state['tenant']} team",
        description=lambda run_context: f"Coordinates for {run_context.metadata['user_id']}",
        role=lambda: "Lead analysts",
    ...
    assert "Your name is: acme team." in content
    assert "<description>\nCoordinates for user-1\n</description>" in content
    assert "<your_role>\nLead analysts\n</your_role>" in content
    assert "<expected_output>\nSummary for acme\n</expected_output>" in content
    assert "<additional_context>\ntenant=acme\n</additional_context>" in content


@pytest.mark.asyncio
async def test_dynamic_team_prompt_fields_async():
    async def name_fn(session_state):
        return f"{session_state['tenant']} team"

    async def description_fn(run_context):
        return f"Coordinates for {run_context.metadata['user_id']}"

    async def role_fn():
        return "Lead analysts"
...
def test_dynamic_team_prompt_fields_validate_string_type():
    run_context = RunContext(run_id="run-1", session_id="session-1")
    team = Team(
        members=[],
        model=OpenAIChat(id="gpt-4o-mini"),
        role=lambda: ["not", "a", "string"],
    )

    with pytest.raises(Exception, match="role must resolve to a string"):
        team.get_system_message(
            session=TeamSession(session_id="session-1"),
            run_context=run_context,
        )
