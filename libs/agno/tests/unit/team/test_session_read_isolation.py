...
def test_read_session_passes_user_id_to_db():
    ...
    _read_session(team, session_id="s1", user_id="alice")
...
async def test_aread_session_passes_user_id_to_db():
    ...
    await _aread_session(team, session_id="s1", user_id="alice")
...
def test_read_or_create_session_passes_user_id_through():
    ...
    result = _read_or_create_session(team, session_id="s1", user_id="bob")
    assert result.user_id == "bob"
    assert result.session_id == "s1"

    result = _read_or_create_session(team, session_id="s1", user_id="alice")
...
async def test_aread_or_create_session_passes_user_id_async_branch():
    ...
    with patch("agno.team._init._has_async_db", return_value=True):
        result = await _aread_or_create_session(team, session_id="s1", user_id="bob")
    assert result.user_id == "bob"
...
def test_cached_session_not_returned_to_wrong_user():
    ...
    alice_result = _read_or_create_session(team, session_id="s1", user_id="alice")
    assert alice_result.user_id == "alice"
    assert team._cached_session is not None

    bob_result = _read_or_create_session(team, session_id="s1", user_id="bob")
...
def test_cached_session_returned_when_user_id_none():
    ...
    result1 = _read_or_create_session(team, session_id="s1", user_id=None)
    assert team._cached_session is not None

    result2 = _read_or_create_session(team, session_id="s1", user_id=None)
