...
def test_function_process_entrypoint_with_user_input():
    ...
    assert func.user_input_schema[0].field_type == str
    assert func.user_input_schema[1].name == "param2"
    assert func.user_input_schema[1].field_type == int
...
def test_tool_decorator_with_user_input():
    ...
    assert user_input_func.user_input_schema[0].field_type == str
    assert user_input_func.user_input_schema[1].name == "param2"
    assert user_input_func.user_input_schema[1].field_type == int
...
def test_tool_decorator_exclusive_flags():
    ...
    with pytest.raises(ValueError, match="Only one of 'requires_user_input', 'requires_confirmation', or 'external_execution' can be set to True"):
