"""Unit tests for AuthConfig dataclass and scope aggregation."""
...
class TestAuthConfigInit:
    """Tests for AuthConfig initialization and env var defaults."""
    ...
    def test_oauth_flow_options(self):
        auth = AuthConfig(
            access_type="online",
...
class TestScopeAggregation:
    """Tests for scope aggregation via register_scopes."""

    def test_register_scopes_single_toolkit(self):
        auth = AuthConfig()
    ...
    def test_register_scopes_multiple_toolkits(self):
        auth = AuthConfig()
        gmail_scopes = [
    ...
    def test_register_scopes_deduplicates(self):
        auth = AuthConfig()
    ...
    def test_scopes_property_returns_list(self):
        auth = AuthConfig()
...
class TestCredsProperty:
    """Tests for creds property getter/setter."""

    def test_creds_default_none(self):
        auth = AuthConfig()
        assert auth.creds is None

    def test_creds_setter(self):
        auth = AuthConfig()
    ...
    def test_creds_shared_across_reads(self):
        auth = AuthConfig()
        mock_creds = MagicMock()
...
class TestSharedAuthPattern:
    """Tests for shared auth= parameter across multiple toolkits."""
    ...
    def test_shared_auth_aggregates_scopes(self, mock_creds):
        auth = AuthConfig()
    ...
    def test_shared_auth_same_object(self, mock_creds):
        auth = AuthConfig()
    ...
    def test_no_shared_auth_creates_separate_configs(self, mock_creds):
        with patch("googleapiclient.discovery.build"):
    ...
    def test_shared_auth_creds_propagate(self, mock_creds):
        auth = AuthConfig()
        auth.creds = mock_creds
    ...
    def test_toolkit_auto_creates_auth_config(self, mock_creds):
        with patch("googleapiclient.discovery.build"):
            gmail = GmailTools(creds=mock_creds)
    ...
    def test_toolkit_params_passed_to_auto_auth_config(self, mock_creds):
        with patch("googleapiclient.discovery.build"):
            gmail = GmailTools(
                creds=mock_creds,
...
class TestAuthConfigWithDb:
    """Tests for AuthConfig with DB token storage."""

    def test_db_param_stored(self):
        mock_db = MagicMock()
        auth = AuthConfig(db=mock_db)

        assert auth.db is mock_db

    def test_db_default_none(self):
        auth = AuthConfig()
        assert auth.db is None

    def test_token_encryption_key_from_env(self, monkeypatch):
        monkeypatch.setenv("GOOGLE_TOKEN_ENCRYPTION_KEY", "my-secret-key")
        auth = AuthConfig()

        assert auth.token_encryption_key == "my-secret-key"

    def test_token_encryption_key_explicit(self, monkeypatch):
...
class TestAuthConfigRepr:
    """Tests for AuthConfig repr and internal fields."""

    def test_scopes_not_in_repr(self):
        auth = AuthConfig()
        auth.register_scopes(["scope1", "scope2"])

        repr_str = repr(auth)

        # Internal fields have repr=False, so scope values shouldn't appear
...
class TestMultiToolkitScenarios:
    """Integration-style tests for multi-toolkit scenarios."""
    ...
    def test_gmail_calendar_drive_shared_auth(self, mock_creds):
        auth = AuthConfig()
    ...
    def test_scope_order_independent(self, mock_creds):
        auth1 = AuthConfig()
        auth2 = AuthConfig()
