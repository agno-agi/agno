...
def test_authenticate_google_produces_verifiable_state(google_auth):
    resp = json.loads(google_auth.authenticate_google(services=["gmail"], run_context=_fake_run_context("alice")))
...
def test_env_var_fallback(monkeypatch):
    ...
    ga.register_service("gmail", ["https://www.googleapis.com/auth/gmail.readonly"])
    resp = json.loads(ga.authenticate_google(services=["gmail"], run_context=_fake_run_context("alice")))
    state = _state_from_url(resp["url"])
...
def test_fabricated_state_rejected(google_auth):
    ...
    result = google_auth.handle_oauth_callback(code="unused", state=forged)
...
def test_tampered_state_rejected(google_auth):
    ...
    result = google_auth.handle_oauth_callback(code="unused", state=tampered)
    assert "error" in result
    assert "Invalid state" in result["error"]
...
def test_expired_state_rejected(google_auth):
    ...
    result = google_auth.handle_oauth_callback(code="unused", state=expired)
    assert "error" in result
    assert "Invalid state" in result["error"]
...
def test_authenticate_google_without_state_secret_errors(monkeypatch):
    ...
    ga = GoogleAuth(client_id="id")
    ga.register_service("gmail", ["https://www.googleapis.com/auth/gmail.readonly"])
    resp = json.loads(ga.authenticate_google(services=["gmail"]))
