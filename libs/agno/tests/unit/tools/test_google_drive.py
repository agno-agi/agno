...
def mock_service():
    ...
    files.list.return_value.execute.return_value = {
        'files': [
            {'id': '1', 'name': 'TestFile', 'mimeType': 'text/plain', 'modifiedTime': '2025-09-17T12:00:00Z'}
        ]
    }
    return service
...
def drive_tools(mock_creds, mock_service):
    with patch('agno.tools.google_drive.build') as mock_build, \
...
def test_upload_file_success(tmp_path, drive_tools):
    ...
    mock_create.return_value.execute.return_value = {
        'id': '123', 'name': 'test_upload.txt', 'mimeType': 'text/plain', 'modifiedTime': '2025-09-17T12:00:00Z'
    }
...
def test_download_file_success(tmp_path, drive_tools):
    ...
    mock_downloader.next_chunk.side_effect = [(MagicMock(progress=lambda: 0.5), False), (MagicMock(progress=lambda: 1.0), True)]
