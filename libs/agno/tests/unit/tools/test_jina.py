...
def jina_tools_basic():
    ...
    return JinaReaderTools(
        api_key="test_api_key", read_url=True, search_query=False, timeout=30, max_content_length=5000
    )
...
def jina_tools_search():
    ...
    return JinaReaderTools(
        api_key="test_api_key", read_url=False, search_query=True, timeout=30, max_content_length=5000
    )
...
def jina_tools_full():
    ...
    return JinaReaderTools(
        api_key="test_api_key", read_url=True, search_query=True, timeout=30, max_content_length=5000
    )
...
def jina_tools_no_api_key():
    ...
    return JinaReaderTools(api_key=None, read_url=True, search_query=True, timeout=30, max_content_length=5000)


def test_jina_reader_tools_config():
    """Test JinaReaderToolsConfig initialization."""
    config = JinaReaderToolsConfig(api_key="test_key", max_content_length=8000, timeout=60)
...
def test_read_url_success(mock_client, jina_tools_basic):
    ...
    mock_response = Mock()
    mock_response.json.return_value = {"code": 200, "data": {"content": "This is the content from the URL."}}
...
def test_read_url_api_error(mock_client, jina_tools_basic):
    ...
    mock_response = Mock()
    mock_response.json.return_value = {"code": 400, "message": "Invalid URL provided"}
    mock_response.raise_for_status.return_value = None
...
def test_read_url_http_error(mock_client, jina_tools_basic):
    ...
    mock_client_instance = Mock()
    mock_client_instance.post.side_effect = httpx.HTTPStatusError("Request failed", request=Mock(), response=Mock())
    mock_client.return_value.__enter__.return_value = mock_client_instance
...
def test_search_query_success(mock_client, jina_tools_search):
    ...
    mock_response.json.return_value = {
        "code": 200,
        "data": [
            {"title": "Test Result 1", "url": "https://example1.com", "content": "Content for result 1" + "x" * 500},
            {"title": "Test Result 2", "url": "https://example2.com", "content": "Content for result 2"},
        ],
    }
    mock_response.raise_for_status.return_value = None
...
def test_search_query_api_error(mock_client, jina_tools_search):
    ...
    mock_response = Mock()
    mock_response.json.return_value = {"code": 400, "message": "Invalid search query"}
    mock_response.raise_for_status.return_value = None
...
def test_search_query_limits_results(mock_client, jina_tools_search):
    ...
    for i in range(7):
        mock_results.append(
            {"title": f"Result {i + 1}", "url": f"https://example{i + 1}.com", "content": f"Content {i + 1}"}
        )

    mock_response = Mock()
    mock_response.json.return_value = {"code": 200, "data": mock_results}
    mock_response.raise_for_status.return_value = None
