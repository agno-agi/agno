...
async def test_multimcp_connect_merges_init_headers_when_streamable_http_headers_default_to_none():
    ...
    with (
        patch(
            "agno.tools.mcp.multi_mcp._streamable_http_connection",
            return_value=(_AsyncContextManager(("read", "write")), 30.0),
        ) as streamable_http_mock,
...
async def test_multimcp_connect_failure_closes_partially_entered_stack():
    ...
    first_transport_context = _SucceedingAenterContext(("read-1", "write-1"))
    ...
    with (
        patch(
            "agno.tools.mcp.multi_mcp._streamable_http_connection",
            side_effect=[(first_transport_context, 30.0), (second_transport_context, 30.0)],
        ),
