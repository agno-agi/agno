...
def notion_tools():
    ...
    return NotionTools(
        ...
        database_id="28fee27f-d912-8039-b3f8-f47cb7ade7cb"
    )
        ...
        "results": [
            {
                ...
                "properties": {
                    "Name": {
                        "title": [{"text": {"content": "Travel Collection"}}]
                    },
                    "Tag": {
                        "select": {"name": "travel"}
                    }
            ...
            },
            {
                ...
                "properties": {
                    "Name": {
                        "title": [{"text": {"content": "Another Travel Page"}}]
                    },
                    "Tag": {
                        "select": {"name": "travel"}
                    }
...
def test_update_page_success(notion_tools, mock_update_page_response):
    ...
    with patch.object(notion_tools.client.blocks.children, 'append', return_value=mock_update_page_response):
        result = notion_tools.update_page(
            page_id="page-123",
...
def test_notion_tools_with_all_methods(notion_tools, mock_create_page_response, 
                                       mock_update_page_response, mock_search_pages_response):
...
def test_database_id_formatting():
    ...
    tools2 = NotionTools(
        api_key="test_key",
        database_id="28fee27fd9128039b3f8f47cb7ade7cb"
    )
