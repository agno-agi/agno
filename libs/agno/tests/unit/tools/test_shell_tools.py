...
def test_shell_tools_not_registered_by_default():
    """run_shell_command is NOT registered by default (security: RCE sink)."""
    tools = ShellTools()
    assert "run_shell_command" not in tools.functions


def test_shell_tools_registered_when_enabled():
    """run_shell_command is registered when explicitly enabled."""
    tools = ShellTools(run_shell_command=True)
    assert "run_shell_command" in tools.functions


def test_enabled_mode_runs_command():
    """When enabled, executes commands."""
