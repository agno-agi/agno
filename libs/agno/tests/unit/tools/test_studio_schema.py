...
@pytest.mark.parametrize(
    "create_request",
    [
        AgentCreate(name="Agent", instructions="Work.", if_exists="return_existing"),
        TeamCreate(
            name="Team",
        ...
        WorkflowCreate(
            name="Workflow",
            steps=[AgentWorkflowStep(kind="agent", name="Run", component_id="member")],
            if_exists="return_existing",
        ),
    ],
)
def test_create_requests_carry_the_idempotency_policy(create_request):
    assert create_request.if_exists == "return_existing"

    with pytest.raises(ValidationError):
        type(create_request).model_validate({**create_request.model_dump(), "if_exists": "overwrite"})
