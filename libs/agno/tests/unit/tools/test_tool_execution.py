...
def code_mode_callables():
    return _ToolExecutionKit(tools=[search_items, get_item_details])


@pytest.fixture
def code_mode_toolkit():
    return _ToolExecutionKit(tools=[SimpleToolkit()])


@pytest.fixture
def code_mode_mixed():
    return _ToolExecutionKit(tools=[SimpleToolkit(), search_items])
...
class TestInitialization:
    ...
    def test_accepts_function_objects(self):
        ...
        cm = _ToolExecutionKit(tools=[func])
...
class TestStubGeneration:
    ...
    def test_stubs_exclude_framework_params(self):
        cm = _ToolExecutionKit(tools=[tool_needing_agent])
...
class TestFrameworkInjection:
    ...
    def test_run_context_forwarded_when_requested(self):
        cm = _ToolExecutionKit(tools=[tool_needing_run_context])
        mock_ctx = Mock()
        run_code_func = cm.functions.get("run_code")
        if run_code_func:
...
class TestAdditionalModules:
    def test_additional_module_available(self):
        ...
        cm = _ToolExecutionKit(tools=[search_items], additional_modules={"datetime": datetime})
...
class TestDiscoveryMode:
    def test_auto_enables_above_threshold(self):
        cm = _ToolExecutionKit(tools=_make_many_callables(20), discovery_threshold=15)
        assert cm.discovery_enabled is True

    def test_auto_disables_below_threshold(self):
        cm = _ToolExecutionKit(tools=_make_many_callables(5), discovery_threshold=15)
    ...
    def test_explicit_true_with_few_tools(self):
        cm = _ToolExecutionKit(tools=[search_items], discovery=True)
        assert cm.discovery_enabled is True

    def test_explicit_false_with_many_tools(self):
        cm = _ToolExecutionKit(tools=_make_many_callables(20), discovery=False)
        assert cm.discovery_enabled is False

    def test_invalid_discovery_value_raises(self):
        with pytest.raises(ValueError, match="discovery must be"):
            _ToolExecutionKit(tools=[search_items], discovery="always")
    ...
    def test_no_search_tools_when_disabled(self):
        cm = _ToolExecutionKit(tools=[search_items], discovery=False)
...
class TestSearchTools:
    def test_search_by_name(self):
        cm = _ToolExecutionKit(tools=[search_items, get_item_details], discovery=True)
    ...
    def test_search_caps_at_10(self):
        cm = _ToolExecutionKit(tools=_make_many_callables(20), discovery=True)
...
class TestDiscoveryExecution:
    ...
    def test_code_execution_identical_to_standard(self):
        cm_discovery = _ToolExecutionKit(tools=[search_items], discovery=True)
        cm_standard = _ToolExecutionKit(tools=[search_items], discovery=False)
...
class Test_ToolExecutionKitlInit:
    def test_code_model_disables_discovery(self):
        mock_model = Mock()
        cm = _ToolExecutionKit(tools=_make_many_callables(20), code_model=mock_model)
        assert cm.discovery_enabled is False
    ...
    def test_code_model_generates_catalog(self):
        mock_model = Mock()
        cm = _ToolExecutionKit(tools=[search_items, get_item_details], code_model=mock_model)
        assert cm.catalog
...
class Test_ToolExecutionKitlExecution:
    ...
    def test_code_model_no_retry_on_tool_error(self):
        ...
        cm = _ToolExecutionKit(tools=[failing_tool], code_model=mock_model)
    ...
    def test_code_model_max_retries_exceeded(self):
        ...
        cm = _ToolExecutionKit(tools=[search_items], code_model=mock_model, max_code_retries=2)
...
class Test_ToolExecutionKitlExtractCode:
    def test_extract_python_fence(self):
        text = '```python\nresult = "hello"\n```'
        assert _ToolExecutionKit._extract_code_block(text) == 'result = "hello"'
...
class Test_ToolExecutionKitlAsync:
    @pytest.mark.asyncio
...
class Test_ToolExecutionKitlMetrics:
    ...
    def test_fc_run_response_survives_retries(self):
        ...
        cm = _ToolExecutionKit(tools=[search_items], code_model=mock_model, max_code_retries=3)
    ...
    def test_model_type_set_to_code_model(self):
        """code_model gets ModelType.TOOL_EXECUTION_MODEL set in __init__."""
        from agno.metrics import ModelType

        mock_model = Mock()
        _ToolExecutionKit(tools=[search_items], code_model=mock_model)
        assert mock_model.model_type == ModelType.TOOL_EXECUTION_MODEL
