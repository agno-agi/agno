...
class TestTrafilaturaToolsInitialization:
    ...
    def test_initialization_default(self, trafilatura_tools):
        ...
        # Check registered functions - safe tools enabled by default
        ...
        # Token-heavy tools disabled by default
        assert "convert_html" not in function_names
        assert "scrape_batch" not in function_names
    ...
    def test_initialization_custom(self, custom_trafilatura_tools):
        ...
        # Check all functions are registered when using custom settings
        function_names = [func.name for func in tools.functions.values()]
        assert "scrape" in function_names
        assert "get_metadata" in function_names
        # Token-heavy tools still disabled unless explicitly enabled
        assert "convert_html" not in function_names
        assert "scrape_batch" not in function_names

    def test_initialization_include_tools(self, mock_trafilatura_modules):
        """Test initialization with include_tools parameter."""
        # Enable tools first, then filter with include_tools
        tools = TrafilaturaTools(scrape_batch=True, include_tools=["scrape", "scrape_batch"])
        function_names = [func.name for func in tools.functions.values()]
        assert "scrape" in function_names
    ...
    def test_initialization_exclude_tools(self, mock_trafilatura_modules):
        ...
        tools = TrafilaturaTools(exclude_tools=["scrape"])
        function_names = [func.name for func in tools.functions.values()]
        assert "get_metadata" in function_names
        assert "scrape" not in function_names
    ...
    def test_initialization_without_spider(self, mock_trafilatura_modules):
        ...
        function_names = [func.name for func in tools.functions.values()]
        # crawl should not be in functions when spider is not available
        assert "crawl" not in function_names


class TestExtractTextMethod:
    """Test class for scrape method."""

    def test_scrape_success(self, trafilatura_tools, mock_trafilatura_modules):
    ...
    def test_scrape_fetch_failure(self, trafilatura_tools, mock_trafilatura_modules):
        """Test scrape when fetch_url fails."""
        # Setup mocks
    ...
    def test_scrape_with_custom_params(self, trafilatura_tools, mock_trafilatura_modules):
        """Test scrape with custom output format."""
        # Setup mocks
        mock_trafilatura_modules["fetch_url"].return_value = "<html><body>Test content</body></html>"
    ...
    def test_scrape_exception_handling(self, trafilatura_tools, mock_trafilatura_modules):
...
class TestExtractMetadataOnlyMethod:
    """Test class for get_metadata method."""

    def test_get_metadata_success(self, trafilatura_tools, mock_trafilatura_modules):
    ...
    def test_get_metadata_fetch_failure(self, trafilatura_tools, mock_trafilatura_modules):
        """Test get_metadata when fetch fails."""
        # Setup mocks
        mock_trafilatura_modules["fetch_url"].return_value = None
...
class TestCrawlWebsiteMethod:
    """Test class for crawl method."""

    @patch("agno.tools.trafilatura.SPIDER_AVAILABLE", True)
    def test_crawl_success(self, trafilatura_tools, mock_trafilatura_modules):
    ...
    @patch("agno.tools.trafilatura.SPIDER_AVAILABLE", False)
    def test_crawl_spider_unavailable(self, trafilatura_tools, mock_trafilatura_modules):
        """Test crawl when spider is not available."""
        # Execute
        result = trafilatura_tools.crawl("https://example.com")
    ...
    def test_crawl_with_content_extraction(self, trafilatura_tools, mock_trafilatura_modules):
        """Test crawl with content extraction enabled."""
        # Setup mocks
...
class TestHtmlToTextMethod:
    """Test class for convert_html method."""

    def test_convert_html_success(self, trafilatura_tools, mock_trafilatura_modules):
    ...
    def test_convert_html_with_clean_false(self, trafilatura_tools, mock_trafilatura_modules):
    ...
    def test_convert_html_empty_result(self, trafilatura_tools, mock_trafilatura_modules):
...
class TestExtractBatchMethod:
    """Test class for scrape_batch method."""

    def test_scrape_batch_success(self, trafilatura_tools, mock_trafilatura_modules):
    ...
    def test_scrape_batch_partial_failure(self, trafilatura_tools, mock_trafilatura_modules):
...
class TestToolkitIntegration:
    ...
    def test_toolkit_registration_default(self, trafilatura_tools):
        ...
        function_names = [func.name for func in trafilatura_tools.functions.values()]
        # Default: safe tools enabled, token-heavy tools disabled
        assert "scrape" in function_names
        assert "get_metadata" in function_names
        assert "convert_html" not in function_names
        assert "scrape_batch" not in function_names
    ...
    def test_toolkit_registration_custom(self, custom_trafilatura_tools):
        ...
        # Custom settings don't auto-enable all tools
        assert "scrape" in function_names
        assert "get_metadata" in function_names
