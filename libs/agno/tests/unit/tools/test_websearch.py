...
def test_disable_both():
    ...
    with patch("agno.tools.websearch.DDGS"):
        tools = WebSearchTools(search_web=False, search_news=False)
...
def test_search_web_basic(mock_ddgs):
...
def test_search_web_with_timelimit(mock_ddgs):
...
def test_search_web_with_region(mock_ddgs):
...
def test_search_web_with_modifier(mock_ddgs):
...
def test_search_web_with_all_params(mock_ddgs):
    ...
    result = tools.search_web("asyncio")
    result_data = json.loads(result)
...
def test_search_web_returns_json(mock_ddgs):
    """Test that search_web returns valid JSON."""
    mock_instance, _ = mock_ddgs
    mock_instance.text.return_value = [
...
def test_search_web_preserves_unicode_characters(mock_ddgs):
    """Test that search_web preserves non-ASCII characters in JSON output."""
    mock_instance, _ = mock_ddgs
    mock_instance.text.return_value = [
...
def test_search_web_empty_results(mock_ddgs):
    """Test search_web with empty results."""
    mock_instance, _ = mock_ddgs
    mock_instance.text.return_value = []

    tools = WebSearchTools()
    result = tools.search_web("nonexistent query")
