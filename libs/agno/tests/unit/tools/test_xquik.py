...
def test_get_trends_uses_current_route(xquik_tools: XquikTools, monkeypatch: pytest.MonkeyPatch) -> None:
    requests = _install_transport(
        monkeypatch,
        lambda request: _response(request, {"count": 1, "trends": [{"name": "#AI"}], "woeid": 1}),
    )

    result = json.loads(xquik_tools.get_trends(count=100))

    assert requests[0].url.path == "/api/v1/x/trends"
    assert requests[0].url.params["count"] == "50"
    assert result == {"trends": [{"name": "#AI"}], "count": 1, "woeid": 1}
...
async def test_async_get_trends(xquik_tools: XquikTools, monkeypatch: pytest.MonkeyPatch) -> None:
    _install_transport(
        monkeypatch,
        lambda request: _response(request, {"count": 0, "trends": [], "woeid": 23424969}),
    )

    result = json.loads(await xquik_tools.aget_trends(woeid=23424969))

    assert result == {"trends": [], "count": 0, "woeid": 23424969}


@pytest.mark.parametrize(
    "payload",
    [
        {"count": 1, "trends": ["invalid"], "woeid": 1},
        {"total": 0, "trends": [], "woeid": 1},
    ],
)
def test_get_trends_rejects_invalid_response(
    xquik_tools: XquikTools, monkeypatch: pytest.MonkeyPatch, payload: Dict[str, Any]
) -> None:
    _install_transport(monkeypatch, lambda request: _response(request, payload))
