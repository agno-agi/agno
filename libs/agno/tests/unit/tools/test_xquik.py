...
def xquik_tools():
    ...
    with patch.dict("os.environ", {"XQUIK_API_KEY": "xq_test_key"}):
        return XquikTools()
...
class TestInit:
    ...
    def test_init_with_env_var(self):
        ...
        with patch.dict("os.environ", {"XQUIK_API_KEY": "xq_test_key"}):
            tools = XquikTools()
            assert tools.api_key == "xq_test_key"

    def test_init_with_explicit_key(self):
        """Test initialization with explicit API key."""
        tools = XquikTools(api_key="xq_explicit_key")
        assert tools.api_key == "xq_explicit_key"
...
class TestSearchPosts:
    ...
    def test_request_uses_api_key_header(self, xquik_tools):
        ...
        assert request.get_header("X-api-key") == "xq_test_key"
        assert request.headers["Accept"] == "application/json"
        assert request.headers["User-agent"] == "agno-xquik"

    def test_request_does_not_forward_api_key_on_redirect(self, xquik_tools):
        """Test that redirects do not receive the API key."""
        with patch("urllib.request.urlopen", return_value=_mock_urlopen({"tweets": []})) as mock_open:
            xquik_tools.search_posts("hello")
            request = mock_open.call_args[0][0]
    ...
    def test_search_forwards_cursor(self, xquik_tools):
        """Test that callers can continue a paginated search."""
        mock_data = {"tweets": [], "has_next_page": False, "next_cursor": ""}

        with patch("urllib.request.urlopen", return_value=_mock_urlopen(mock_data)) as mock_open:
            result = json.loads(xquik_tools.search_posts("test", cursor="cursor-1"))
            call_url = mock_open.call_args[0][0].full_url

        assert "cursor=cursor-1" in call_url
    ...
    def test_search_handles_missing_author(self, xquik_tools):
        """Test that a nullable author does not discard an otherwise valid post."""
        mock_data = {
            "tweets": [
                {
                    "id": "123456",
                    "text": "Unavailable author",
                    "author": None,
                }
            ]
        }
        ...
        assert result["posts"][0]["url"] == "https://x.com/unknown/status/123456"
...
class TestGetTweet:
    ...
    def test_get_tweet_rejects_invalid_response(self, xquik_tools):
        """Return a clear error when the tweet envelope is malformed."""
        with patch("urllib.request.urlopen", return_value=_mock_urlopen({"tweet": None})):
            result = json.loads(xquik_tools.get_tweet("987654"))
