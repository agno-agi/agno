...
def test_metadata_flattening(chroma_db):
    ...
    for key, value in flattened.items():
        assert isinstance(value, (str, int, float, bool)), f"Value {value} of type {type(value)} is not ChromaDB compatible"
...
def test_update_metadata_with_complex_data(chroma_db):
    ...
    complex_update_metadata = {
        "doc_type": {
            ...
            "subcategory": "thai_cooking"
        },
        "filters": {
            "cuisine": "thai",
            "difficulty": "medium",
            "ingredients": ["coconut", "lemongrass", "chilies"]
        },
        "ratings": [4, 5, 4, 5, 3],
        "is_featured": True,
        "price": 29.99
