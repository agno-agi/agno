...
class TestSearchIsolationContract:
    ...
    def test_admin_user_id_none_sees_every_collection(self, populated_db):
        """``user_id=None`` is the unscoped read, so it covers every collection
        this knowledge base owns - the shared base one plus one per owner.

        Chroma has no owner column to drop a predicate from, so the unscoped
        read has to iterate the collections instead. Reading only the base one
        would leave an admin seeing strictly less than a regular user, who gets
        their own collection plus the shared one."""
