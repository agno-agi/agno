...
class TestLegacyTableMigration:
    """A table created before this branch lacks the ``user_id`` column.
    Opening it must auto-add the column (existing rows -> NULL = shared)
    so old tables keep working — inserts and scoped searches that would
    otherwise raise now succeed, and no data is dropped."""

    @pytest.fixture
    def legacy_table_path(self):
        import pyarrow as pa

        path = TEST_PATH + "_legacy"
        ...
        # Pre-isolation schema: no ``user_id`` column.
        ...
        tbl = conn.create_table(TEST_TABLE, schema=old_schema, mode="overwrite")
        import json
                    ...
                    "payload": json.dumps(
                        {
                            "name": "legacy-doc",
                            "meta_data": {},
                            "content": "legacy salary data",
                            "usage": None,
                            "content_id": "legacy-c",
                            "content_hash": "legacy-h",
                        }
                    ),
                }
            ]
        )
        assert "user_id" not in tbl.schema.names
        yield path
        if os.path.exists(path):
            shutil.rmtree(path)

    def test_open_migrates_and_preserves_data(self, legacy_table_path, mock_embedder):
        db = LanceDb(uri=legacy_table_path, table_name=TEST_TABLE, embedder=mock_embedder)

        assert db.USER_ID_COL in db.table.schema.names, "Legacy table was not migrated"
        assert db.table.count_rows() == 1, "Migration dropped existing rows"
        # The migrated row is NULL-owned (shared) — visible to any scoped search.
        rows = db.table.search().select([db.USER_ID_COL]).to_list()
        assert rows[0][db.USER_ID_COL] is None

    def test_insert_and_search_work_after_migration(self, legacy_table_path, mock_embedder):
        db = LanceDb(uri=legacy_table_path, table_name=TEST_TABLE, embedder=mock_embedder)

        # Pre-fix this insert raised (row dict carries an unknown user_id key).
        db.insert(content_hash="new-h", documents=[Document(name="new-doc", content="new")], user_id="alice")
        assert db.table.count_rows() == 2

        # Pre-fix a scoped search raised (No field named user_id). The legacy
        # row is shared, alice's new row is hers — both visible to alice.
        names = {d.name for d in db.search(query="data", limit=10, user_id="alice")}
