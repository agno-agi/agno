...
class TestLegacyTableMigration:
    ...
    must ALTER it in place so inserts and scoped searches keep working —
    instead of hard-failing on UndefinedColumn or silently returning [].
    """

    def _make_legacy_table(self, table: str):
        from sqlalchemy import create_engine
        from sqlalchemy import text as sa_text
    ...
    def test_create_migrates_and_search_works(self):
        from sqlalchemy import create_engine
        from sqlalchemy import inspect as sa_inspect
        from sqlalchemy import text as sa_text

        table = f"test_legacy_{uuid.uuid4().hex[:8]}"
        eng = self._make_legacy_table(table)
        try:
            cols_before = [c["name"] for c in sa_inspect(eng).get_columns(table, schema=TEST_SCHEMA)]
            assert "user_id" not in cols_before

            db = PgVector(table_name=table, schema=TEST_SCHEMA, db_url=PG_URL, embedder=HashEmbedder())
            db.create()  # must auto-migrate

            # Read the schema back through a fresh engine so no pooled
            # connection's pre-migration reflection can mask the new column.
            cols_after = [c["name"] for c in sa_inspect(create_engine(PG_URL)).get_columns(table, schema=TEST_SCHEMA)]
            assert "user_id" in cols_after

            db.insert(content_hash="ha", documents=_alice_docs(), user_id="alice")
            db.insert(content_hash="hb", documents=_bob_docs(), user_id="bob")
            res = {d.name for d in db.search("salary", limit=10, user_id="alice")}
            assert "alice-salary" in res
            assert "bob-salary" not in res

            # idempotent: a second create() changes nothing.
            db.create()
        finally:
            with eng.begin() as conn:
                conn.execute(sa_text(f"DROP TABLE IF EXISTS {TEST_SCHEMA}.{table};"))
