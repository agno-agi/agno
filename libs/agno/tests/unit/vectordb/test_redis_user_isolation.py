...
class TestSchemaMigration:
    """An index created before isolation lacks the ``user_id`` field. Opening
    it must add the field in place (re-index, no data loss) so legacy shared
    chunks stay searchable via ``ismissing``."""

    def test_legacy_index_migrated_in_place(self):
        from redisvl.index import SearchIndex
        from redisvl.redis.utils import array_to_buffer
        from redisvl.schema import IndexSchema

        emb = _DeterministicEmbedder()
        name = f"iso_mig_{uuid.uuid4().hex[:10]}"
        legacy = SearchIndex(
            IndexSchema.from_dict(
                {
                    ...
                    "fields": [
                        ...
                        {"name": "content_hash", "type": "tag"},
                        {
                            "name": "embedding",
                            "type": "vector",
                            "attrs": {"dims": 8, "distance_metric": "cosine", "algorithm": "flat"},
                        },
                    ],
                }
            ),
        ...
        legacy.create(overwrite=True, drop=True)
        legacy.load(
            [
                {
                    "id": "legacy1",
                    ...
                    "content_hash": "old",
                    "embedding": array_to_buffer(emb.get_embedding("legacy shared secret"), "float32"),
                }
            ],
            id_field="id",
        )

        db = RedisDB(index_name=name, redis_url=REDIS_URL, embedder=emb, search_type=SearchType.vector)
        try:
            db.create()  # should migrate in place
            db.insert(content_hash="ha", documents=[_embedded("alice-doc", "alice new secret")], user_id="alice")

            # Legacy shared row survived migration and surfaces for scoped users.
            alice = {d.name for d in db.search("secret", limit=10, user_id="alice")}
            assert "legacy-doc" in alice
            assert "alice-doc" in alice

            bob = {d.name for d in db.search("secret", limit=10, user_id="bob")}
            assert "legacy-doc" in bob
            assert "alice-doc" not in bob
        finally:
            db.drop()
