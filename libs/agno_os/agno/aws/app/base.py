...
class AwsApp(InfraApp):
    ...
    def set_aws_env_vars(self, env_dict: Dict[str, str], aws_region: Optional[str] = None) -> None:
        ...
        elif self.os_settings is not None and self.os_settings.aws_region is not None:
            # logger.debug(f"Setting AWS Region to {aws_region} using os_settings")
            env_dict[AWS_REGION_ENV_VAR] = self.os_settings.aws_region
            env_dict[AWS_DEFAULT_REGION_ENV_VAR] = self.os_settings.aws_region
