...
def log_active_os_not_available() -> None:
    logger.error("Could not find an active os. You can:")
    logger.error("- Run `ag os setup` to setup a os at the current path")
    logger.error("- Run `ag os create` to create a new os")


def print_available_os(avl_os_list) -> None:
    avl_os_names = [o.os_root_path.stem for o in avl_os_list] if avl_os_list else []
    print_info("Available os:\n  - {}".format("\n  - ".join(avl_os_names)))
