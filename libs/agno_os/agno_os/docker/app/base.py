...
from agno_os.docker.context import DockerBuildContext
from agno_os.infra.app import InfraApp
from agno_os.infra.context import ContainerContext
from agno_os.utils.logging import logger

if TYPE_CHECKING:
    from agno_os.docker.resource.base import DockerResource
...
class DockerApp(InfraApp):
    ...
    # Mount the os directory from host machine to the container
    mount_os_dir: bool = False
        ...
        container_env.update(
            {
                ...
                "MOUNT_OS_DIR": str(self.mount_os_dir),
    ...
    def get_container_volumes(self, container_context: ContainerContext) -> Dict[str, dict]:
        from agno_os.utils.defaults import get_default_volume_name
        ...
        if self.mount_os_dir:
            os_root_in_container = container_context.os_root
            os_root_on_host = str(self.os_root)
            logger.debug(f"Mounting: {os_root_on_host}")
            logger.debug(f"      to: {os_root_in_container}")
            container_volumes[os_root_on_host] = {
                "bind": os_root_in_container,
                "mode": "rw",
            }
        ...
        if self.create_volume:
            ...
            if self.volume_dir is not None:
                volume_host = str(self.os_root.joinpath(self.volume_dir))
        ...
        if self.mount_resources:
            resources_dir_path = str(self.os_root.joinpath(self.resources_dir))
        ...
        if self.mount_agno_config:
            ...
            agno_config_container_path = f"{self.os_dir_container_path}/.config/ag"
    ...
    def build_resources(self, build_context: DockerBuildContext) -> List["DockerResource"]:
        from agno_os.docker.resource.base import DockerResource
        from agno_os.docker.resource.container import DockerContainer
        from agno_os.docker.resource.network import DockerNetwork
    ...
    def get_infra_resources(self) -> Optional[Any]:
        from agno_os.docker.resources import DockerResources
